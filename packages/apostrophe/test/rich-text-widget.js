...
describe('Rich Text Widget', function () {
  ...
  describe('font size (size toolbar item)', function () {
    it('registers the size tool and adds it to the default toolbar', async function () {
      apos = await t.create({
        root: module,
        modules: {}
      });
    ...
    it('permits span font-size styling for valid pixel sizes when the size tool is enabled', async function () {
      apos = await t.create({
        root: module,
        modules: {}
      });
      ...
      // Valid pixel sizes (integer or decimal) survive sanitization.
      const valid = [ '16px', '24px', '1.5px', '0.5px', '100px' ];
    ...
    it('strips non-pixel, invalid or malicious font-size values even when the size tool is enabled', async function () {
      apos = await t.create({
        root: module,
        modules: {}
      });
      ...
      // Only pixels are allowed: other units, keywords, unit-less numbers,
      // function values and negatives must all be rejected (the font-size
      // declaration dropped).
      const invalid = [
    ...
    it('strips font-size when the size tool is not in the toolbar', async function () {
      apos = await t.create({
        root: module,
        modules: {}
      });
      ...
      // With color enabled but size disabled, the span and its color survive
      // but the font-size declaration is removed.
      const config = manager.optionsToSanitizeHtml({
        toolbar: [ 'color' ],
        insert: []
      });
