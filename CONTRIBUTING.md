...
3. Once you've completed your code changes, `git add` and `git commit` then locally. Then run the `pnpm changeset` command to generate "changesets," which will be merged into the changelog later. When you are asked if a major, minor or patchlevel bump will be required, keep in mind that only backwards incompatible changes require a major version bump, and in most cases would not be accepted without extensive discussion. New features are minor bumps and bug fixes are patchlevel bumps. Please do not directly edit the `CHANGELOG.md` file.
