// `@apostrophecms/piece-widget-type` provides widgets that display display pieces of a
...
module.exports = {
  ...
  beforeSuperClass(self, options) {
    self.piecesModuleName = options.piecesModuleName || self.__meta.name.replace(/-widget$/, '');

    self.pieces = options.apos.modules[self.piecesModuleName];
    if (!self.pieces) {
      if (!options.piecesModuleName) {
        throw new Error('The module ' + self.__meta.name + ' extends @apostrophecms/piece-widget-type, but the piecesModuleName option is not set, and we can\'t guess it from the name of your module.');
      } else {
    ...
    options.browser = _.defaults(options.browser || {}, { piecesModuleName: self.piecesModuleName });
  },
  init(self, options) {
  ...
  methods(self, options) {
    return {
      ...
      async load(req, widgets) {
        // Since we are overriding the default load method of `@apostrophecms/widget-type`, make
        ...
        if (!self.options.loadManyById) {
          ...
          await self.loadOthersOneAtATime(req, widgets);
          return self.loadOtherJoins(req, widgets);
        }
        const byId = _.filter(widgets, { by: 'id' });
        const byOther = _.difference(widgets, byId);
        await self.loadManyById(req, byId);
        await self.loadOthersOneAtATime(req, byOther);
        return self.loadOtherJoins(req, widgets);
      },
      ...
      async loadManyById(req, widgets) {

        const ids = _.reduce(widgets, function (ids, widget) {
          return ids.concat(widget.pieceIds || []);
        }, []);

        const widgetsByPieceId = {};
        ...
        const cursor = self.widgetCursor(req, { _id: { $in: ids } });
        ...
        for (const widget of widgets) {
          await self.afterLoadOne(req, widget);
        }
      },
      ...
      async loadOthersOneAtATime(req, widgets) {
        for (const widget of widgets) {
          await self.loadOne(req, widget);
          await self.afterLoadOne(req, widget);
        }
      },
      ...
      // Load additional joins in the schema, if any, beyond the `_pieces`
      // join that we have implemented ourselves with higher performance

      async loadOtherJoins(req, widgets) {
        const schema = _.filter(self.schema, function (field) {
          return field.type.match(/^join/) && field.name !== '_pieces';
        });
        return self.apos.schema.join(req, schema, widgets, undefined);
      },
      ...
      pushPieceForWidget(widget, piece) {
        if (self.joinById && self.joinById.relationship && widget.by === 'id') {
          piece._relationship = widget[self.joinById.relationshipsField] || {};
        }
      ...
      // always gets called for each. Override this method to do whatever you want.

      async afterLoadOne(req, widget) {
      },
      ...
      widgetCursor(req, criteria) {
        const filters = self.filters || (self.joinById && self.joinById.filters);
        return self.pieces.find(req, criteria).applyBuilders(filters);
      },
