...
// inheritance via `extend`/`extends`, the `init`, `adjustOptions`,
...
module.exports = function(options) {
  ...
  // `createSync`, which run init functions with and without `await`.
  ...
  self.createWithoutInit = function(className, options) {
    ...
    for (const step of steps) {
      applyOptions(step.options || {}, options);
      if (step.adjustOptions) {
        step.adjustOptions(options);
      }
    }
    ...
    for (const step of steps) {
      ...
      if (step.beforeConstruct) {
        throw new Error(step.__meta.name + ': in Apostrophe 3.x, "beforeConstruct" has been replaced with "adjustOptions". It takes only the "options" argument and should be solely concerned with modifying the options before the base class sees them.');
      }
