...
module.exports = function(options) {
  ...
  self.create = async function(className, _options) {
    ...
    for (const step of steps) {
      ...
      if (fields) {
        const valid = [ 'add', 'remove', 'arrange' ];
        if (fields.add) {
          options.addFields = (options.addFields || []).concat(fieldsToArray(fields.add));
        }
        if (fields.remove) {
          options.removeFields = (options.removeFields || []).concat(fields.remove);
        }
        if (fields.arrange) {
          options.arrangeFields = (options.arrangeFields || []).concat(Object.keys(fields.arrange).map(name => ({
            name,
            ...fields.arrange[name]
          })));
        }
