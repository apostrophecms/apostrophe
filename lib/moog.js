...
module.exports = function(options) {
  ...
  self.create = async function(className, _options) {
    ...
    const validKeys = [ '__meta', 'options', 'fields', 'beforeSuperClass', 'init', 'afterAllSections', 'extend', 'improve', 'methods', 'extendMethods' ]
    ...
    for (const step of steps) {
      ...
      // You can have access to options within a fields function, if you choose to provide one
      const fields = ((typeof step.fields) === 'function') ? step.fields(that, options) : step.fields;
      if (fields) {
        const valid = [ 'add', 'remove', 'groups' ];
        if (fields.add) {
          options.addFields = (options.addFields || []).concat(fieldsToArray(fields.add));
        }
        if (fields.remove) {
          // Rather than building up removeFields, we nick entries
          // out of addFields. This way it is properly cumulative,
          // a later module can "add" the field back
          options.addFields = options.addFields.filter(field => !fields.remove.includes(field.name));
        }
        if (fields.groups) {
          console.log('fields.groups', require('util').inspect(fields.groups, { colors: true, depth: 1 }))
          options.arrangeFields = (options.arrangeFields || []).concat(Object.keys(fields.groups).map(name => ({
            name,
            ...fields.groups[name]
          })));
        }
        for (const key of Object.keys(fields)) {
          if (!valid.includes(key)) {
            throw `${clarifyModuleName(step.__meta.name)}: ${key} is not valid inside "fields".\nPossibly you forgot to nest a field in "add".`;
          }
        }
      }
    }

    // This needs to be after the options and fields are compiled so it can manipulate
      ...
      for (const name of Object.keys(fields)) {
        ...
        if ((field.type === 'object') || (field.type === 'array')) {
          if (!Array.isArray(field.schema)) {
            field.schema = fieldsToArray(field.schema);
          }
        }
        result.push(field);
      }
