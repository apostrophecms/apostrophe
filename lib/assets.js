...
module.exports = {
  ...
  construct: function(self) {
    ...
    self.pushAsset = function(type, name, options) {
      ...
      if (exists) {
        self._assets[self._assetTypes[type].key].push({ file: filePath, web: webPath, data: data, when: when, minify: options.minify });
      }
      return exists;
    };
        ...
        minify: function(callback) {
          ...
          return self.forAllAssetScenesAndUpgrades(function(scene, callback) {
            return async.eachSeries([ 'stylesheets', 'scripts' ], function(type, callback) {
              ...
              var file = self.options.rootDir + '/public/' + folderMap[type] + '/minified-' + scene + '-' + self._generation + '.' + folderMap[type];
              if (fs.existsSync(file)) {
                ...
                self._minified[scene][type] = fs.readFileSync(file);
        ...
        minifiedStatic: function(callback) {
          _.each(self._minified, function(byType, scene) {
            _.each(byType, function(content, type) {
              ...
              var dir = self.options.rootDir + '/public/' + folderMap[type];
              if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir);
              }
              fs.writeFileSync(dir + '/minified-' + scene + '-' + self._generation + '.' + folderMap[type], content);
            });
          });
          return setImmediate(callback);
        }
