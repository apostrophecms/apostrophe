...
module.exports = function(self) {
  self._aposLocals = {
    ...
    aposSingleton: function(page, name, type, options) {
      ...
      // We must also omit all _ fields here to avoid
      // pushing massive joins into the singleton's JSON

      var del = [];
      _.each(options.options, function(val, key) {
        if (key.match(/^_/)) {
          del.push(key);
        }
      });
      _.each(del, function(key) {
