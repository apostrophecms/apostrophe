  ...
  self._aposLocals = {
    ...
    aposScripts: function(when) {
      ...
      if (self.options.minify) {
        ...
        var result = scriptTags(unminifiable);
        result += '<script src="/apos/scripts.js?pid=' + self._pid + '&when=' + when + '"></script>\n' + setScene();
        return result;
      } else {
        var minifiable = self.filterAssets(self._assets['scripts'], when);
        return scriptTags(minifiable) + setScene();
      }
    ...
    //
    // aposAreaImage({ area: page.areas.body })
    //
    // OPTIONS:
    //
    ...
    // CLASSIC SYNTAX:
    //
    // aposAreaFile({ area: page.areas.body })
    //
    // OPTIONS:
    //
    ...
    aposItemNormalView: function(item, options) {
      ...
      var response = self.partial('itemNormalView', { item: item, itemType: itemType, empty: empty, options: options, jsonOptions: jsonOptions, attributes: attributes, alwaysEditing: self.alwaysEditing });
      return response;
    },
