  ...
  init: function(self) {
    ...
    self.acceptFiles = function(req, files, options, callback) {
      ...
      return async.map(newFiles, function(file, callback) {
        // ignore query string when determining extension for better results
        var extReady = file.name.replace(/\?.*$/, '');
        var extension = path.extname(extReady);
    ...
    self.app.post('/apos/crop', function(req, res) {
      ...
      return self.cropFile(req, _id, crop, function(err, file) {
        if (err) {
          res.statusCode = 404;
    ...
    // API implementing the POST route (see above).

    self.cropFile = function(req, _id, crop, callback) {
      var file;
      return async.series([
        function(callback) {
          return callback(self.permissions.can(req, 'edit-file') ? null : 'forbidden');
    ...
    // from mongodb. This ain't the "trash can," it's gone forever. If
    // options.missingOk is true, there is no error if the physical files
    // are gone already (uploadfs errors are ignored).

    self.deleteFileForever = function(file, options, callback) {
      var info = file;
      return async.series({
        deleteOriginal: function(callback) {
          var name = '/files/' + info._id + '-' + info.name + '.' + info.extension;
          return self.uploadfs.remove(name, function(err) {
        ...
        deleteDb: function(callback) {
          return self.files.remove({ _id: file._id }, callback);
        }
      }, callback);
    };
    ...
    self.filePath = function(file, options) {
      ...
      if (!file) {
        if (options.def === undefined) {
          throw new Error('You did not pass a file object to apos.filePath(). This probably means you are trying to get a filepath for an area that doesn\'t have a file in it (probably via aposAreaImagePath). If you want to deal with that yourself, you can pass a `def` option as a fallback.');
        } else {
          return options.def;
        }
      }
