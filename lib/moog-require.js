...
module.exports = async function(options) {
  ...
  self.define = async function(type, definition, extending) {
    ...
    if (options.nestedModuleSubdirs) {
      if (!self._indexes) {
        // Fetching a list of index.js files on the first call and then searching it
        // each time for one that refers to the right type name shaves as much as
        // 60 seconds off the startup time in a large project, compared to using
        // the glob cache feature
      ...
      const matches = self._indexes.filter(function(index) {
        ...
        // making sure "type" is not preceded by an npm namespace folder. If type is
        // itself namespaced this never comes up (because npm namespaces don't nest).
        // The risk is that a legitimate project level module that happens to end with
        // the same name as a namespaced module will be rejected as a duplicate when
        // nestedModuleSubdirs is present
