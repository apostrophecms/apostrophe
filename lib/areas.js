...
module.exports = function(self) {
    ...
    function permissions(callback) {
      return self.get(req, { slug: pageSlug }, { editable: true, fields: { _id: 1 } }, function(err, results) {
        ...
        if (!results.pages.length) {
          ...
          // due to nonexistence
          return self.pages.findOne({ slug: pageSlug }, { _id: 1 }, function(err, _page) {
            if (err) {
              return callback(err);
            }
            if (!_page) {
            ...
            // Exists and we're just not cool enough to edit it
            return callback('forbidden');
          });
        }
        // Check for conflict with a schema property that isn't an area
        var page = results.pages[0];
        if (page[areaSlug] && (page[areaSlug]['type'] !== 'area')) {
          return callback('forbidden area name, conflicts with non-area property: ' + areaSlug);
        }
  ...
  // Returns a plaintext representation of the area's contents. Not all widgets have
  // a plaintext representation. Plaintext means truly plain, so if you want to
  // escape entities for display of plaintext as part of a webpage that is your responsibility;
  // use the `e` filter of Nunjucks. You may supply a "truncate" option with a
  // number of characters to restrict the output to (breaking at the nearest word).
  // You must supply an "area" option.
