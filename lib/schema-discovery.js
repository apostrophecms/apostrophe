...
export class SchemaDiscovery {
  ...
  getModuleFields(module) {
    ...
    // Fallback: try to access the schema through the module's self reference
    if (module.self && Array.isArray(module.self.schema)) {
      console.log(`Using self.schema for ${module.name || 'unknown'} (${module.self.schema.length} fields)`);

      const fields = module.self.schema
        .filter(f => f && f.name)
        .reduce((acc, f) => {
          acc[f.name] = f;
          return acc;
    ...
    // Another fallback: check if the module has a getSchema method
    ...
    // Final fallback: traverse the inheritance chain manually
    console.warn(`No compiled schema found for ${module.name || 'unknown'}, using manual field collection`);
    const allFields = {};

    // Get the module's inheritance chain
    const chain = this.getInheritanceChain(module);

    // Collect fields from all modules in the chain (base to derived)
    for (const moduleInChain of chain) {
      const moduleFields = this.getDirectModuleFields(moduleInChain);
      Object.assign(allFields, moduleFields);
    }

    this.addImplicitCoreFields(allFields, module);
    return allFields;
  }
  ...
  addImplicitCoreFields(fields, module) {
    ...
    // Add piece-specific implicit fields
    if (this.isPieceType(module)) {
      const pieceFields = {
        'published': {
          name: 'published',
          type: 'boolean',
      ...
      for (const [fieldName, fieldDef] of Object.entries(pieceFields)) {
        if (!fields[fieldName]) {
          fields[fieldName] = fieldDef;
        }
      }
  ...
  /**
   * Get fields directly defined on a specific module (not inherited).
   *
   * @param {any} module
   * @returns {Record<string, any>}
   */
  getDirectModuleFields(module) {
    const fields = {};

    // Check various places where fields might be defined
    if (module.options?.fields?.add) {
      Object.assign(fields, module.options.fields.add);
    }

    if (module.options?.addFields) {
      Object.assign(fields, module.options.addFields);
    }

    // Some modules might have fields defined differently
    if (module.fields) {
      Object.assign(fields, module.fields);
    }
  ...
  /**
   * Get the inheritance chain for a module, from base classes to the current module.
   * This ensures we collect fields from all parent classes.
   *
   * @param {any} module
   * @returns {Array<any>}
   */
  getInheritanceChain(module) {
    const chain = [];

    // Try to get the inheritance chain from module metadata
    if (module.__meta?.chain) {
      // __meta.chain contains module names, we need to resolve them to actual modules
      for (const moduleName of module.__meta.chain) {
        const moduleInChain = this.apos.modules[moduleName];
        if (moduleInChain) {
          chain.push(moduleInChain);
        }
      }
    } else {
      // Fallback: traverse manually using extend option
      this.buildInheritanceChainRecursive(module, chain, new Set());
    }
  ...
  /**
   * Recursively build inheritance chain when __meta.chain is not available.
   *
   * @param {any} module
   * @param {Array<any>} chain
   * @param {Set<string>} visited - Prevent infinite loops
   */
  buildInheritanceChainRecursive(module, chain, visited) {
    if (!module || !module.name || visited.has(module.name)) {
      return;
    }

    visited.add(module.name);

    // Add parent first (base classes before derived classes)
    const extendModule = module.options?.extend;
    if (extendModule && typeof extendModule === 'string') {
      const parentModule = this.apos.modules[extendModule];
      if (parentModule) {
        this.buildInheritanceChainRecursive(parentModule, chain, visited);
      }
    }
  ...
  convertFieldsToOpenApiSchema(fields, moduleName, module) {
    ...
    for (const [fieldName, field] of Object.entries(fields)) {
      ...
      if (field.name && field.name.startsWith('_') && field.name !== '_id') continue;
