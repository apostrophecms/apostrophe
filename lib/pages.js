...
module.exports = function(self) {
  ...
  self.get = function(req, userCriteria, options, mainCallback) {
    ...
    function count(callback) {
      self.pages.find(criteria, {}, findOptions).count(function(err, count) {
        results.total = count;
        return callback(err);
      });
    }
    ...
    function loadPages(callback) {
      var q = self.pages.find(criteria, projection, findOptions);
  ...
  self.hintGetCriteria = function(criteria) {
    ...
    var hint = self._scanCriteriaFor(criteria, { 'path': 1, 'slug': 1 });
    console.log(hint);
    return hint;
  };
