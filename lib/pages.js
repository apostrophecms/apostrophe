...
module.exports = function(self) {
  ...
  self.get = function(req, userCriteria, options, mainCallback) {
    ...
    var areas = (options.areas !== undefined) ? options.areas : true;
    ...
    async.series([fetch, cleanPages, markPermissions, loadWidgets], done);

    function fetch(callback) {
      return self.getBackend(criteria, projection, skip, limit, sort, function(err, answer) {
        if (err) {
          return callback(err);
        }
        results.total = answer.count;
        results.pages = answer.results;
        return callback(null);
      });
    }
  ...
  self.getBackend = function(criteria, projection, skip, limit, sort, callback) {

    var count = true;
    if ((skip === undefined) && (limit === undefined)) {
      // Why query twice if we're getting everything anyway?
      count = false;
    }
    ...
    return async.series({
      count: function(callback) {
        ...
        return self.pages.count(criteria, function(err, _count) {
          answer.count = _count;
          return callback(err);
        });
      },
      find: function(callback) {
        ...
        return q.toArray(function(err, _results) {
          answer.results = _results;
          return callback(err);
        });
      }
    }, function(err) {
