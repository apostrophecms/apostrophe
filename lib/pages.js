...
module.exports = function(self) {
  ...
  self.versionPage = function(req, pageOrSlug, callback) {
    ...
    return async.series([findPage, addVersion, pruneVersions], callback);
  };
  ...
  //
  // `options.q` searches for a particular string using MongoDB's
  // full-text search capabilities. Results are weighted toward titles
  // first, then other metadata, then area text. `options.titleSearch`
  // is accepted as a synonym for backwards compatibility and no longer
  // performs an inefficient search on titles only.
  //
  ...
  self.get = function(req, userCriteria, options, mainCallback) {
    ...
    // titleSearch is a synonym for q for bc
    var q = options.q || options.titleSearch;
    ...
    if (sort === false) {
      ...
      // specify a sort or specify sort: 'q', sort by
    ...
    async.series([count, loadPages, markPermissions, loadWidgets, afterGet], done);
  ...
  self.hintGetCriteria = function(criteria) {
    ...
    // (cue Master Shake)
    if (self._scanCriteriaFor(criteria, { $text: 1 })) {
      return undefined;
    }
  ...
  self.callLoadersForPage = function(req, page, callback) {
    ...
    if (req.loaderRecursion[page._id]) {
      if (req.loaderRecursion[page._id] === maxLoaderRecursion) {
    ...
    // Assumption is that the number of areas in a single page is not ridiculous so we
    // can safely do them in parallel
    async.map(
      _.values(areas),
