...
module.exports = {
  construct: function(self) {
    ...
    // Sanitizes options. Use `options.prefix` for autocomplete. Use
    // `options.tags` to limit results to an array of tag names of
    // interest. You may not combine these options. The `options` argument
    // may be skipped.
    //
    // Tags are searched for in the pages, files, videos, and allowedTags
    // collections.
    //
    // Additional collections can be added via the `addTaggables` option
    // when configuring Apostrophe.
  ...
  init: function(self) {
    ...
    self.app.all('/apos/autocomplete-tag', function(req, res) {
      ...
      return self.getTags({ prefix: data.term }, function(err, tags) {
        if (err) {
