...
module.exports = function(self) {
  ...
  self.render = function(req, res, template, info) {
    return res.send(self.partial(req, res, template, info));
  };
  ...
  self.partial = function(req, res, name, data, dirs) {
    if(typeof req === 'string'){
      // in case we didn't get any req or res objects
      dirs = name;
      data = res;
      name = req;
      req = null;
      res = null;
    } else if(typeof req === 'object' && typeof res === 'object'){
      self.initI18nLocal(req, res);
    }
    ...
    data.partial = function(name, data) {
      return self.partial(name, data, dirs);
    };
  ...
  self.decoratePageContent = function(args, req, res) {
    ...
    if (args.refreshing) {
      ...
      if (typeof(self.options.outerLayout) === 'function') {
        ...
        return self.partial(req, res, self.options.outerLayout || 'outerLayout', args);
      }
    }
  };
