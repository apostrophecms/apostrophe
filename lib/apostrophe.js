...
function Apos() {
  ...
  self.init = function(options, callback) {
    ...
    if (!options.generation) {
      self._generation = self._pid;
      // In dev this folder can otherwise accumulate
      // old assets forever
      if (fs.existsSync(self.options.rootDir + '/public/apos-minified')) {
        var oldFiles = glob.sync(self.options.rootDir + '/public/apos-minified/*');
        _.each(oldFiles, function(file) {
          fs.unlinkSync(file);
        });
      }
    } else {
      self._generation = options.generation;
    }
