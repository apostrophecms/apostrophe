...
module.exports = function(self) {
  ...
  self.addMigration = function(name, fn) {
    self._migrations[name] = fn;
  };
  ...
  self.addMigration('addHighSearchWords', function addHighSearchWords(callback) {
    var needed = false;
  ...
  self.migrate = function(callback) {
    console.log('Migrating...');
    var cache = self.getCache('migrations');
    return async.eachSeries(_.keys(self._migrations), function(name, mainCallback) {
      return async.series({
        ...
        runMigration: function(callback) {
          ...
          return self._migrations[name](callback);
        },
