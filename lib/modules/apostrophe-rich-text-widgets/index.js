...
module.exports = {
  ...
  minimumDefaultOptions: {
    toolbar: [ 'styles', '|', 'bold', 'italic', 'strike', 'link', '|', 'bullet_list', 'ordered_list', '|', 'blockquote', 'code_block', '|', 'horizontal_rule', '|', 'undo', 'redo' ],
    styles: [
  ...
  construct: function(self, options) {
    ...
    self.sanitize = async function(req, input, options) {
      const output = await superSanitize(req, input, options);
    ...
    self.compare = function(old, current) {
      ...
      const diff = jsDiff.diffSentences(self.apos.utils.htmlToPlaintext(oldContent).replace(/\s+/g, ' '), self.apos.utils.htmlToPlaintext(currentContent).replace(/\s+/g, ' '), { ignoreWhitespace: true });
    ...
    self.isEmpty = function(widget) {
      const text = self.apos.utils.htmlToPlaintext(widget.content || '');
