$(function() {
  ...
  $('body').on('click', '[data-i18n-tooltips-toggle]', function() {
    ...
    retrieve(function(err) {
      ...
      $('[data-i18n-tooltips-toggle]').prop('disabled', false);
      if (keys) {
        $('[data-i18n-tooltips-toggle="keys"]').addClass('apos-active');
        $('[data-i18n-tooltips-toggle="keys"]').prop('disabled', true);
      } else {
        $('[data-i18n-tooltips-toggle="translations"]').addClass('apos-active');
        $('[data-i18n-tooltips-toggle="tranlsations"]').prop('disabled', true);
      }
    });
    return false;
  });
  ...
  function retrieve(callback) {
    return apos.utils.post('/modules/apostrophe-i18n-tooltips/fetch', {}, function(err, data) {
      if (err) {
        return callback(err);
      }
