...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    const superGetCreateSingletonOptions = self.__meta.getCreateSingletonOptions;
    self.getCreateSingletonOptions = function(req) {
      const options = superGetCreateSingletonOptions(req);
      options.poll = self.shouldPoll(req);
      options.longPolling = self.options.longPolling;
      return options;
    };

    // By default, we poll for notifications for all users. However this
    // method is provided as a project level override point where req.user
    // may be inspected to determine if polling for notifications is
    // appropriate

    self.shouldPoll = function(req) {
      return true;
    };
    ...
    self.expressMiddleware = function(req, res, next) {
      ...
      function attempt() {
        if ((self.options.longPolling !== false) && (Date.now() - start >= (self.options.longPollingTimeout || 10000))) {
          return res.send({
            status: 'ok',
            notifications: [],
            dismissed: []
        ...
        return Promise.try(function() {
          ...
          if ((!notifications.length) && (!dismissed.length)) {
            if (self.options.longPolling === false) {
              return res.send({
                status: 'ok',
                notifications: [],
                dismissed: []
