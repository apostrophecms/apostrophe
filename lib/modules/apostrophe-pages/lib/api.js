...
module.exports = function(self, options) {
  ...
  self.insert = async function(req, parentOrId, page, options) {
    ...
    return await self.withLock(req, body);

    async function body() {
      let parent = await getParent();
      page.rank = await determineNextRank();
      await self.emit('beforeInsert', req, page, options);
    ...
    async function getParent() {
      if (typeof (parentOrId) === 'object') {
        ...
        const parent = await cursor.toObject();
        if (!parent) {
    ...
    async function determineNextRank() {
  ...
  self.move = async function(req, movedId, targetId, position, options) {
    ...
    async function body() {
      ...
      const oldParent = page._ancestors[0];
      ...
      async function getMoved() {
        const moved = await return self.find(req, mergeCriteria({ _id: movedId }))
          .permission(false)
          .trash(null)
          .published(null)
          .areas(false)
  ...
  self.moveToTrash = async function(req, _id) {
    if (self.apos.docs.trashInSchema) {
      return await self.trashInSchema(req, _id, true);
    } else {
      return await self.moveToSharedTrash(req, _id);
    }
  };
  ...
  self.trashInSchema = async function(req, _id) {
    let page, parent;
    let tree = [];
    ...
    async function trashOrUntrashPages() {
      ...
      const res = await self.apos.docs.db.updateMany({ _id: { $in: ids } }, action);
    }

    // iterate over page tree and deduplicate requisite fields
    async function dedupePages() {
      return await self.deduplicatePages(req, tree, toTrash);
    }
  ...
  self.deduplicatePages = async function(req, pages, toTrash) {
    ...
    for (page of pages) {
      ...
      async function deduplicate() {
        if (toTrash) {
          return await self.apos.docs.getManager(page.type).deduplicateTrash(req, page);
        } else {
          return await self.apos.docs.getManager(page.type).deduplicateRescue(req, page);
        }
      }
  ...
  self.rescueInTree = async function(req, _id) {
    return await self.trashInSchema(req, _id, false);
  };
  ...
  self.moveToSharedTrash = async function(req, _id) {
    ...
    async function findTrash() {
      ...
      return await self.find(req, { trash: true, level: 1 })
        .permission(false)
        .published(null)
        .trash(null)
        .areas(false)
    ...
    function findPage() {
      ...
      return await self.find(req, { _id: _id }).permission('edit').ancestors({ depth: 1, published: null, trash: null, areas: false }).toObject();
    }

    function movePage() {
      return await self.move(req, page._id, trash._id, 'inside');
    }
  ...
  self.deleteFromTrash = async function(req, _id) {
    ...
    async function findPage() {
      // Also checks permissions
      return await self.find(req, { _id: _id }).permission('publish').trash(true).ancestors({ depth: 1, published: null, trash: null, areas: false }).toObject();
    }
  ...
  self.serve = async function(req, res) {
    ...
    try {
      ...
      await self.serveDeliver(err);
      return;
    }
    await self.serveDeliver(null);
  };
  ...
  self.serveDeliver = async function(req, err) {
    ...
    return await self.sendPage(req, req.template, args);
  ...
  self.isFound = function(req) {
    let found = req.loginRequired || req.insufficient || req.redirect || (req.data.page && (!req.notFound));
    return found;
  };
  ...
  self.updateDescendantsAfterMove = async function(req, page, originalPath, originalSlug, options) {
    ...
    let done = false;
    let cursor = self.apos.docs.db.findWithProjection(mergeCriteria({ path: matchParentPathPrefix }), { slug: 1, path: 1, level: 1 });
    while (!done) {
      ...
      await self.apos.docs.retryUntilUnique(req, desc, updateDescendant);

      async function updateDescendant() {
  ...
  self.manageOrphans = async function() {
    ...
    async function patchUndefined() {
      return await self.apos.docs.db.updateMany({
        type: { $exists: 0 }
      }, {
        $set: {
          type: 'WASUNDEFINED'
    ...
    function find() {
      return await self.apos.docs.db.distinct('type');
    }
  ...
  self.implementParkOne = async function(req, item) {
    ...
    async function findParent() {
      ...
      return await self.find(req, { slug: parentSlug })
        .joins(false)
        .areas(false)
        .published(null)
        .permission(false)
    ...
    async function findExisting() {
      ...
      return await self.find(req, criteria)
        .joins(false)
        .areas(false)
        .published(null)
        .permission(false)
    ...
    async function backupFindExisting() {
      ...
      if (existing && existing.parkedId && (existing.parkedId !== item.parkedId)) {
        throw new Error('The slug you wish to use for a parked page with the parkedId ' + _existing.parkedId + ' is already in use by a parked page that has a different parkedId.');
      }
      return existing;
    }
    ...
    async function insert() {
      ...
      if (!parent) {
        return await self.apos.docs.insert(req, item);
      } else {
        return await self.insert(req, parent, item);
      }
    }
  ...
  self.registerGenericPageType = async function(type) {
    ...
    const manager = await self.apos.create(typeName, {
      apos: self.apos
    });
  ...
  self.batchSimpleRoute = async function(req, name, change) {
    ...
    async function runJob() {
      return await self.apos.modules['apostrophe-jobs'].run(req, ids, one, {
        labels: {
