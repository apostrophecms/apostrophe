  ...
  self.trashInSchema = function(req, _id, toTrash, callback) {
    ...
    return async.series(
      [
        ...
        // iterate over pages array: deduplicate requisite fields and flag as trash
        function dedupeAndTrashPages(cb) {
          children.push(page);
          return self.dedupeAndTrashPages(req, children, toTrash, (err, res) => {
            return cb(err);
          });
        }
      ], err => {
        return callback(err, parent && parent.slug, children); // @@TODO PAUL check that children is equivalent to changed in the old implementation
      });
  };

  self.dedupeAndTrashPages = function(req, pages, toTrash, callback) {
    return async.forEach(pages, (page, cb) => {
      if (toTrash) {
        // @@TOM still a little unclear if I need to grab the manager per page OR
        // if we are EXTENDING the manager here already -- just to use self.deduplicateTrash
