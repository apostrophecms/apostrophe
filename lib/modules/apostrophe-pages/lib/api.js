...
module.exports = function(self, options) {
  ...
  self.moveToTrash = function(req, _id, callback) {
    ...
    function findTrash(callback) {
      ...
      self.find({ trash: true, level: 1 }).permissions(false).toObject(function(err, _trash) {
        if (err || (!_trash)) {
    ...
    function findPage(callback) {
      ...
      self.find({ _id: _id }).permission('edit').ancestors({ depth: 1 }).toObject(function(err, _page) {
        if (err || (!_page)) {
    ...
    function movePage(callback) {
      self.move(req, page._id, trash._id, 'inside', function(err, changedArg) {
        if (err) {
          return callback(err);
        }
        changed = changedArg;
