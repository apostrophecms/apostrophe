...
module.exports = function(self, options) {
  ...
  // This method creates a new object suitable to be inserted
  // as a child of the specified parent with properties copied
  // from the specified sibling, via insert(). It DOES NOT
  // insert it at this time. If the parent page is locked down
  // such that no child page types are permitted, this method
  // returns null. The permissions of the new child page match
  // the permissions of the parent, not the sibling.

  self.newSibling = function(parentPage, siblingPage) {
    var child = self.newChild(parentPage);
    if (!child) {
      return null;
    }
    _.assign(child, _.omit(siblingPage,
      '_id',
      'path',
      'rank',
      'loginRequired',
    ...
    child = self.apos.utils.clonePermanent(child);
    return child;
  };
  ...
  self.pruneCurrentPageForBrowser = function(page) {
    ...
    page = _.pick(page, 'title', 'slug', '_id', 'type', '_ancestors', '_url');

    // Limit information about ancestors to avoid
    // excessive amounts of data in the page
    page._ancestors = _.map(page._ancestors, function(ancestor) {
