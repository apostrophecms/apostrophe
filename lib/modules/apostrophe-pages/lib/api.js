...
module.exports = function(self, options) {
  ...
  self.trashInTreeBody = function(req, _id, trash, callback) {
    ...
    var matchParentPathPrefix = new RegExp('^' + self.apos.utils.regExpQuote(page.path + '/'));
    ...
    function findDescendants(callback) {
      return self.apos.docs.db.find({ path: matchParentPathPrefix }, { slug: 1, _id: 1 }).toArray(function(err, _changed) {
        if (err) {
          return callback(err);
        }
        changed = _changed;
