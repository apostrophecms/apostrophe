...
module.exports = function(self, options) {
  ...
  self.newChild = function(parentPage) {
    ...
    if (!pageType) {
      self.apos.warn('No allowed Page types are specified.');
      return null;
    }
  ...
  self.moveToTrash = function(req, _id, callback) {
    if (self.apos.docs.trashInSchema) {
      return self.trashInSchema(req, _id, callback);
    } else {
  ...
  self.trashInSchema = function(req, _id, callback) {
    return self.trashInSchemaBody(req, _id, true, callback);
  };
  ...
  self.rescueInTree = function(req, _id, callback) {
    return self.trashInSchemaBody(req, _id, false, callback);
  };

  // Implementation detail of `trashInSchema` and `rescueInTree`. Accepts a boolean
  // to do either.
  self.trashInSchemaBody = function(req, _id, trash, callback) {
    var page;
    var parent;
    var changed;
    var matchPathPrefix;
    var action = trash ? { $set: { trash: true } } : { $unset: { trash: 1 } };
    return async.series([ findPage, trashPage, trashDescendants, findDescendants ], function(err) {
      return callback(err, parent && parent.slug, changed);
    });
    function findPage(callback) {
      // Also checks permissions
      return self.find(req, { _id: _id }).permission('edit').trash(null).ancestors({ depth: 1, published: null, trash: null, areas: false }).toObject(function(err, _page) {
        if (err) {
          return callback(err);
        }
        if (!_page) {
        ...
        matchPathPrefix = new RegExp('^' + self.apos.utils.regExpQuote(page.path + '/'));
        if (!parent) {
          return callback('Cannot move the home page to or from the trash');
        }
        return callback(null);
      });
    }
    function trashPage(callback) {
      return self.apos.docs.db.update({ _id: _id }, action, callback);
    }
    function trashDescendants(callback) {
      return self.apos.docs.db.update({ path: matchPathPrefix }, action, { multi: true }, callback);
    }
    function findDescendants(callback) {
      return self.apos.docs.db.find({ path: matchPathPrefix }, { slug: 1, _id: 1 }).toArray(function(err, _changed) {
        if (err) {
          return callback(err);
        }
        changed = _changed;
  ...
  self.manageOrphans = function(callback) {
    ...
    function manage(callback) {
      ...
      self.apos.error('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *');
      self.apos.error('HEY DEVELOPER: the following types exist in your aposDocs collection,');
      self.apos.error('but are not managed by any module. A generic manager is being stubbed in');
      self.apos.error('for each one to keep your site from crashing, but you should remove these');
      self.apos.error('permanently from the database OR manage them properly by:\n');
      self.apos.error('* Listing them as "types" for "apostrophe-pages", OR');
      self.apos.error('* Providing an appropriate subclass of apostrophe-pieces.');
      self.apos.error();
      self.apos.error(orphans.join(', '));
      self.apos.error();
      if (_.contains(orphans, 'WASUNDEFINED')) {
        self.apos.error('Some of your docs had no type at all. The type WASUNDEFINED');
        self.apos.error('has been set for them to allow a generic manager to work.');
      }
      self.apos.error('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *');
