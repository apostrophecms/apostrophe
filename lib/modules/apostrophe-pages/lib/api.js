...
module.exports = function(self, options) {
  ...
  self.pageBeforeSend = function(req, callback) {
    ...
    // Check if level is 0, then we are on the home page. Set the body class to
    // apos-homepage so we can pick it up in JavaScript on the browser side.
    // The admin bar usses this to stay open if configured by the user
    if (req.data.page && req.data.page.level === 0) {
      self.apos.templates.addBodyDataAttribute(req, 'apos-home');
    } else {
      self.apos.templates.addBodyDataAttribute(req, 'apos-subpage');
    }
