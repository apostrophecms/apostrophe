...
module.exports = function(self, options) {
  ...
  self.apos.tasks.add('apostrophe-pages', 'find-detached',
    'Usage: node app apostrophe-pages:find-detached\n\n' +
    'This locates any pages that are not properly attached to the page tree\n' +
    'and prints their slugs. If workflow is present, the analysis is done\n' +
    'across all locales.',
    function(apos, argv) {
      // Wrapping a method makes it easy to override
      // that method
      return self.findDetachedTask();
    }
  );

  self.findDetachedTask = function() {
    let locales = [ null ];
    if (self.apos.modules['apostrophe-workflow']) {
      locales = Object.keys(self.apos.modules['apostrophe-workflow'].locales);
    }
    return Promise.mapSeries(locales, function(locale) {
      return Promise.try(function() {
        ...
        return self.apos.docs.db.find(criteria).project({ path: 1, rank: 1, level: 1, workflowGuid: 1, workflowLocale: 1 }).sort({ level: 1, rank: 1 }).toArray();
      }).then(function(pages) {
        const parents = {};
        const nextChildRank = {};
        _.each(pages, function(page) {
          if (!page.path) {
            console.error('Page has slug beginning with / but has no path property:', page);
            return;
          }
          if (page.path === '/') {
            if (page.rank !== 0) {
              console.error('Home page has inappropriate rank:', page);
            }
            if (page.level !== 0) {
              console.error('Home page has inappropriate rank:', page);
            }
            parents[page.path] = page;
            nextChildRank[page.path] = 0;
          } else {
            const slashes = page.path.replace(/[^\/]/g, '').length;
            let parentPath = page.path.replace(/\/[^\/]+$/, '');
            if (parentPath === '') {
              parentPath = '/';
            }
            if (path === parentPath) {
              console.error('Page path does not equal / or end with /non-slash-characters:', page);
            }
            if (!page.level) {
              console.error('Page path does not equal / yet level is 0 or missing:', page);
            }
            if (slashes !== page.level) {
              console.error('Number of slashes in path does not match level:', page);
            }
            if (!parents[parentPath]) {
              console.error('There is no parent page with the expected path ' + parentPath + ':', page);
            }
            // if (page.rank !== nextChildRank[page.path]) {
            //   console.error('The rank does not match that expected for the next child:', page);
            // } else {
            //   nextChildRank[page.path]++; 
            // }
            parents[page.path] = page;
            nextChildRank[page.path] = 0;
          }
