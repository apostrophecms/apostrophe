...
module.exports = function(self, options) {
  ...
  self.newChild = function(parentPage) {
    ...
    if (!pageType) {
      console.warn('No allowed Page types are specified.');
      return null;
    }
  ...
  self.implementParkOne = function(req, item, callback) {
    ...
    return async.series({
      ...
      findExisting: function(callback) {
        // Tolerate presence or absence of a trailing slash on the
        // existing page, to reduce false negatives
        return self.find(req, { slug: new RegExp('^' + self.apos.utils.regExpQuote(item.slug.replace(/\/$/, '')) + '/?$') }).published(null).permission(false).trash(null).toObject(function(err, _existing) {
          if (err) {
            return callback(err);
          }
          existing = _existing;
