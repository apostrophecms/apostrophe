...
module.exports = function(self, options) {
  ...
  self.movePermissions = function(req, moved, data, options, callback) {
    ...
    if (workflow && workflow.options.autoCommitPageMoves) {
      console.log('workflow.options ===> ', require('util').inspect(workflow.options, { colors: true, depth: 0 }));
    }
  ...
  self.requireEditor = function(req, res, next) {
    console.log('=============> PAGES REQUIRE EDITOR <================');
    if (!self.apos.permissions.can(req, 'edit-apostrophe-page')) {
      console.log('=============> edit apos page forbidden <================');
      return res.send({
        status: 'forbidden'
      });
    }
