...
module.exports = function(self, options) {
  ...
  self.move = function(req, movedId, targetId, position, options, callback) {
    ...
    function body(callback) {
      ...
      function moveSelf(callback) {
        ...
        if (parent._id !== oldParent._id && !options.slugUpdated) {
  ...
  self.movePermissions = function(req, moved, data, options, callback) {
    ...
    if (
      (data.oldParent._id !== data.parent._id) &&
      (data.parent.type !== 'trash') &&
      (!data.parent._edit)
    ) {
      return callback('forbidden');
    }

    if (workflow && workflow.options.autoCommitPageMoves) {
      const livifiedMoved = self.apos.utils.clonePermanent(moved);

      livifiedMoved.workflowLocale = workflow.liveify(moved.workflowLocale);

      if (!self.apos.permissions.can(req, 'edit-apostrophe-page', livifiedMoved)) {
