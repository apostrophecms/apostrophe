...
module.exports = function(self, options) {
  ...
  // Park all parked pages, unless `options.autoPark` is explicitly `false`. Invoked
  // for you at startup. You can run the `apostrophe-pages:park` command line task to park pages
  // at a time of your choosing.

  self.autoPark = function(callback) {
    if (self.options.autoPark === false) {
      return callback(null);
    }
    return self.implementParkAll(callback);
  };

  // Implements the page parking mechanism. This is invoked via `autoPark` at startup
  // unless the `autoPark` option is explicitly false. It is also invoked by the
  // `apostrophe-pages:park` command line task. You should not need to call this yourself.
  ...
  // Implements the page parking mechanism for a specific page. Invoked via
  // `implementParkAll`; you normally won't need to call this yourself.
  ...
  self.apos.tasks.add('apostrophe-pages', 'park',
    'Usage: node app apostrophe-pages:park\n\n' +
    'This parks pages configured for automatic creation and fixed positioning via the park option.',
  ...
  self.apos.tasks.add('apostrophe-pages', 'unpark',
    'Usage: node app apostrophe-pages:unpark --parkedId=xxx [/page/slug]\n\n' +
    'This unparks a page that was formerly locked in a specific\n' +
    'position in the page tree. You should remove it from your "park" option first.\n\n' +
    'You should use the --parkedId option. Support\n' +
    'for specifying a slug is a legacy feature and does not always work well with workflow.',
    function(apos, argv, callback) {
      // Wrapping a method makes it easy to override
      // that method
  ...
  self.unparkTask = function(callback) {
    var criteria = {};
    if (self.apos.argv.parkedId) {
      criteria.parkedId = self.apos.argv.parkedId;
    } else {
      if (!self.apos.argv._[1]) {
        return callback('You must specify either --parkedId=xxx (preferred) or a page slug (deprecated).');
      }
      criteria.slug = self.apos.argv_[1];
      console.warn('Note: use of a page slug with this task is deprecated. All of your\n' +
        'parked pages should have a parkedId, and you should use --parkedId=xxx\n' +
        'to specify one.');
    }
    var identifier = self.apos.argv._[1];
    ...
    },
    {
      ...
      if (!count) {
        return callback('No pages found matching your criteria.');
      }
      return callback(null);
    });
  };
