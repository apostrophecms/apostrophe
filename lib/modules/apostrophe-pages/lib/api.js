...
module.exports = function(self, options) {
  ...
  self.serveGetPage = function(req, callback) {
    ...
    var cursor = self.find(req, { slug: req.slug });
  ...
  self.implementParkOne = function(req, item, mainCallback) {
    var parent;
    return async.series({
      findParent: function(callback) {
        ...
        return self.find(req, { slug: parentSlug }).published(null).permission(false).toObject(function(err, _parent) {
          if (err) {
            return callback(err);
          }
          parent = _parent;
      ...
      findExisting: function(callback) {
        return self.find(req, { slug: item.slug }).published(null).permission(false).toObject(function(err, existing) {
          if (err) {
            return callback(err);
          }
          if (!existing) {
          ...
          // Just mark the page as parked.
          // This is useful for parking a previously
          // mobile page. Then bail all the way out
