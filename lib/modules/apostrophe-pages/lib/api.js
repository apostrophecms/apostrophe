...
module.exports = function(self, options) {
  ...
  self.serveGetPage = function(req, callback) {
    ...
    let filters = self.getServePageFilters();

    let cursor = self.find(req);
  ...
  // This methos is called automatically every
  ...
  self.manageOrphans = function(callback) {
    let managed = self.apos.docs.getManaged();
    let distinct;
    ...
    function manage(callback) {
      let orphans = _.difference(distinct, managed);
      if (!orphans.length) {
        return setImmediate(callback);
      }
  ...
  self.registerGenericPageTypes = function(callback) {
    let types = _.pluck(self.typeChoices, 'name');
