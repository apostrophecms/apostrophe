...
module.exports = function(self, options) {
  ...
  // had to move too). If the `trashInTree: true` option was
  ...
  self.moveToTrash = function(req, _id, callback) {    
    if (self.options.trashInTree) {
      return self.trashInTree(req, _id, callback);
    } else {
  ...
  // when the `trashInTree` flag is in effect. The home page
  ...
  self.trashInTree = function(req, _id, callback) {
    return self.trashInTreeBody(req, _id, true, callback);
  };

  // Rescue a page previously trashed via `trashInTree`. This is an operation that only
  // makes sense when the `trashInTree` option flag is set for the module.
  ...
  self.rescueInTree = function(req, _id, callback) {
    return self.trashInTreeBody(req, _id, false, callback);
  };
  
  // Implementation detail of `trashInTree` and `rescueInTree`. Accepts a boolean
  // to do either.
  self.trashInTreeBody = function(req, _id, trash, callback) {
    var page;
    var parent;
    var changed;
    var matchPathPrefix;
  ...
  // Implements `moveToTrash` when `trashInTree` is false (the default),
  ...
  self.serveGetPage = function(req, callback) {
    ...
    req.slug = self.removeTrailingSlugSlashes(req.slug);
