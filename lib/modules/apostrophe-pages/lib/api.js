...
module.exports = function(self, options) {
  ...
  self.manageOrphans = function(callback) {
    ...
    function manage(callback) {
      ...
      return async.eachSeries(orphans, function(name, callback) {
        return self.registerGenericPageType({ name: name }, callback);
      }, callback);
    }
  };
  ...
  self.registerGenericPageTypes = function(callback) {
    return async.eachSeries(self.typeChoices, self.registerGenericPageType, callback);
  };
  ...
  self.registerGenericPageType = function(typeInfo, callback) {
    var manager = self.apos.docs.getManager(typeInfo.name);
    if (manager) {
      return setImmediate(callback);
    }
    var typeName = typeInfo.name + '-auto-pages';
    ...
    return self.apos.create(typeName, {
      ...
      self.apos.docs.setManager(typeInfo.name, manager);
      return callback(null);
    });
  };

  self.registerTrashPageType = function(callback) {
    return self.registerGenericPageType({ name: 'trash' }, callback);
  };
