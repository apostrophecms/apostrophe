...
module.exports = function(self, options) {
    ...
    return async.series(
      [
        ...
        function getPages(cb) {
          self.apos.docs.db.update({ path: self.matchDescendants(page) }, action, (err, res) => {
            return cb(err);
          });
        },

        // update duplicate field names
        function doDupe(cb) {
          const path = self.matchDescendants(page);
          const type = page.type;
          self.apos.docs.db.update({ path: path }, type, action, (err, pages) => {
            console.log("pp", path, pages);
            if (err) {
              return cb(err);
            } else {
              return self.handleDeduplication(req, path, type, toTrash, cb);
            }
          });
        }
  ...
  self.handleDeduplication = function(req, path, type, toTrash, callback) {
    return async.forEach(pages, (page, cb) => {
      console.log('dedup', type);
      if (toTrash) {
        return self.apos.docs.getManager(type).deduplicateTrash(req, page, cb);
      } else {
        return self.apos.docs.getManager(type).deduplicateRescue(req, page, cb);
      }
    }, callback);
  };
