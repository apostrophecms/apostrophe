...
module.exports = function(self, options) {
  ...
  // Currently a wrapper for docs.getManager.
  ...
  // Removes properties that are not safe to roll back
  // because the page tree is a shared resource

  self.docVersionPrune = function(req, version) {
    // Don't let people roll back moves in the tree as
    // that could lead to collisions
    if (version.slug.match(/^\//)) {
      delete version.path;
