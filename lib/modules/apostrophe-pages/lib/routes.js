...
module.exports = function(self, options) {
  ...
  self.route('post', 'editor', function(req, res) {
    ...
    return res.send(self.render(req, 'editor', { schema: schema, verb: verb, trashInTree: self.options.trashInTree }));
  });
  ...
  self.route('post', 'fetch-to-update', function(req, res) {
    ...
    return self.find(req, { _id: _id }).permission('edit-apostrophe-page').trash(self.options.trashInTree ? null : false).toObject(function(err, page) {
      if (err) {
        console.error(err);
        return res.send({ status: 'error' });
      }
  ...
  self.route('post', 'update', function(req, res) {
    ...
    return async.series({
      find: function(callback) {
        return self.find(req, { _id: id }).permission('publish-apostrophe-page').trash(self.options.trashInTree ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
      ...
      findAgain: function(callback) {
        // Fetch the page. Yes, we already have it, but this way all the cursor
        // filters run and we have access to ._url
        return self.find(req, { _id: existingPage._id }).published(null).trash(self.options.trashInTree ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
  ...
  self.route('post', 'fetch-to-copy', function(req, res) {
    ...
    return self.find(req, { _id: _id }).permission('edit-apostrophe-page').toObject(function(err, page) {
      if (err) {
        console.error(err);
        return res.send({ status: 'error' });
      }
