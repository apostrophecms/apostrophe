...
module.exports = function(self, options) {
  ...
  self.route('post', 'fetch-to-insert', function(req, res) {
    ...
    var copy = self.apos.launder.boolean(req.body.copy);
    ...
    var cursor = self.find(req, { _id: _id });
    if (copy) {
      cursor.ancestors({ permission: false });
    } else {
      cursor.permission('publish-doc');
    }
    return cursor.toObject(function(err, relatedPage) {
      if (err) {
        console.error(err);
      ...
      if (copy) {
        var ancestors = relatedPage._ancestors;
        if (!ancestors.length) {
          return res.send({ status: 'notfound' });
        }
      ...
      if (copy) {
        child = self.newSibling(parentPage, relatedPage);
      } else {
        child = self.newChild(parentPage);
      }
      return res.send({ status: 'ok', data: child, parentPageId: parentPage._id, schema: manager.allowedSchema(req) });
    });
  });

  self.route('post', 'insert', function(req, res) {
    // bc
    var parentId = self.apos.launder.id(req.body.parentPageId || req.body.currentPageId);
  ...
  self.route('post', 'get-jqtree', function(req, res) {
    return self.find(req, { slug: '/' }).children({ depth: 1000, published: null, trash: null, orphan: null, reorganize: true, areas: false }).published(null).trash(null).reorganize(true).toObject(function(err, page) {
      if (err) {
        console.error(err);
        return res.send({ status: (typeof(err) === 'string') ? err : 'error' });
      }
