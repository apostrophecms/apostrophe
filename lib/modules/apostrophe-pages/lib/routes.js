...
module.exports = function(self, options) {
  ...
  self.route('post', 'editor', async function(req, res) {
    try {
      ...
      if (verb === 'update') {
        ...
        page = await self.find(req, { _id: id }).ancestors(true).permission('publish-doc').trash(trash).toObject();
        if (!page) {
          throw 'notfound';
        }
        ...
        parentPage = await self.find(req, { _id: id }).permission('publish-doc').toObject();
        if (!_parentPage) {
          throw 'notfound';
        }
