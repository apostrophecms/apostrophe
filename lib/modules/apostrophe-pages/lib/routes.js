...
module.exports = function(self, options) {
  ...
  self.renderRoute('post', 'editor', function(req, res, next) {
    ...
    if (verb === 'update') {
      ...
      return self.find(req, { _id: id }).ancestors(true).permission('publish-doc').trash(trash).toObject(function(err, page) {
        ...
        schema = self.allowedSchema(req, page, parentPage, verb);
      ...
      return self.find(req, { _id: id }).ancestors(true).toObject(function(err, peerPage) {
        ...
        delete child.parked;
        schema = self.allowedSchema(req, child, parentPage, verb);
        return succeed();
      });
    } else {
  ...
  self.apiRoute('post', 'fetch-to-insert', function(req, res, next) {
    ...
    return self.find(req, { _id: _id }).permission('edit-apostrophe-page').toObject(function(err, parentPage) {
      ...
      return next(null, { data: child, schema: self.allowedSchema(req, child, parentPage, 'insert') });
    });
  });
  ...
  self.apiRoute('post', 'insert', function(req, res, next) {
    ...
    return async.series({
      ...
      convert: function(callback) {
        ...
        var schema = self.allowedSchema(req, safePage, parentPage, 'insert');
  ...
  self.apiRoute('post', 'fetch-to-update', function(req, res, next) {
    ...
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-apostrophe-page').trash(self.apos.docs.trashInSchema ? null : false).toObject(function(err, page) {
      ...
      var schema = self.allowedSchema(req, page, parentPage, 'update');
      // We modified it, we have to bless the new version
      self.apos.schemas.bless(req, schema);
  ...
  self.apiRoute('post', 'update', function(req, res, next) {
    ...
    return async.series({
      ...
      convert: function(callback) {
        ...
        var schema = self.allowedSchema(req, existingPage, parentPage, 'update');
  ...
  self.apiRoute('post', 'fetch-to-copy', function(req, res, next) {
    ...
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-apostrophe-page').toObject(function(err, page) {
      ...
      // missing schema fields for the new copy
      delete page.parked;
      var schema = self.allowedSchema(req, page, parentPage, 'copy');
  ...
  self.apiRoute('post', 'copy', function(req, res, next) {
    ...
    return async.series({
      ...
      convert: function(callback) {
        ...
        var schema = self.allowedSchema(req, safePage, parentPage, 'copy');
        return self.apos.schemas.convert(req, schema, 'form', page, safePage, callback);
      },
