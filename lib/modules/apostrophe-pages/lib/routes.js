...
module.exports = function(self, options) {
  ...
  self.apiRoute('post', 'editor', async function(req) {
    ...
    if (verb === 'update') {
      ...
      page = await self.find(req, { _id: _id }).ancestors(true).permission('edit-doc').trash(trash).toObject();
      if (!page) {
        throw 'notfound';
      }
      const parentPage = page._ancestors && page._ancestors.length && page._ancestors[page._ancestors.length - 1];
      if (!parentPage) {
        // convert 0 to null
        parentPage = null;
      }
      ...
      const peerPage = await self.find(req, { _id: id }).ancestors(true).toObject();
      if (!peerPage) {
        throw 'notfound';
      }
      ...
      parentPage = await self.find(req, { _id: id }).permission('edit-doc').toObject();
      if (!_parentPage) {
        throw 'notfound';
      }
  ...
  self.apiRoute('post', 'insert', async function(req) {
    ...
    return await self.find(req, { _id: safePage._id }).published(null).toObject();
  });
  ...
  self.apiRoute('post', 'update', async function(req) {
    ...
    const existingPage = await self.find(req, { _id: id }).ancestors(true).permission('edit-apostrophe-page').trash(self.apos.docs.trashInSchema ? null : false).toObject();
    if (!existingPage) {
      throw 'notfound';
    }
    ...
    return await self.find(req, { _id: existingPage._id }).published(null).trash(self.apos.docs.trashInSchema ? null : false).toObject();
  });
  ...
  self.apiRoute('post', 'copy', async function(req) {
    ...
    const currentPage = await self.find(req, { _id: currentPageId }).permission('edit-apostrophe-page').ancestors({ permission: false }).toObject();
    if (!currentPage) {
      throw 'notfound';
    }
  ...
  self.apiRoute('post', 'move-to-trash', async function(req) {
    await self.moveToTrash(req, self.apos.launder.id(req.body._id));
    return {
      changed: self.mapMongoIdToJqtreeId(changed), 
      parentSlug
    };
  ...
  self.apiRoute('post', 'delete-from-trash', async function(req) {
    ...
    return await self.deleteFromTrash(req, self.apos.launder.id(req.body._id));
  });

  self.route('post', 'get-jqtree', function(req) {
  ...
  self.apiRoute('post', 'info', async function(req) {
    return await self.find(req, { _id: self.apos.launder.id(req.body._id) }).published(null).areas(false).trash(null).joins(false).toObject();
  });
  ...
  self.apiRoute('post', 'publish', async function(req) {
    return await self.batchSimpleRoute(req, 'publish', async function(req, page, data) {
      page.published = true;
  ...
  self.route('post', 'unpublish', async function(req) {
    return await self.batchSimpleRoute(req, 'unpublish', function(req, page, data) {
  ...
  self.route('post', 'tag', async function(req) {
    return await self.batchSimpleRoute(req, 'tag', function(req, page, data) {
      if (!page.tags) {
        page.tags = [];
      }
  ...
  self.route('post', 'untag', async function(req) {
    return await self.batchSimpleRoute(req, 'untag', function(req, page, data) {
      if (!page.tags) {
        return;
      }
  ...
  self.route('post', 'trash', async function(req) {
    return await self.batchSimpleRoute(req, 'trash', async function(req, page, data) {
      if (page.trash) {
        return;
      }
      if (self.apos.docs.trashInSchema) {
  ...
  self.route('post', 'rescue', function(req, res) {
    return await self.batchSimpleRoute(req, 'rescue', function(req, page, data) {
      if (!page.trash) {
        return;
      }
      if (self.apos.docs.trashInSchema) {
