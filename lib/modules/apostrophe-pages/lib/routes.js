...
module.exports = function(self, options) {
  ...
  self.apiRoute('post', 'editor', async function(req) {
    ...
    if (verb === 'update') {
      ...
      page = await self.find(req, { _id: id }).ancestors(true).permission('edit-doc').trash(trash).toObject();
      if (!page) {
        throw 'notfound';
      }
  ...
  self.route('post', 'fetch-to-insert', async function(req, res) {
    try {
      // parent id
  ...
  self.route('post', 'insert', function(req, res) {
    let parentId = self.apos.launder.id(req.body.currentPageId);
    ...
    return async.series({
      findParent: function(callback) {
        return self.find(req, { _id: parentId }).permission('edit-apostrophe-page').toObject(function(err, _parentPage) {
          if (err) {
            return callback(err);
          }
          if (!_parentPage) {
      ...
      convert: function(callback) {
        let manager = self.apos.docs.getManager(self.apos.launder.string(page.type));
        if (!manager) {
          // sneaky
          return callback('notfound');
        }
  ...
  self.route('post', 'update', function(req, res) {
    let id = self.apos.launder.id(req.body.currentPageId);
    let page = req.body.page || {};
    if (typeof (page) !== 'object') {
      // cheeky
      return res.send({ status: 'notfound' });
    ...
    return async.series({
      find: function(callback) {
        return self.find(req, { _id: id }).ancestors(true).permission('edit-apostrophe-page').trash(self.apos.docs.trashInSchema ? null : false).toObject(function(err, _page) {
          if (err) {
            return callback(err);
          }
          if (!_page) {
      ...
      convert: function(callback) {
        let parentPage = existingPage._ancestors && existingPage._ancestors.length && existingPage._ancestors[existingPage._ancestors.length - 1];
        let manager = self.apos.docs.getManager(self.apos.launder.string(page.type || existingPage.type));
        if (!manager) {
          // sneaky
          return callback('notfound');
        }
  ...
  self.route('post', 'fetch-to-copy', function(req, res) {
    ...
    return self.find(req, { _id: _id }).ancestors(true).permission('edit-apostrophe-page').toObject(function(err, page) {
      ...
      let parentPage = page._ancestors && page._ancestors[0] && page._ancestors[page._ancestors.length - 1];
      let schema = self.allowedSchema(req, page, parentPage);
      // We modified it, we have to bless the new version
      self.apos.schemas.bless(req, schema);
      return res.send({ status: 'ok', data: page, schema: schema });
  ...
  self.route('post', 'copy', function(req, res) {
    let currentPageId = self.apos.launder.id(req.body.currentPageId);
    let page = req.body.page || {};
    if (typeof (page) !== 'object') {
      // cheeky
      return res.send({ status: 'notfound' });
    }
    let siblingPage, parentPage;
    let safePage;
    return async.series({
      findParentAndSibling: function(callback) {
        return self.find(req, { _id: currentPageId }).permission('edit-apostrophe-page').ancestors({ permission: false }).toObject(function(err, currentPage) {
          if (err) {
            return callback(err);
          }
          if (!currentPage) {
  ...
  self.route('post', 'reorganize', function(req, res) {
    return self.send(req, 'reorganize.html', {
  ...
  self.route('post', 'chooser-modal', function(req, res) {
    return self.send(req, chooserModal.html', {});
  });
