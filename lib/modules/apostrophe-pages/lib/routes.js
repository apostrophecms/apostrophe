...
module.exports = function(self, options) {
  ...
  self.route('post', 'editor', function(req, res) {
    ...
    if (verb === 'update') {
      ...
      return self.find(req, { _id: id }).permission('publish-doc').trash(trash).toObject(function(err, page) {
        if (err) {
          return fail(err);
        }
        if (!page) {
        ...
        schema = self.removeSlugFromHomepageSchema(page, schema);
  ...
  self.route('post', 'fetch-to-update', function(req, res) {
    ...
    return self.find(req, { _id: _id }).permission('edit-apostrophe-page').trash(self.apos.docs.trashInSchema ? null : false).toObject(function(err, page) {
      if (err) {
        console.error(err);
        return res.send({ status: 'error' });
      }
  ...
  self.route('post', 'update', function(req, res) {
    ...
    return async.series({
      ...
      convert: function(callback) {
        ...
        var schema = manager.allowedSchema(req);
        schema = self.addApplyToSubpagesToSchema(schema);
        schema = self.removeSlugFromHomepageSchema(existingPage, schema);
