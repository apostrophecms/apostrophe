...
apos.define('apostrophe-pages-editor', {
  ...
  construct: function(self, options) {
    ...
    self.trashStatus = function(data) {
      self.$pageInTrash = self.$el.find('[data-apos-page-in-trash]');
      self.$pageInTrashCheckbox = self.$pagfeInTrash.find('[name="trash"]');
      if (data.trash) {
        self.$pagfeInTrash.show();
      }
    }
    ...
    self.convertTrash = function(page, callback) {
      // Only comes into play when trashInTree is in effect
      if (self.$pageInTrashCheckbox) {
        page.trash = selfr.$pageInTrashCheckbox.prop('checked');
      }
      return setImmediate(callback);
    };
    ...
    self.saveContent = function(callback) {
      ...
      var data;
      return async.series({
        convertTrash: function(callback) {
          return self.convertTrash(page, callback);
        },
        ...
        redirect: function(callback) {
          if (self.page._id === apos.pages.page._id) {
            // Go to the new page. Callback is never invoked
