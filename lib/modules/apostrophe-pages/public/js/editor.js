...
apos.define('apostrophe-pages-editor', {
  ...
  construct: function(self, options) {
    ...
    self.addTypeChangeHandler = function() {
      let $type = apos.schemas.findField(self.$el, 'type');
    ...
    self.changedType = function(type) {
      ...
      let page = _.cloneDeep(self.page);
      return async.series({
        convert: function(callback) {
        ...
        editor: function(callback) {
          return self.html('editor', { id: self.page._id, type: type, verb: self.verb }, function(html) {
            let $html = $(html);
            let $newFields = $html.find('[data-page-schema-fields]').children();
            let $wrapper = self.$el.find('[data-page-schema-fields]');
            $wrapper.html('');
