  ...
  construct: function(self, options) {
    ...
    self.enhanceNode = function(node, $li) {
      ...
      if (!node.virtualTrashcan) {
        ...
        let $checkbox = $('<input type="checkbox" name="selected" value="' + node.id + '" />');
    ...
    self.configureTitleContainer = function(node, $li) {
      let $titleContainer = $('<span class="apos-reorganize-title"></span>');
    ...
    self.addVisitLink = function(node, $li) {
      ...
      let $link = $('<a class="apos-visit" target="_blank"></a>');
      $link.attr('data-node-id', node.id);
    ...
    self.afterHide = function() {
      ...
      self.api('info', { _id: _id }, function(data) {
        ...
        let newPathname = data.page.slug.replace(/^\/\//, '/');
    ...
    self.visit = function($node) {
      ...
      let node = self.$tree.tree('getNodeById', nodeId);
      let tab = window.open(apos.prefix + node.slug, '_blank');
      tab.focus();
    };
    ...
    self.edit = function($node) {
      ...
      apos.create('apostrophe-pages-editor-update', {
        ...
        afterSave: function(page, callback) {
          ...
          let node = self.$tree.tree('getNodeById', page._id);
          if (node) {
            let newNode = _.pick(node, 'children', 'id');
            ...
            if (isTrash && (!newNode.trash)) {
              ...
              let virtualTrashcan = _.find(parent.children, { virtualTrashcan: true });
      ...
      apos.pages.trash(node.id, function(err, parentSlug, changed) {
        ...
        if (apos.docs.trashInSchema) {
          ...
          _.each(changed, function(info) {
            let node = self.$tree.tree('getNodeById', info.id);
            if (node) {
              node.slug = info.slug;
            }
          });
    ...
    self.deleteFromTrash = function($node) {
      ...
      let warning = 'Are you sure you want to DESTROY THIS PAGE FOREVER? This CANNOT BE UNDONE.';
    ...
    self.move = function(e) {
      ...
      let target = e.move_info.target_node;
      ...
      if (apos.docs.trashInSchema) {
        if (e.move_info.target_node.virtualTrashcan) {
          if ((e.move_info.position === 'before') || (e.move_info.position === 'after')) {
            let after = e.move_info.target_node.getPreviousSibling();
            if (after) {
              data.position = 'after';
      ...
      function trash(callback) {
        ...
        let node = e.move_info.moved_node;
        if (trashing) {
    ...
    self.reload = function(callback) {
      self.api('get-jqtree', {}, function(data) {
        if (data.status === 'ok') {
          if (apos.docs.trashInSchema) {
            ...
            let trashIndex = _.findIndex(data.tree[0].children, { slug: '/trash' });
            if (trashIndex !== -1) {
              let trash = data.tree[0].children[trashIndex];
      ...
      function addVirtualTrashcan(node) {
        let trashed = _.filter(node.children || [], { trash: true });
        let regular = _.filter(node.children || [], function(node) {
          return !node.trash;
        });
    ...
    self.updateVirtualTrashcan = function(node) {
      ...
      let trashcan = _.find(node.children || [], { virtualTrashcan: true });
      let trashed = _.filter(node.children || [], { trash: true });
      let regular = _.filter(node.children || [], function(node) {
    ...
    self.reflectSelectAll = function() {
      let $selectAll = self.$el.find('[name="select-all"]');
    ...
    self.reflectChoiceInCheckbox = function(id) {
      let state = _.includes(self.getIds(), id);
    ...
    self.displayChoiceInCheckbox = function(id, checked) {
      let $checkbox = self.getCheckbox(id);
    ...
    self.reflectBatchOperation = function() {
      ...
      let val = self.$batchOperation.val();

      // TODO bring back code from manager-modal that determines which

      let $selected = self.$batchOperation.find('[value="' + val + '"]');
      if ($selected.length) {
      ...
      let $buttons = self.$batch.find('.apos-button');
      if (count) {
    ...
    self.enableBatchOperation = function(batchOperation, callback) {
      ...
      let data = apos.schemas.newInstance(batchOperation.schema);
    ...
    self.batchSimple = function(operationName, confirmationPrompt, options) {
      ...
      let operation = self.batchOperations[operationName];
      ...
      function save(callback) {
        ...
        return self.api(operation.route || operationName, data, function(result) {
          ...
          if (result.jobId) {
            let jobs = apos.modules['apostrophe-jobs'];
