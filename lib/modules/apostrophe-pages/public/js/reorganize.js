...
apos.define('apostrophe-pages-reorganize', {
  ...
  construct: function(self, options) {
    ...
    self.enhanceNode = function(node, $li) {
      ...
      if (node.getLevel() !== 1) {
        $li.find('.apos-reorganize-title').css('padding-left', 10 * node.getLevel() + 'px');
      }
      ...
      if (!node.virtualTrashcan) {
        ...
        $li.find('.jqtree-element .apos-reorganize-controls--published').append('<span class="apos-reogranize-published apos-reogranize-published--' + node.publish + '"></span>');
