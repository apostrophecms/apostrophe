...
apos.define('apostrophe-pages-reorganize', {
  ...
  construct: function(self, options) {
    self.beforeShow = function(callback) {
	    ...
	    self.$tree.tree({
	      ...
	      onCreateLi: function(node, $li) {
            ...
            if (self.options.emptyTrash) {
              // "Destroy forever" button. CSS reveals when
              // already in the trash
              link = $('<a class="apos-empty-trash"></a>');
  	          link.attr('data-node-id', node.id);
  	          link.attr('data-empty-trash', '1');
  	          link.attr('href', '#');
      ...
      self.$tree.on('click', '[data-empty-trash]', function() {
	      self.emptyTrash($(this));
	      return false;
			});
    ...
    self.emptyTrash = function($node) {
      ...
      apos.pages.emptyTrash(node.id, function(err, parentSlug) {
        if (err) {
          alert('An error occurred.');
          return;
        }
