...
// whenever the site starts up, or when the `apostrophe-pages:park` command line
// task is run.** The parked pages you get are actually the
// concatenation of the `minimumPark` and `park` options. The automatic parking
// at every startup can be disabled by setting the `autoPark` option to `false`.
//
...
// ```
//
// **`autoPark`: by default, Apostrophe automatically parks parked pages at startup.
// If the `autoPark` option is explicitly set to `false`, this does not happen.
// Instead, you must invoke the `apostrophe-pages:park` command line task. Most of the time
// this is not necessary but it can be useful to avoid unexpected startup delays on
// very large sites using workflow.
//
