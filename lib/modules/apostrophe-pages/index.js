...
module.exports = {
    ...
    return {
      'beforeSend': {
        async attachHomeBeforeSend(req) {
          ...
          //
          // home: { children: false }
          let filters = self.getServePageFilters().ancestors || { children: !(self.options.home && self.options.home.children === false) };
  ...
  methods(self, options) {
    return {
        ...
        async function body() {
          ...
          async function getMoved() {
            const moved = await self.find(req, mergeCriteria({ _id: movedId })).permission(false).trash(null).published(null).areas(false).ancestors(_.assign({
              ...
              areas: false,
              permission: false
            }, options.filters || {})).applyFilters(options.filters || {}).toObject();
            if (!moved) {
      ...
      matchPageAndPrefixes(cursor, slug) {
        let slugs = [];
        let components;
      ...
      async updateDescendantsAfterMove(req, page, originalPath, originalSlug, options) {
        ...
        let done = false;
        let cursor = self.apos.docs.db.find(mergeCriteria({ path: matchParentPathPrefix })).project({
          slug: 1,
          path: 1,
          level: 1
        });
