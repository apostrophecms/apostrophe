...
module.exports = {
  ...
  methods(self, options) {
    return {
      ...
      //
      // Returns the page.
      async insert(req, parentOrId, page, options) {
        if (!options) {
          options = {};
        }
        ...
        async function body() {
          ...
          }).project({ rank: 1 }).sort({ rank: -1 }).limit(1).toArray();
          if (!results.length) {
            page.rank = 0;
          } else {
  ...
  helpers(self, options) {
    return {
      ...
      afterContextMenu: function () {
        return self.apos.templates.safe(_.map(self.afterContextMenuHelpers || [], function (helper) {
