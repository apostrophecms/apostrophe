...
module.exports = {
  ...
  async init(self, options) { 
    ...
    await self.registerTrashPageType();
  ...
  restApiRoutes(self, options) {
    return {
        ...
        function clean(nodes) {
          ...
          function prune(nodes) {
            ...
            _.each(nodes, function(node) {
              ...
              if (node.good) {
                newNodes.push(_.pick(node, 'title', 'slug', '_id', 'type', 'metaType', 'tags', '_url', '_children'));
              }
            });
            return newNodes;
          }
      ...
      // `_targetId` is the _id of another page, and `_position` may be `before`, `after` or
      // `inside`. If you do not specify these properties they default to the homepage and `inside`,
      ...
      async post(req) {
        ...
        const position = self.apos.launder.string(req.body._position) || 'inside';
        const copyingId = self.apos.launder.id(req.body._copyingId);
        const input = _.omit(req.body, 'parentId');
        if (typeof (page) !== 'object') {
          // cheeky
          throw 'invalid';
        }
        return self.withLock(req, async () => {
          const targetPage = await self.findForEditing(req, targetid ? { _id: targetId } : { level: 0 }).permission('edit-apostrophe-page').toObject();
          if (!targetPage) {
            throw 'notfound';
          }
          ...
          let page = self.newChild(parentPage);
          await manager.convert(req, input, page, { onlyPresentFields: true, copyingId });
          await self.emit('afterConvert', req, input, page);
          // Already locked
          await self.insert(req, targetPage, position, page, { lock: false });
          return self.findOneForEditing(req, { _id: piece._id }, { annotate: true });
        });
      },
      // Update the page via `PUT`. The entire page should be at req.body. Consider
      // using `PATCH` if you are concerned about pushing stale data for properties
      // you are not actually trying to change.
      //
      // To move a page in the tree you should use `PATCH`.
      //
      // This call is atomic with respect to other REST write operations on pages.
      async put(req, _id) {
      ...
      //
      // You must pass `_targetId` and `_position` so the page is added to the tree properly. `_position`
    ...
    return {
      post: {
        ...
        async editor(req) {
          ...
          return self.render(req, 'editor', {
            schema: schema,
            verb: verb,
            trashInSchema: self.apos.docs.trashInSchema
          });
        },
        async fetchToInsert(req) {
          // parent id
        ...
        async update(req) {
          ...
          const existingPage = await self.find(req, { _id }).ancestors(true).permission('edit-apostrophe-page').trash(self.apos.docs.trashInSchema ? null : false).toObject();
          if (!existingPage) {
            throw 'notfound';
          }
          ...
          // Fetch the page. Yes, we already have it, but this way all the query
          // builders run and we have access to ._url
          return self.find(req, { _id: existingPage._id }).published(null).trash(self.apos.docs.trashInSchema ? null : false).toObject();
        },
        async fetchToCopy(req) {
          const _id = self.apos.launder.id(req.body._id);
          const type = self.apos.launder.string(req.body.type);
          const manager = self.apos.docs.getManager(type);
          if (!manager) {
        ...
        async copy(req) {
          ...
          const currentPage = await self.find(req, { _id }).permission('edit-apostrophe-page').ancestors({ permission: false }).toObject();
          if (!currentPage) {
            throw 'notfound';
          }
        ...
        async move(req) {
          ...
          return { changed: self.mapMongoIdToJqtreeId(changed) };
        },
        ...
        async getJqtree(req) {
          const page = await self.find(req, { level: 0 }).children({
            depth: 1000,
            published: null,
            trash: null,
            orphan: null,
            ...
            areas: false,
            permission: false
          }).published(null).trash(null).reorganize(true).toObject();
          if (!page) {
            throw 'notfound';
          }
  ...
  handlers(self, options) {
    return {
      ...
      'apostrophe:modulesReady': {
        async registerGenericPageTypes() {
        ...
        async manageOrphans() {
          ...
          async function patchUndefined() {
            return self.apos.docs.db.updateMany({ type: { $exists: 0 } }, { $set: { type: 'WASUNDEFINED' } });
          }
          async function find() {
  ...
  methods(self, options) {
    return {
      find(req, criteria, projection) {
        return self.apos.modules['apostrophe-any-page-manager'].find(req, criteria, projection);
      },
      getBrowserData(req) {
        if (!req.user) {
          return false;
      ...
      // Insert a page. `target` must be another page, and
      ...
      async insert(req, target, position, page, options = { permissions: true }) {
        return self.withLock(req, async () => {
          let parent;
          if (position !== 'inside') {
          ...
          const peers = await self.findForEditing(req, {
            path: self.matchDescendants(parent),
            level: parent.level + 1
          }).project({ rank: 1 }).sort({ rank: 1 });
          let pushed = [];
          if (position === 'inside') {
            if (!peers.length) {
              page.rank = 0;
          ...
          page.path = self.apos.utils.addSlashIfNeeded(parent.path) + pathFrom;
      ...
      // If `options.builders` is present, those builders are invoked
      ...
      async move(req, movedId, targetId, position, options) {
        if (!options) {
          options = {};
        } else {
        ...
        async function body() {
          ...
          async function getMoved() {
            const moved = await self.find(req, mergeCriteria({ _id: movedId })).permission(false).trash(null).published(null).areas(false).ancestors(_.assign({
              depth: 1,
              trash: null,
              published: null,
              areas: false,
              permission: false
            }, options.builders || {})).applyBuilders(options.builders || {}).toObject();
            if (!moved) {
          ...
          async function getTarget() {
            const target = await self.find(req, mergeCriteria({ _id: targetId })).permission(false).trash(null).published(null).areas(false).ancestors(_.assign({
              depth: 1,
              trash: null,
              published: null,
              areas: false,
          ...
          async function nudgeNewPeers() {
            ...
            await self.apos.docs.db.updateMany(mergeCriteria({
              path: self.matchDescendants(parent),
              level: parent.level + 1,
              rank: { $gte: rank }
            }), { $inc: { rank: 1 } });
      ...
      async updateDescendantsAfterMove(req, page, originalPath, originalSlug, options) {
        ...
        let done = false;
        let query = self.apos.docs.db.find(mergeCriteria({ path: matchParentPathPrefix })).project({
          slug: 1,
          path: 1,
          level: 1
        });
        ...
        async function updateDescendant(desc) {
          await self.apos.docs.db.updateOne(mergeCriteria({ _id: desc._id }), { $set: _.pick(desc, 'path', 'slug', 'level') });
        }
      },
      ...
      async implementParkOne(req, item) {
        ...
        async function findParent() {
          ...
          return self.find(req, { slug: parentSlug }).joins(false).areas(false).published(null).permission(false).trash(null).toObject();
        }
        async function findExisting() {
          ...
          return self.find(req, criteria).joins(false).areas(false).published(null).permission(false).trash(null).toObject();
        }
        async function backupFindExisting() {
          ...
          let slugWithOrWithoutSlash = new RegExp('^' + self.apos.utils.regExpQuote(item.slug.replace(/\/$/, '')) + '/?$');
          criteria.slug = slugWithOrWithoutSlash;
          const existing = await self.find(req, criteria).joins(false).areas(false).published(null).permission(false).trash(null).toObject();
      ...
      matchDescendants(page) {
        ...
        return new RegExp('^' + self.apos.utils.regExpQuote(self.apos.utils.addSlashIfNeeded(page.path)) + '.');
      },
      ...
      // `manageOrphans`
      async registerGenericPageType(type) {
        ...
        manager = await self.apos.create(typeName, { apos: self.apos });
        self.apos.docs.setManager(type, manager);
      },
      async registerTrashPageType() {
      ...
      async batchSimpleRoute(req, name, change) {
        ...
        async function runJob() {
          return self.apos.modules['apostrophe-jobs'].run(req, ids, one, { labels: { title: batchOperation.progressLabel || batchOperation.buttonLabel || batchOperation.label } });
        }
