...
// at the very end of the process, in an `apostrophe:afterInit` handler,
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.on('apostrophe:modulesReady', 'registerGenericPageTypes');
    self.on('apostrophe:modulesReady', 'manageOrphans');

    // Wait until the last possible moment to add
    // the wildcard route for serving pages, so that
    // other routes are not blocked

    self.on('apostrophe:afterInit', 'addServeRoute', function() {
      self.apos.app.get('*', self.serve);
    });
    self.on('apostrophe:afterInit', 'implementParkAll');
