    ...
    return {
      ...
      async post(req) {
        ...
        return self.withLock(req, async () => {
          const targetPage = await self.findForEditing(req, targetId ? { _id: targetId } : { level: 0 }).ancestors(true).permission('edit-apostrophe-page').toObject();
          if (!targetPage) {
            throw self.apos.error('notfound');
          }
        ...
        return self.withLock(req, async () => {
          ...
          if (position === 'firstChild') {
            ...
            if (!peers.length) {
              ...
              pushed = peers.map(peer => peer._id);
            }
