...
module.exports = function(self, options) {
  ...
  self.routes.arrayEditor = function(req, res) {
    return self.send(req, 'arrayEditor', req.body);
  };
  ...
  self.routes.arrayItems = function(req, res) {
    ...
    return async.series([ convert, join ], function(err) {
      ...
      return self.send(req, 'arrayItems', { active: active, arrayItems: receptacle[name], field: req.body.field });
    });
  ...
  self.routes.arrayItem = function(req, res) {
    return self.send(req, 'arrayItem', req.body);
  };
};
