...
module.exports = function(self, options) {
  ...
  self.routes.choices = function(req, res, next) {
    var choices = self.apos.launder.string(req.body.choices);
    if (!self.apos.utils.isBlessed(req, choices, 'choices')) {
      return next(new Error('Unblessed choices request: ' + choices));
    }
    choices = choices.split(':');
    return self.apos.modules[choices[0]][choices[1]](req).then(function(choices) {
      return {
        status: 'ok',
        choices: choices
      };
