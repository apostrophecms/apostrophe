apos.define('apostrophe-array-editor-modal', {
  ...
  construct: function(self, options) {
    ...
    self.addToItems = function() {
      self.arrayItems.push({ _title: "New Item" });
      self.active = self.arrayItems.length - 1;
      self.refreshItems();
    };

    self.setTitles = function(){
      self.arrayItems = _.map(self.arrayItems, function(item){
        item._title = item.clubType || item.title; // TODO make this a configurable field title
        return item;
      });
    }
