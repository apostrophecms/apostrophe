...
apos.define('apostrophe-schemas', {
  ...
  construct: function(self, options) {
    ...
    self.populate = function($el, schema, object, callback) {
      ...
      function populate(callback) {
        return async.eachSeries(schema, function(field, callback) {
          ...
          let $field = self.findField($el, field.name);
          if (field.contextual) {
    ...
    self.enableSlugSuggestions = function($el, schema, object) {
      let title = _.find(schema, { name: 'title' });
      let slug = _.find(schema, { name: 'slug', type: 'slug' });

      if (!(title && slug)) {
        return;
      }

      let $title = self.findField($el, 'title');
      let $slug = self.findField($el, 'slug');
    ...
    self.convert = function($el, schema, data, options, callback) {
      ...
      self.findSafe($el, '[data-name]').each(function() {
        let $fieldset = $(this);
        let classes = $fieldset[0].className.split(/\s+/);
      ...
      return async.eachSeries(schema, function(field, callback) {
        ...
        let $fieldset = self.findFieldset($el, field.name);
    ...
    self.returnToError = function($el, schema, errorPath, error, callback) {
      let first = errorPath.shift();
      let $field = self.findField($el, first);
      let field = _.find(schema, { name: first });
      if (!field) {
        return setImmediate(callback);
      }
      let fieldType = self.fieldTypes[field.type];
    ...
    self.contextualConvertArea = function(data, name, $el, field) {
      let editor = $el.findSafe('[data-apos-area][data-dot-path$="' + name + '"]', '[data-apos-area]').data('editor');
      if (editor) {
        data[name] = {
          type: 'area',
    ...
    self.contextualIsPresentArea = function($el, field) {
      let editor = $el.findSafe('[data-apos-area][data-dot-path$="' + field.name + '"]', '[data-apos-area]').data('editor');
      return !!editor;
    };
    ...
    self.enableArea = function($el, name, area, options, callback) {
      ...
      let $fieldset = self.findFieldset($el, name);
      $.jsonCall(apos.areas.options.action + '/edit-virtual-area',
        { dataType: 'html' },
        { items: items, options: options }, function(data) {
          let $editView = self.findSafe($fieldset, '[data-' + name + '-edit-view]');
    ...
    self.getArea = function($el, name) {
      let $fieldset = self.findFieldset($el, name);
      let $property = self.findSafe($fieldset, '[data-' + name + '-edit-view]');
    ...
    self.showError = function($el, error) {
      let $fieldset = self.findFieldset($el, error.field.name);
    ...
    self.scrollToError = function($el) {
      let $element = self.findSafe($el, '.apos-error');
      if ($element.length) {
    ...
    self.showGroupContaining = function($element) {
      let $group = $element.closest('.apos-schema-group');
      if ($group.length) {
        if (!$group.hasClass('apos-active')) {
          let group = $group.attr('data-apos-group');
          let $container = $group.closest('[data-apos-form]');
    ...
    self.findSafe = function($el, sel) {
      ...
      let fields = el.querySelectorAll(sel);
      fields = _.filter(fields, function(field) {
        ...
        while (parent) {
          ...
          if (parent.classList && parent.classList.contains('apos-field')) {
            ...
            let matches = parent.querySelectorAll(sel);
    ...
    self.enhanceSelectiveWithSlugs = function($field) {
      ...
      // allow styling. -Tom
      let $autocomplete = self.findSafe($field, "[data-autocomplete]");
      $autocomplete.data("ui-autocomplete")._renderItem = function(ul, item) {
        let inner = '<a><div class="apos-autocomplete-label">' + item.label + '</div>';
    ...
    self.enableSlug = function($title, $slug, title, slug) {
      ...
      let components = currentSlug.split('/');
      let currentSlugTitle = components.pop();
      let prefix = '';
      if ($slug.data('prefix')) {
      ...
      function afterChange() {
        ...
        _.each(field.choices || [], function(choice) {
          ...
          _.each(choice.showFields || [], function(fieldName) {
            ...
            let $fieldset = self.findFieldset($el, fieldName);
    ...
    self.addFieldType({
      name: 'array',
      populate: function(data, name, $field, $el, field, callback) {
        let $button = self.findSafeInFieldset($el, name, '[data-apos-edit-array]');
    ...
    self.addFieldType({
      ...
      populate: function(data, name, $field, $el, field, callback) {
        let manager;
        let $fieldset = self.findFieldset($el, name);
        let chooser = $fieldset.data('aposChooser');
        let chooserGetter;
        if (!chooser) {
    ...
    self.addFieldType({
        ...
        return async.series({
          ...
          set: function(callback) {
            ...
            if (field.relationship) {
              let relationships = data[field.relationshipsField] || {};
              _.each(choices, function(choice) {
                let relationship = relationships[choice.value] || {};
      ...
      convert: function(data, name, $field, $el, field, callback) {
        ...
        return chooser.getFinal(function(err, choices) {
          ...
          let removed = _.filter(choices, { __removed: true });
    ...
    self.addFieldType({
      ...
      convert: function(data, name, $field, $el, field, callback) {
        ...
        for (let c in field.choices) {
          let val = field.choices[c].value;
          let checked = $field.filter('[value="' + val + '"]').prop('checked');
          if (checked) {
            values.push(val);
          }
        }
    ...
    self.addFieldType({
      ...
      populate: function(data, name, $field, $el, field, callback) {
        ...
        let value = ((data[name] === undefined) && field.choices[0]) ? field.choices[0].value : data[name];
        $field.val(value);
        self.enableShowFields(data, name, $field, $el, field);
        return setImmediate(callback);
      },
