  ...
  construct: function(self, options) {
    ...
    self.populate = function($el, schema, object, callback) {
      return async.eachSeries(schema, function(field, callback) {
        ...
        return displayer(object, field.name, $field, $el, field, function() {
    ...
    self.converters = {
      ...
      joinByOne: function(data, name, $field, $el, field, callback) {
        ...
        $field = self.findSafe($el, '[data-name="' + name + '"] [data-selective]');
      ...
      radioTable: function(data, name, $field, $el, field, callback) {
        var newData = {};
    ...
    self.displayers = {
      ...
      joinByOne: function(data, name, $field, $el, field, callback) {
        ...
        if (!$field.length) {
          apos.log('Error: your new.html template for the ' + self.name + ' module does not have a objectSelective call for the ' + name + ' join yet');
        }
        var selectiveData = [];
        ...
        var autocomplete = '/modules/apostrophe-docs/autocomplete?type=' + field.withType;
        $field.find('[data-selective]').selective({ limit: 1, data: selectiveData, source: autocomplete, nestGuard: '[data-selective]' });
      ...
      joinByArray: function(data, name, $field, $el, field, callback) {
        ...
        $field.find('[data-selective]').selective({ preventDuplicates: true, sortable: field.sortable, extras: !!field.relationship, data: data[field.idsField] || [], source: autocomplete, nestGuard: '[data-selective]' });
        // self.enhanceSelectiveWithSlugs($field);
        return setImmediate(callback);
      },
      joinByArrayReverse: function(data, name, $field, $el, field, callback) {
