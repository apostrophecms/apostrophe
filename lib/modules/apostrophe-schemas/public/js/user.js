    ...
    self.addFieldType({
      ...
      populate: function(data, name, $field, $el, field, callback) {
        ...
        if (!chooser) {
          ...
          chooser = manager.getChooser({ field: field, $el: $fieldset.find('[data-chooser]') });
        }
        var choices = [];
      ...
      populate: function(data, name, $field, $el, field, callback) {
        ...
        return async.series({
          getChooser: function(callback) {
            ...
            return manager.getChooser({ field: field, $el: $fieldset.find('[data-chooser]') }, function(err, _chooser) {
              if (err) {
                return callback(err);
              }
              chooser = _chooser;
