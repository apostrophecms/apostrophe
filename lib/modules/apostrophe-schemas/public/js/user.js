apos.define('apostrophe-schemas', {
  ...
  construct: function(self, options) {
    ...
    self.addFieldType({
      name: 'array',
      populate: function(data, name, $field, $el, field, callback) {
        var $button = self.findSafe($el, '[name="'+  field.name + '"] [data-apos-edit-array]');
        var array = data[name];
        function saveArray(val){
          array = val;
          $field.data('apos-array', val);
          setLengthLabel(array);
        }

        function setLengthLabel(array){
          var length = array ? array.length : 0;
          $field.find('[data-array-length]').html(field.label  + ' ('+ length + ' Item' + ((length > 1) ? 's)' : ')'));
        }
        ...
        $button.on('click', function() {
          ...
          apos.create('apostrophe-array-editor-modal', {field: field, arrayItems: array, action: options.action, save: saveArray}, function(err){
            if (err){
              console.error(err);
            }
          });
