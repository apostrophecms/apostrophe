      ...
      populate: function(data, name, $field, $el, field, callback) {
        ...
        if (!chooser) {
          if (Array.isArray(field.withType)) {
            chooserGetter = apos.modules['apostrophe-polymorphic-choosers'].get;
          } else {
        ...
        return async.series({
          getChooser: function(callback) {
            ...
            if (!chooser) {
              if (Array.isArray(field.withType)) {
                chooserGetter = apos.modules['apostrophe-polymorphic-choosers'].get;
                console.log('poly based');
              } else {
                manager = apos.docs.getManager(field.withType);
                chooserGetter = _.partial(manager.getTool, 'chooser');
                console.log('manager based');
              }
              console.log('invoking chooserGetter');
              console.log(chooserGetter);
              return chooserGetter({ field: field, $el: $fieldset.find('[data-chooser]') }, function(err, _chooser) {
                console.log('in chooserGetter callback');
                if (err) {
                  console.log('error', err);
                  return callback(err);
                }
                chooser = _chooser;
                console.log('chooser object is');
                console.log(chooser);
                var choices = [];
                if (data[field.idField]) {
                  choices.push({ value: data[field.idField] });
