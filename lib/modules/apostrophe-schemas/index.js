...
module.exports = {
  ...
  construct: function(self, options) {
        ...
        _.each(field.choices, function(choice) {
          _.each(choice.showFields || [], function(name) {
            if (!_.find(schema, { name: name })) {
              fail('showFields includes ' + name + ', a field that does not exist in the schema');
            }
          });
        });
      }      
    ...
    self.addFieldType({
      ...
      validate: function(field, options, warn, fail, schema) {
        if (!field.choices) {
          fail('choices property is missing for field of type checkboxes');
        }
        _.each(field.choices, function(choice) {
          _.each(choice.showFields || [], function(name) {
            if (!_.find(schema, { name: name })) {
              fail('showFields for the choice ' + choice.label + ' includes ' + name + ', a field that does not exist in the schema');
            }
          });
        });
      }      
    ...
    self.addFieldType({
      ...
      validate: function(field, options, warn, fail, schema) {
        if (!field.choices) {
          fail('choices property is missing for field of type select');
        }
        _.each(field.choices, function(choice) {
          _.each(choice.showFields || [], function(name) {
            if (!_.find(schema, { name: name })) {
    ...
    self.addFieldType({
      ...
      validate: function(field, options, warn, fail) {
        if (!field.schema) {
          fail('Field of type array has no schema property');
        }
    ...
    self.addFieldType({
      ...
      validate: function(field, options, warn, fail) {
        if (!field.schema) {
          fail('Field of type object has no schema property');
        }
        self.validate(field.schema, {
          type: options.type,
          subtype: options.subtype ? (options.subtype + '.' + field.name) : field.name
