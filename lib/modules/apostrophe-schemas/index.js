...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Delete all fields in the object `data` that are
    // marked as secret in `schema`.
    self.deleteSecret = function(schema, data) {
      _.each(schema, function(field) {
        if (field.secret) {
          delete data[field.name];
        }
        if (field.type === 'array') {
          _.each(data[field.name], _.partial(self.deleteSecret, field.schema));
        } else if (field.type === 'object') {
          self.deleteSecret(field.schema, data[field.name]);
        }
      });
    };
