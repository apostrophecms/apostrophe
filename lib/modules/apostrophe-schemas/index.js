...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.addFieldType({
      ...
      exporters: {
        string: function(req, object, field, name, output, callback) {
          ...
          output[field.name] = object[field.name];
          return setImmediate(callback);
        }
      },
      index: function(value, field, texts) {
    ...
    self.addFieldType({
      ...
      converters: {
        form: function(req, data, name, object, field, callback) {
          var submission = (typeof (data[field.name]) === 'object') ? data[field.name] : {};
    ...
    self.addFieldType({
      ...
      converters: {
        string: function (req, data, name, object, field, callback) {
          var test = self.apos.launder.string(data[field.name]);
    ...
    self.addFieldType({
      ...
      converters: {
        string: function(req, data, name, object, field, callback) {
          ...
          var titleOrId = self.apos.launder.string(data[field.name]);
    ...
    self.addFieldType({
      ...
      converters: {
        string: function(req, data, name, object, field, callback) {
          ...
          var titlesOrIds = self.apos.launder.string(data[field.name]).split(/\s*,\s*/);
    ...
    self.joinFilterChoices = function(field, cursor, valueField) {
      ...
      return function(callback) {
        ...
        function fetch(callback) {
          var manager = self.apos.docs.getManager(field.withType);
          var joinCursor = manager.find(cursor.req, { _id: { $in: ids } }).projection(manager.getAutocompleteProjection({ field: field }));
          if (field.filters) {
    ...
    self.addJoinSlugFilter = function(field, cursor, suffix) {
      ...
      cursor.addFilter(name + suffix, {
        prefinalize: function() {
          ...
          var joinCursor = self.apos.docs.getManager(field.withType).find(cursor.req).joins(false).areas(false);
          var criteria = {};
          // Even programmers appreciate shortcuts, so it's not enough that the
          // sanitizer (which doesn't apply to programmatic use) accepts these
          if (Array.isArray(value)) {
