...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.converters.csv = {
      ...
      // leading slash required). Otherwise, expect a piece-style slug
      // (no slashes at all)
      slug: function(req, data, name, snippet, field, callback) {
        ...
        snippet[name] = self.apos.utils.slugify(self.apos.launder.string(data[name], field.def), options);
        if (field.page) {
          if (!(snippet[name].charAt(0) === '/')) {
      ...
      radioTable: function(req, data, name, snippet, field, callback) {
    ...
    self.converters.form.radioTable = function(req, data, name, snippet, field, callback) {
