...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.addFieldType({
      ...
      addFilter: function(field, cursor) {
        ...
        return cursor.addFilter(field.name, {
          ...
          choices: joinByOneChoices,
          safeFor: 'public',
          sanitize: joinFilterSanitize
        });

        function joinByOneChoices(callback) {
          var ids;
          var choices;
          return async.series([
            distinct,
          ...
          function distinct(callback) {
            return cursor.toDistinct(field.idField, function(err, _ids) {
              if (err) {
                return callback(err);
              }
              ids = _ids;
          ...
          function choices(callback) {
            return joinFilterChoices(field, cursor, ids, function(err, _choices) {
              if (err) {
                return callback(err);
              }
              choices = _choices;
    ...
    function joinFilterChoices(field, cursor, ids, callback) {
      var manager = self.apos.docs.getManager(field.withType);
