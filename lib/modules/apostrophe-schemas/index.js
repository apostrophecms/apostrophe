...
module.exports = {
  ...
  init(self, options) {
    ...
    self.addFieldType({
      ...
      extend: 'area',
      isEmpty: function (field, value) {
        return self.apos.areas.isEmptySingleton({
          area: value,
    ...
    self.addFieldType({
      ...
      addFilter: function (field, query) {
        query.addFilter(field.name, {
          finalize: function () {
            if (self.queryFilterInterested(query, field.name)) {
              let criteria = {};
              criteria[field.name] = new RegExp(self.apos.utils.regExpQuote(query.get(field.name)), 'i');
              query.and(criteria);
            }
          },
          safeFor: 'manage',
        ...
        query.addFilter(field.name, {
          finalize: function () {
            if (self.queryFilterInterested(query, field.name)) {
              ...
              criteria[field.name] = new RegExp(self.apos.utils.regExpQuote(self.apos.utils.slugify(query.get(field.name), slugifyOptions)));
              query.and(criteria);
            }
          },
          safeFor: 'manage',
      ...
      addFilter: function (field, query) {
        query.addFilter(field.name, {
          finalize: function () {
            if (self.queryFilterInterested(query, field.name)) {
              let criteria = {};
              criteria[field.name] = { $in: query.get(field.name) };
              query.and(criteria);
            }
          },
          safeFor: 'manage',
    ...
    self.addFieldType({
      ...
      addFilter: function (field, query) {
        return query.addFilter(field.name, {
          finalize: function () {
            if (self.queryFilterInterested(query, field.name)) {
              let criteria = {};
    ...
    self.addFieldType({
      ...
      addFilter: function (field, query) {
        ...
        query.addFilter(field.name, {
          ...
          choices: self.joinFilterChoices(field, query, '_id'),
          safeFor: 'manage',
          launder: joinFilterLaunder
        });
        
        self.addJoinSlugFilter(field, query, '');
      },
    ...
    self.addFieldType({
      ...
      addFilter: function (field, query) {
        ...
        self.addJoinSlugFilter(field, query, '');
        self.addJoinSlugFilter(field, query, 'And');
        
        function addOperationFilter(suffix, operator) {
          return query.addFilter(field.name + suffix, {
            finalize: function () {
              
              if (!self.queryFilterInterested(query, field.name + suffix)) {
                return;
              }
  ...
  handlers(self, options) {
    return {
      'apostrophe-pages:serve': {
        blessDocTypeSchemas(req) {
          if (req.user) {
  ...
  methods(self, options) {
    return {
      ...
      // TODO: this `self.joinDriver` was overwritten and doesn't work itself.
      // This should be removed once we confirm that the subsequent
      ...
      //   query.queryToFilters(hints, 'manage');
        ...
        for (const join of joins) {
          ...
          if (Array.isArray(join.withType)) {
            ...
            for (let type of join.withType) {
              ...
              const options = {
                find: find,
                filters: { joins: withJoinsNext[join._dotPath] || false },
                hints: {}
              };
      ...
      // Given a schema and a query, add filter methods to the query
      ...
      // this is done even if such a filter is already present on the query object.
      
      addFilters(schema, options, query) {
        _.each(schema, function (field) {
          let fieldType = self.fieldTypes[field.type];
          if (query[field.name] && !_.includes(options.override || [], field.name)) {
            return;
          }
          if (fieldType.addFilter) {
            fieldType.addFilter(field, query);
          }
        });
      },
      
      // You don't need to call this. It returns an async function that, when later called
      // with no arguments, will give you filter choices based on the given field, query
      // and value field of interest. Join field types use this method to implement
      // their filter `choices`.
      
      joinFilterChoices(field, query, valueField) {
        return async function () {
          ...
          const manager = self.apos.docs.getManager(field.withType);
          const joinQuery = manager.find(query.req, { _id: { $in: ids } }).projection(manager.getAutocompleteProjection({ field: field }));
          if (field.filters) {
      ...
      // adds query filter methods for all joins.
      //
      ...
      addJoinSlugFilter(field, query, suffix) {
        ...
        if (query.filters[name + suffix]) {
          // Don't crush an existing filter by this name
          return;
        }
        
        query.addFilter(name + suffix, {
          prefinalize: async function () {
            if (!self.queryFilterInterested(query, name + suffix)) {
              return;
            }
            ...
            const joinQuery = self.apos.docs.getManager(field.withType).find(query.req).joins(false).areas(false);
            const criteria = {};
            // Even programmers appreciate shortcuts, so it's not enough that the
            // sanitizer (which doesn't apply to programmatic use) accepts these
            if (Array.isArray(value)) {
            ...
            joinQuery.projection({ _id: 1 });
            const docs = await joinQuery.toArray();
            query.set(field.name + suffix, _.map(docs, '_id'));
            query.set(name + suffix, undefined);  
          },
          choices: self.joinFilterChoices(field, query, 'slug'),
          safeFor: 'manage',
      ...
      queryFilterInterested(query, name) {
