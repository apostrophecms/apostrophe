...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.compose = function(options) {
      ...
      // if arrangeFields is an array of strings, replace the fields in the default group
      // if it is an array of objects, override groups all together
      if(options.arrangeFields && options.arrangeFields.length > 0) {
        if( _.isString(options.arrangeFields[0]) ) {
          ...
          groups = options.arrangeFields;
        }
      }

      _.each(groups, function(group) {
        // save the order so we can loop through it as we redefine fields
        var order = group.fields;
        group.fields = {};

        _.each(order, function(field) {
          group.fields[field] = _.find(schema, { name: field });

          // address special cases for strings and selects
