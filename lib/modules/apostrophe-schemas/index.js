      ...
      addFilter: function(field, cursor) {
        return cursor.addFilter(field.name, {
          finalize: function() {
            ...
            if (value) {
              var criteria = {};
              criteria[field.idField] = self.apos.launder.id(value);
              cursor.and(criteria);
            }
          },
          choices: function(callback) {
            return cursor.toDistinct(field.idField, function(err, ids) {
              if (err) {
                return callback(err);
              }
              var manager = self.apos.docs.getManager(field.withType);
              return manager.find({ _id: { $in: ids } }).projection(manager.getAutocompleteProjection({ field: field })).toArray(function(err, docs) {
                if (err) {
                  return callback(err);
                }
