...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // if supported by the field type

    self.addFilters = function(schema, cursor) {
      _.each(schema, function(field) {
        var fieldType = self.fieldTypes[field.type];
      ...
      addFilter: function(field, cursor) {
        cursor.addFilter(field.name, {
          ...
          sanitize: function(tags) {
            return self.apos.launder.tags(tags);
          },
          choices: function(callback) {
            return self.sortedDistinct(field.name, cursor, callback);
          }
      ...
      addFilter: function(field, cursor) {
        return cursor.addFilter(field.name, {
          finalize: function() {
            if (self.cursorFilterInterested(cursor, field.name)) {
              var criteria = {};
              criteria[field.name] = { $in: self.apos.launder.tags([ cursor.get(field.name) ]) };
              cursor.and(criteria);
            }
          },
          safeFor: 'public',
        ...
        return cursor.addFilter(field.name, {
          finalize: function() {
            ...
            if (value) {
              ...
              if (Array.isArray(value)) {
                value = { rule: 'or', ids: value };
              } else if (typeof(value) !== 'object') {
                if (value === 'none') {
                  value = { rule: 'none' };
                } else {
                  value = { rule: 'and', ids: [ value ] };
                }
              }

              if (value.rule === 'and') {
                // Doesn't truly make sense for joinByOne, so use $in
                criteria[field.idField] = { $in: value.ids };
              } else if (value.rule === 'or') {
                criteria[field.idField] = { $in: value.ids };
              } else if (value.rule === 'none') {
                criteria.$or = [];
                var clause = {};
                ...
                criteria.$or.push(clause);
              } else if (value.rule === 'noOperation') {
                // For instance, they passed the parameter but with zero IDs
                return;
              }
              cursor.and(criteria);
            }
          ...
          choices: function(callback) {
            ...
            function distinct(callback) {
              return cursor.toDistinct(field.idsField, function(err, _ids) {
                if (err) {
                  return callback(err);
                }
                ids = _ids;
            ...
            function choices(callback) {
              return joinFilterChoices(field, ids, function(err, _choices) {
                if (err) {
                  return callback(err);
                }
                choices = _choices;
        ...
        return cursor.addFilter(field.name, {
          finalize: function() {
            ...
            if (value) {
              var criteria = {};
              if (value.rule === 'and') {
                criteria[field.idsField] = { $all: value.ids };
              } else if (value.rule === 'or') {
                criteria[field.idsField] = { $in: value.ids };
              } else if (value.rule === 'none') {
                // Null, a nonexistent ids property and an empty array
                // all count as "none"
                criteria.$or = [];
                var clause = {};
          ...
          choices: function(callback) {
            ...
            function distinct(callback) {
              return cursor.toDistinct(field.idsField, function(err, ids) {
                if (err) {
                  return callback(err);
                }
                ids = _ids;
            ...
            function choices(callback) {
              return joinFilterChoices(field, ids, callback);
            }
          },
          safeFor: 'public',
          sanitize: joinFilterSanitize
    ...
    function joinFilterSanitize(v) {
      var clean = {
        rule: 'or'
      };
      if (v && (typeof(v) === 'object')) {
        clean.rule = self.apos.launder.select(v.rule, [ 'or', 'and', 'none' ], 'or');
        clean.ids = self.apos.launder.ids(v.ids);
      } else if (Array.isArray(v)) {
        clean.ids = self.apos.launder.ids(v);
      } else if (v === 'none') {
        clean.rule = 'none';
      }
      if (!clean.ids.length) {
        clean.rule = 'noOperation';
      }
      return clean;
    };

    function joinFilterChoices(field, ids, callback) {
      var manager = self.apos.docs.getManager(field.withType);
      var joinCursor = manager.find({ _id: { $in: ids } }).projection(manager.getAutocompleteProjection({ field: field }));
      if (field.filters) {
      ...
      return joinCursor.toArray(function(err, docs) {
        ...
        // TODO an option to further sort these would be interesting but should not be
        // mandatory since the default sort for the joined doc type might be what they want
        var choices = _.map(docs, function(doc) {
          return {
            value: doc._id,
            label: manager.getAutocompleteTitle(doc, { field: field })
          };
        });
