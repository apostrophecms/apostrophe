...
module.exports = function(self, options) {
  ...
  self.query = function(url, options, mainCallback) {
    if (!mainCallback) {
    ...
    return async.series({
      ...
      fetch: function(callback) {
        if (options.alwaysIframe) {
          return self.iframe(url, options, function(err, _response) {
            if (err) {
              return callback(err);
            }
            response = _response;
        ...
        return self.oembetter.fetch(url, function(err, _response) {
          if (err) {
            // Try open graph as a fallback
            return self.openGraph(url, function(err, _response) {
              if (err) {
                return callback(err);
              }
              response = _response;
      ...
      setCache: function(callback) {
        ...
        return self.cache.set(url, response, self.options.cacheLifetime, callback);
      }
    }, function(err) {
      if (err) {
  ...
  self.openGraph = function(url, callback) {
