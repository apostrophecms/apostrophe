apos.define('apostrophe-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableSlugifyTitle = function() {
      // If a slug has not yet been specified, make sure
      // the field automatically populates itself as a
      // slugified version of the title.
      var $form = self.$el.find('[data-apos-form]');
      var $slug = $form.find('[name="slug"]');
      var $title = $form.find('[name="title"]');
      var val = $slug.val();

      $title.on('keyup', function(e) {
        $slug.val(val.replace(/[^\/]*$/, apos.utils.slugify($title.val())));
      });   
    }
