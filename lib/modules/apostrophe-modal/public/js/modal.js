  ...
  construct: function(self, options) {
    ...
    self.enableLifecycleEvents = function() {
      self.$el.on('aposModalCancel', function() {
        ...
        return self.findLastSlide().cancel();
      });
      self.$el.on('aposModalHide', function() {
        self.findLastSlide().hide();
      });
    };
    ...
    self.slideIn = function() {
      ...
      self.$dad = stack[stack.length - 1];

      if (!self.$dad.hasClass('apos-modal-slideable')) {
        return self.stackPush();
      }

      self.$dad.addClass('apos-modal-sliding');
      var $wrapper = self.$dad.find('[data-modal-contents]:first');
    ...
    self.slideOut = function() {
      ...
      self.$el.remove();
      self.$dad.find('[data-modal-content]:last').addClass('apos-modal-slide-current');
    };
