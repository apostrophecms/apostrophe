  ...
  construct: function(self, options) {
    ...
    self.enableButtonEvents = function() {
      ...
      self.$controls.on('click', '[data-apos-cancel]', function() {
        console.log('cancel click received in: ' + self.__meta.name + ' ' + Date.now());
        return self.cancel();
      });
    ...
    self.indicateCurrentModal = function(adding) {
      console.log(((adding && '>') || '<') + 'in indicateCurrentModal for ' + self.__meta.name);
      if (adding) {
      ...
      apos.utils.info(latest && latest.__meta.name);
      if (!latest) {
        console.log('THERE IS NO LATEST MODAL');
      }
      if (latest) {
    ...
    self.cancel = function(callback) {
      console.log('cancelling ' + self.__meta.name);
      console.trace();
      return async.eachSeries(self.getViews(), function(view, callback) {
    ...
    self.stackPop = function() {
      ...
      self.$el.one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function() {
        console.log('removing elements on transition end for ' + self.__meta.name);
        $blackout.remove();
        self.$el.remove();
