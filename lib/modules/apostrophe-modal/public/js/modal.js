...
apos.define('apostrophe-modal', {
  ...
  construct: function(self, options) {
    ...
    self.show = function() {
      ...
      apos.modalSupport.loadVirtualAreas(self.$el);
      self.shown = true;
    };
...
apos.modalSupport = {
  ...
  // controls won't be clickable. Intentionally keeping it out as a seperate method so that it's reusable in the future.
  toggleModalControls: function ($modal, disable) {
    if (disable) {
      $modal.find('.apos-modal-controls').children().addClass('apos-container-disabled');
    } else {
      $modal.find('.apos-modal-controls').children().removeClass('apos-container-disabled');
    }
  },
  loadVirtualAreas: function ($modal) {
    var $virtualAreas = $modal.find('[data-virtual-area]');
    ...
    if ($virtualAreas.length > 0) {
      ...
      apos.modalSupport.toggleModalControls($modal, true);
      $virtualAreas.each(function (index, element) {
        var $fieldset = $(element);
        var area = $fieldset.data('virtualArea');
        // Remove the data-attribute after extracting data.
        $fieldset.removeData('virtualArea');
        ...
        $.jsonCall(apos.areas.options.action + '/edit-virtual-area',
          ...
          { items: area.items, options: area.options }, function (data) {
            ...
            if (virtualAreaLength === virtualAreaLoadedCount) {
              apos.modalSupport.toggleModalControls($modal, false);
            }
          });
      });
    }
