...
apos.define('apostrophe-modal', {
  ...
  construct: function(self, options) {
    ...
    self.show = function() {
      ...
      self.initializeVirtualAreaObserver();
      self.shown = true;
    };

    // Populates Virtual Areas.
    self.initializeVirtualAreaObserver = function () {
      // Intersection Observer Configuration.
      const virtualAreaObserverConfig = {
        root: null,
        threshold: 0.1
      };
      const virtulAreaObserver = new window.IntersectionObserver(function (entries, areaObserver) {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            // Check for the data attribute before appending.
            var areaAttribute = $(entry.target).attr('data-virtual-area');
            if (!areaAttribute) return;
            var $fieldset = $(entry.target);
            var area = JSON.parse(areaAttribute);
            // Remove the data attribute as it is no longer required.
            $fieldset.removeAttr('data-virtual-area');
            // Unobserve the target as it should happen only once.
      ...
      }, virtualAreaObserverConfig);
      // Adds virtual areas to observer.
      $('[data-virtual-area]').each((index, element) => {
        virtulAreaObserver.observe(element);
      })
    }
