apos.define('apostrophe-modal', {
  ...
  construct: function(self, options) {
    self.getSource = function(callback) {
      return self.html(options.source, options.query || {}, function(html) {
        self.$el = $(html);
    ...
    self.link = function(verb, subject, fn) {
      if (arguments.length === 2) {
        fn = subject;
        subject = null;
      }
      return apos.utils.link(self.$el, verb, subject, fn);
    };
    ...
    // fetching HTML fragments asynchronously. Unlike a
    // $.get request there is no limit to the size of the
