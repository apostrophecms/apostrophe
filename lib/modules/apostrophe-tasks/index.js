...
module.exports = {
  ...
  afterConstruct: function(self) {
    self.apos.on('runTask', 'run', self.run);
  },
  ...
  construct: function(self, options) {
    ...
    //
    // Thisis an async function and should be awaited.
    //
    // Examples (assume `products` extends `apostrophe-pieces`):
    //
    // `await self.apos.tasks.invoke('apostrophe-users:add', [ 'admin', 'admin' ])`
    //
    // `await self.apos.tasks.invoke('products:generate', { total: 20 })`
    //
    ...
    self.invoke = async function(name, args, options) {
    ...
    self.add = function(groupName, name, usage, fn) {
    ...
    self.on('apostrophe:run', 'runTask', async function(isTask) {
