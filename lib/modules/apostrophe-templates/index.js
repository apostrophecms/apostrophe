...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.on('apostrophe:modulesReady', 'wrapHelpersForTemplateAposObject', function() {
          ...
          if (typeof (value) === 'object') {
            ...
            object[key] = function() {
              try {
                ...
                self.apos.utils.error('^^^^^ LOOK UP HERE FOR THE LOCATION WITHIN YOUR HELPER');
                throw e;
              }
            };
          }
    ...
    self.renderForModule = async function(req, name, data, module) {
    ...
    self.renderStringForModule = async function(req, s, data, module) {
      if (typeof (req) !== 'object') {
        throw new Error('The first argument to module.render must be req. If you are trying to implement a Nunjucks helper function, use module.partial.');
      }
    ...
    self.renderBody = async function(req, type, s, data, module) {
    ...
    self.getEnv = function(req, module) {
    ...
    self.newEnv = function(req, moduleName, dirs) {

      let loader = self.newLoader(moduleName, dirs, undefined, self);

      let env = new self.nunjucks.Environment(loader, { autoescape: true, apos: self.apos, req: req });
      ...
      _.each(self.apos.modules, function(module, name) {
        if (module.options.customTags) {
          const customTags = module.options.customTags(module, module.options);
          _.each(customTags, function(config, tagName) {
      ...
      function configToExtension(name, config) {
        ...
        extension.run = async function(context /* , ..., callback */) {
          const callback = arguments[arguments.length - 1];
          try {
            // Pass req, followed by other args that are not "context" (first)
            // or "callback" (last)
            const args = [ context.env.opts.req, ...([].slice.call(arguments, 1, arguments.length - 1)) ];
            const result = await config.run.apply(config, args);
            return callback(null, self.apos.templates.safe(result));
          } catch (e) {
            console.error(e);
            return callback(e);
          }
    ...
    self.renderPageForModule = async function(req, template, data, module) {
      ...
      let globalCalls = self.apos.push.getBrowserCalls('always');
      if (scene === 'user') {
      ...
      let reqCalls = req.getBrowserCalls();
      ...
      let parsed = urls.parse(req.url, true);
      ...
      try {
        if (typeof (template) === 'string') {
          ...
          content = await template(req, args);
        }
      } catch (e) {
        // The page template
