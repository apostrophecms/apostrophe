...
module.exports = {
  customTags: function (self, options) {
    return { component: require('./lib/custom-tags/component')(self, options) };
  },
  options: { alias: 'templates' },
  init(self, options) {
    ...
    return {
      'apostrophe:afterInit': {
        wrapHelpersForTemplateAposObject() {
          ...
          console.log('checking self.templateApos', self.templateApos);
  ...
  methods(self, options) {
    return {
      ...
      addHelpersForModule(module, object) {
        console.log('AHFM for ' + module.__meta.name);
      ...
      addStandardFilters(env) {
        ...
        env.addFilter('bless', function (options) {
      ...
      addBodyDataAttribute(req, name, value = '') {
        ...
        _.each(values, (value, key) => {
          ...
          req.data.aposBodyDataAttributes = (req.data.aposBodyDataAttributes ? req.data.aposBodyDataAttributes + ' ' : ' ') + ('data-' + (!_.isUndefined(value) && value.toString().length > 0 ? self.apos.utils.escapeHtml(key) + ('="' + self.apos.utils.escapeHtml(value) + '"') : self.apos.utils.escapeHtml(key)));
        });
      },
