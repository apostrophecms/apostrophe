  ...
  construct: function(self, options) {
    ...
    self.renderBody = function(req, type, s, data, module) {
      ...
      try {
        ...
        if (self.apos.modules['apostrophe-i18n'].options.tooltips && self.apos.permissions.can(req, 'i18n-tooltips')) {
          _.each(fns, function(name) {
            env.addGlobal(name, function(key) {
              req.session.i18nTooltips = req.session.i18nTooltips || {};
              var s = req.res[name].apply(null, arguments);
          ...
          _.each(fns, function(name) {
            env.addGlobal(name, req.res[name]);
          });
        }
    ...
    self.apiRoute('post', 'i18n-tooltips', function(req, res, next) {
      return next(null, {
