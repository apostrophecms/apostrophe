      ...
      try {
        ...
        _.each(fns, function(name) {
          env.addGlobal(name, function(key) {
            // We should have made `module` an argument to `self.i18n`, but we
            // didn't, and it's too late to change that while maintaining bc.
            //
            // So we make the module that called render() available to `self.i18n`
            // as `req.aposI18nCallingModule`. This is OK because `self.i18n` is
            // a synchronous function. Overrides that don't care can just ignore it.
            var previous = req.aposI18nCallingModule;
            req.aposI18nCallingModule = module;
            var result = self.i18n.apply(null, [ req, name ].concat(Array.prototype.slice.call(arguments)));
            req.aposI18nCallingModule = previous;
            return result;
          });
        });
