  ...
  construct: function(self, options) {
    ...
    self.renderBody = function(req, type, s, data, module) {
      ...
      try {
        ...
        var fns = [ '__', '__n', '__mf', '__l', '__h', '__ns', '__ns_n', '__ns_mf', '__ns_l', '__ns_h' ];
    ...
    //
    // In the default implementation, namespaced calls like `__ns('apostrophe', 'test')` or
    // `__ns_n('apostrophe', '%s cat', 2)` are simply passed through to the
    // regular, non-namespaced functions. Modules that actually leverage the
    // namespacing are delivered as separate npm modules for those that need them.
    ...
    self.i18n = function(req, operation, key /* , additional arguments */) {
      ...
      var namespaces = self.apos.modules['apostrophe-i18n'].namespaces;
      var namespace = 'default';
      var i18nModule;
      if (nsOperations[operation]) {
        namespace = self.apos.modules['apostrophe-i18n'].options.namespaces ? arguments[2] : 'default';
        if (!namespaces[namespace]) {
          namespaces[namespace] = self.apos.modules['apostrophe-i18n'].getI18nModule(namespace);
        }
        i18nModule = namespaces[namespace];
        console.log('locale is ' + req.locale);
        i18nModule.setLocale(req.locale.replace(/-draft/, ''));
        return i18nModule[nsOperations[operation]].apply(i18nModule, Array.prototype.slice.call(arguments, 3));
      } else {
        // This one always exists already
        i18nModule = namespaces['default'];
        i18nModule.setLocale(req.locale);
        return i18nModule[operation].apply(i18nModule, Array.prototype.slice.call(arguments, 2));
      }
    };
