...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.templateApos = {
      modules: {},
      log: function(msg) {
        self.apos.log.apply(self.apos, arguments);
      },
          ...
          if (typeof (value) === 'object') {
            ...
            object[key] = function() {
              try {
                ...
                self.apos.error(e);
                self.apos.error(e.stack);
                self.apos.error('^^^^^ LOOK UP HERE FOR THE LOCATION WITHIN YOUR HELPER');
                throw e;
              }
            };
          }
    ...
    self.renderBody = function(req, type, s, data, module) {
      ...
      try {
        ...
        self.apos.error('e.stack: ', e.stack);
        throw e;
      };
      self.renderDepth--;
      if (!self.renderDepth) {
    ...
    self.renderPageForModule = function(req, template, data, module) {
      ...
      function error(e, type) {
        ...
        self.apos.error(':: ' + now + ': ' + type + ' error at ' + req.url);
        self.apos.error('Current user: ' + (req.user ? req.user.username : 'none'));
        self.apos.error(e);
        req.statusCode = 500;
