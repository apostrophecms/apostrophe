...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.pieceFilters = self.options.pieceFilters || [];
    ...
    self.indexPage = function(req, callback) {
      ...
      function getFilters(callback) {
        return self.populatePieceFilters(cursor, callback);
      }
    ...
    // Populate `req.data.pieceFilters` with arrays of choice objects,
    // with label and value properties, for each filter configured in the
    // `pieceFilters` array option. Each filter in that array must have a
    ...
    self.populatePieceFilters = function(cursor, callback) {
      var req = cursor.get('req');
      req.data.pieceFilters = req.data.pieceFilters || {};
      return async.eachSeries(self.pieceFilters || [], function(filter, callback) {
        ...
        return _cursor.toChoices(filter.name, function(err, choices) {
          ...
          req.data.pieceFilters[filter.name] = choices;
          return callback(null);
        });
      }, callback);
    };
