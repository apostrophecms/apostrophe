...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.browseFilters = self.options.browseFilters || [];
    ...
    self.indexPage = function(req, callback) {
      ...
      function getFilters(callback) {
        return self.populateBrowseFilters(cursor, callback);
      }
    ...
    // Populate `req.data.browseFilters` with arrays of choice objects,
    // with label and value properties, for each filter configured in the
    // `browseFilters` array option. Each filter in that array must have a
    ...
    self.populateBrowseFilters = function(cursor, callback) {
      var req = cursor.get('req');
      req.data.browseFilters = req.data.browseFilters || {};
      return async.eachSeries(self.browseFilters || [], function(filter, callback) {
        ...
        return _cursor.toChoices(filter.name, function(err, choices) {
          ...
          req.data.browseFilters[filter.name] = choices;
          return callback(null);
        });
      }, callback);
    };
