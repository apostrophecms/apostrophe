...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.indexCursor = function(req) {
      const cursor = self.pieces.find(req, {})
        .safeFilters(_.map(self.piecesFilters, 'name'))
    ...
    self.beforeIndex = async function(req) {
    };
    ...
    self.showPage = async function(req) {
      ...
      async function findPrevious() {
        ...
        const cursor = self.indexCursor(req);
        previous = await cursor.previous(doc)
          .applyFilters(
    ...
    self.beforeShow = async function(req) {
    };
    ...
    self.addUrlsToPieces = async function(req, results) {
      const pieceName = self.pieces.name;
      if (!(req.aposParentPageCache && req.aposParentPageCache[pieceName])) {
        const pages = await self.findForAddUrlsToPieces(req)
          .toArray();
    ...
    self.populatePiecesFilters = async function(cursor) {
