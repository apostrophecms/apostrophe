...
module.exports = {
  ...
  methods(self, options) {
    return {
      ...
      indexCursor(req) {
        const cursor = self.pieces.find(req, {}).safeFilters(_.map(self.piecesFilters, 'name')).queryToFilters(req.query, 'public').perPage(self.perPage);
      ...
      async showPage(req) {
        ...
        async function findPrevious() {
          ...
          const cursor = self.indexCursor(req);
          previous = await cursor.previous(doc).applyFilters(typeof self.options.previous === 'object' ? self.options.previous : {}).toObject();
        }
        ...
        async function findNext() {
          ...
          const cursor = self.indexCursor(req);
          next = await cursor.next(doc).applyFilters(typeof self.options.next === 'object' ? self.options.next : {}).toObject();
        }  
      },
