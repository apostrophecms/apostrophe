...
module.exports = {
  ...
  construct: function(self, options) {
      ...
      if (fs.existsSync(to)) {
        ...
        if (stat.isSymbolicLink()) {
          try {
            ...
            self.apos.error('WARNING: old style symbolic link exists at ' + to + ' and I do not have the privileges to remove it. You probably ran this site as Administrator before. Remove that symbolic link and the others in that folder as Administrator and try again as this non-Administrator user.');
            process.exit(1);
          }
        } else {
          rimraf.sync(to);
    ...
    self.minify = function(callback) {
      ...
      return self.forAllAssetScenes(function(scene, callback) {
        return async.eachSeries([ 'stylesheets', 'scripts' ], function(type, callback) {
          ...
          if (!needed) {
            needed = true;
            self.log('MINIFYING, this may take a minute...');
          }
    ...
    self.minifyStylesheet = function(stylesheet, callback) {
      return self.compileStylesheet(stylesheet, function(err, code) {
        ...
        if (minified.warnings.length) {
          self.apos.error(minified.warnings.length);
        }
    ...
    self.minifyScript = function(script, callback) {
      ...
      if (!exists) {
        self.apos.warn("Warning: " + script.file + " does not exist");
        return callback(null);
      }
      if (script.preshrunk) {
    ...
    self.compileStylesheet = function(stylesheet, callback) {
      ...
      if (!exists) {
        self.apos.warn('WARNING: stylesheet ' + stylesheet.file + ' does not exist');
        return callback(null, '');
      }
      ...
      return less.render(fs.readFileSync(src, 'utf8'), lessOptions, function(err, css) {
        if (err) {
          self.apos.error('LESS CSS ERROR:');
          self.apos.error(err);
        }
        css = css.css;
