...
//
// If `apostrophe:generation --create-bundle` is invoked without a bundle name,
// or APOS_BUNDLE=1 is present in the environment, `minify` is always treated as `true`.
...
// ### `APOS_BUNDLE`
//
// If set to `1`, Apostrophe will generate static asset URLs referring to the latest asset bundle created and
// tracked in the database by the most recent `apostrophe:generation --create-bundle` run. Apostrophe will also
// extract a tarball of the complete asset bundle as a guard against any URLs or filesystem manipulation that
// is not uploadfs-aware and also needs to see these static assets.
// See [Deploying Apostrophe in the Cloud with Heroku](https://apostrophecms.org/docs/tutorials/howtos/deploying-apostrophe-in-the-cloud-with-heroku.html)
// for more information.
//
// Alternatively, if you specified an explicit bundle name to `--create-bundle`, stored it to git and deployed it,
// you can specify that bundle name as the value of APOS_BUNDLE. But this is more work; we recommend the easy way.
//
// ### `APOS_BUNDLE_IN_UPLOADFS`
//
// For use when `APOS_BUNDLE` is set to an explicit bundle name but you still wish static asset URLs to be
// generated to reference those filesvia uploadfs. But this is the hard way; just run `apostrophe:generation --create-bundle` with
// NO bundle name, and `APOS_BUNDLE=1` in the environment when launching Apostrophe.
// See [Deploying Apostrophe in the Cloud with Heroku](https://apostrophecms.org/docs/tutorials/howtos/deploying-apostrophe-in-the-cloud-with-heroku.html)
// for more information.
//
...
module.exports = {
  ...
  afterConstruct: function(self) {
    self.apos.tasks.add('apostrophe', 'generation',
      ...
      'To create an asset bundle zipfile in uploadfs as a predeployment task, use the\n' +
      '--create-bundle option. Later, on production, start the site normally\n' +
      'with APOS_BUNDLE=1 and the bundle will be extracted. You can also specify a bundle\n' +
      'name in both cases in which case it is written to the project root as a folder\n' +
      'by that name and will be extracted from there as well. This approach is used\n' +
      'if you prefer to run this task in dev and commit the results.',
  ...
  construct: function(self, options) {
    ...
    // If APOS_BUNDLE is '1', determine the current asset generation via the database and
    // set `self.generation` accordingly. If we are running the generation task and
    // --create-bundle is present without a bundle name, set the generation id in the
    // database. In all other cases, determine the generation via legacy methods.

    self.determineGenerationFromDb = function() {
      if (self.isGenerationTask && (self.apos.argv['create-bundle'] === true)) {
      ...
      return Promise.try(function() {
        ...
        if (!c) {
          throw new Error('You must first run the apostrophe:generation task with the --create-bundle option; do not give a bundle name when doing so');
        }
    ...
    self.determineGeneration = function() {
      ...
      if (self.isGenerationTask) {
        ...
        var bundle = self.apos.argv['create-bundle'];
        if (bundle) {
          if (bundle === true) {
    ...
    self.enableBundles = function() {
      ...
      // Handles the synchronous filesystem bundle case, See also extractBundleFromUploadfs
      self.extractBundleIfAppropriate();
    };

    // Extracts the appropriate asset bundle from uploadfs if APOS_BUNDLE is '1'.
    // Returns a promise. Called on the modulesReady event.

    self.extractBundleFromUploadfs = function() {
      if (process.env.APOS_BUNDLE !== '1') {
        return;
      }
      return Promise.try(function() {
        return Promise.promisify(self.apos.attachments.uploadfs.copyOut)(self.getUploadfsBundlePath(), self.getUploadfsBundleTempName());
      }).then(function() {
        return tar.extract({
    ...
    self.on('apostrophe:modulesReady', 'determineGenerationAndExtract', function() {
      return Promise.try(function() {
        ...
        return self.extractBundleFromUploadfs();
      });
    });

    // Extract an asset bundle found in the filesystem if appropriate. The default
    // implementation // looks for an APOS_BUNDLE=XYZ environment variable and,
    // if present, extracts a bundle with the name XYZ.
    //
    // In the more common case where APOS_BUNDLE=1 the appropriate bundle is
    // found via the db, fetched from uploadfs and extracted in the
    // extractBundle method. The name of this method is kept for legacy reasons.
    ...
    self.afterInit = function(callback) {
      ...
      if (self.fromBundle && (!self.isGenerationTask)) {
        if (process.env.APOS_BUNDLE_IN_UPLOADFS || (process.env.APOS_BUNDLE === '1')) {
    ...
    self.generationTask = function(callback) {
      ...
      function copyFilesToUploadfs(callback) {
        if (!(self.apos.argv['sync-to-uploadfs'] || (self.apos.argv['create-bundle'] === true))) {
          return callback(null);
        }
      ...
      function copyTarballToUploadfs(callback) {
        if (!(self.apos.argv['create-bundle'] === true)) { 
          return callback(null);
        }
        ...
        [
          ...
          return Promise.promisify(self.apos.attachments.uploadfs.copyIn)(temp, self.getUploadfsBundlePath());
        }).then(function() {
          return callback(null);
        }).catch(callback);
      }
    ...
    self.getUploadfsBundleTempName = function() {
      return self.apos.rootDir + '/data/temp/assets-' + self.generation + '.tar.gz';
    };
