...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.linkAssetFolder = function(from, to) {
      ...
      // The environment variable is checked to simplify testing
      // of the Windows-style approach on other platforms.
      // You won't need it, the process.platform check is reliable.
      var platform = process.env.APOS_PLATFORM || process.platform;
    ...
    self.linkAssetFolderOnWindows = function(from, to) {
      ...
      //
      // If `from` is a symlink, this is considered an error to be cleaned
      // up, because in Windows we do not use them but older versions
      // attempted to.
      self.removeIfSymlink(from);
      // Switched order is deliberate, see above.
      self.syncFolder(to, from);
    };

    self.removeIfSymlink = function(folder) {
      var stat;
      if (fs.existsSync(folder)) {
    ...
    // Sync the folder `from` to `to`. Endeavors to minimize changes,
    // based on modification times. Creates `to` if necessary.
    //
    // This is a synchronous function. Tt should not be used in response
    // to web requests, only for startup activities.
    //
    // Files and folders in `to` that do not exist in `from` are
    // removed.
    //
    // If `to` does not exist it is created.

    self.syncFolder = function(from, to) {
      ...
      // It is strange to reimplement file sync, but I haven't been able to find
      // a well-supported module with a synchronous programmatic interface. -Tom

      // Copy new and updated things
      syncDir(from, to);
      // Remove obsolete things
      cleanDir(from, to);

      function syncDir(from, to) {
        existsFrom[from] = true;
        var toStat;
        try {
          toStat = fs.statSync(to);
        } catch (e) {
          // Does not yet exist on receiving end
          fs.mkdirSync(to);
        }
        if (toStat && (!toStat.isDirectory())) {
          // Was a file, now it's a directory
          fs.unlinkSync(to);
          fs.mkdirSync(to);
        }
        ...
        _.each(files, function(file) {
          ...
          if (stat.isDirectory()) {
            syncDir(fromFile, toFile);
          } else {
            syncFile(fromFile, toFile);
          }
        });
      }

      function syncFile(fromFile, toFile) {
        existsFrom[fromFile] = true;
        ...
        if (isNew || (fromStat.mtime > toStat.mtime)) {
      ...
      function cleanDir(from, to) {
        var files = fs.readdirSync(to);
        _.each(files, function(file) {
          var fromFile = from + '/' + file;
          var toFile = to + '/' + file;
          if (!existsFrom[fromFile]) {
            rimraf.sync(toFile);
            return;
          }
          var stat = fs.statSync(toFile);
          if (stat.isDirectory()) {
            cleanDir(fromFile, toFile);
          }
        });
      }
      ...
      return self.compileStylesheet(stylesheet, function(err, code) {
        ...
        if (!self.cleanCss) {
          ...
          self.cleanCss = new CleanCss({
            root: self.getAssetRoot() + '/public/apos-minified'
          });
        }
        var minified = self.cleanCss.minify(code);
        if (minified.warnings.length) {
          self.apos.utils.error(minified.warnings.length);
        }
