...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.syncToUploadfs = function(from, to, callback) {
      ...
      var copies = [];
      var oldCopies = [];
      var collection = self.apos.db.collection('apostropheAssets');
      ...
      return performCopies(copies, callback);
      ...
      // uploadfs doesn't have directory listing functions because they would have
      // lousy performance if it did. It's our job to remember what we had before,
      // compare and remove what is not wanted
      function getOld(callback) {
        return collection.findOne({ _id: 'uploadfsCatalog' }, function(err, old) {
          if (err) {
            return callback(err);
          }
          if (!old) {
          ...
          oldCopies = old.copies;
          return callback(null);
        });
      }
      
      function purgeOld(callback) {
        var orphans = {};
        _.each(oldCopies, function(copy) {
          orphans[JSON.stringify(copy)] = copy;
        });
        _.each(copies, function(copy) {
          delete orphans[JSON.stringify(copy)];
        });
        return async.eachLimit(orphans, 5, function(copy, callback) {
          return self.apos.attachments.uploadfs.remove(copy.to, function(err) {
            // Presumably already gone, not a problem
            return callback(null);
          });
        }, callback);
      }
      ...
      function storeNew(callback) {
        return collection.update({ _id: 'uploadfsCatalog' }, { copies: copies }, { upsert: true }, callback);
      }
    ...
    self.compileStylesheet = function(stylesheet, callback) {
      ...
      return less.render(fs.readFileSync(src, 'utf8'),
      {
        ...
        css = css.css;
        if (self.toBundleName === 'uploadfs') {
          css = self.prefixCssUrlsWith(css, self.apos.attachments.uploadfs.getUrl() + '/assets');
    ...
    self.assetUrl = function(web) {
      if (process.env.APOS_BUNDLE === 'uploadfs') {
        return self.apos.attachments.uploadfs.getUrl() + '/assets' + web;
      }
