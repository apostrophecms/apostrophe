...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.linkAssetFolderOnWindows = function(from, to) {
      ...
      if (fs.existsSync(from)) {
        ...
        if (stat.isSymbolicLink()) {
          try {
            ...
            console.error('WARNING: old style symbolic link exists at ' + from + ' and I do not have the privileges to remove it. You probably ran this site as Administrator before. Remove that symbolic link and the others in that folder as Administrator and try again as this non-Administrator user.');
            process.exit(1);
          }
        } else {
          rimraf.sync(from);
      ...
      //
      // "Are these arguments backwards?" No. The goal was to make "from" a symbolic link
      // to the asset folder "to". Since we can't symlink in windows, we make "from" a
      // COPY of to instead, which amounts to the same thing. -Tom

      copyDir(to, from);

      // No these arguments are NOT backwards see above. -Tom
