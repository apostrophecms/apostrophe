...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.afterInit = function(callback) {
      ...
      if (process.env.APOS_LAZY_MINIFY) {
        if (self.apos.isTask()) {
          // Minification is handled at regular startup
          return setImmediate(callback);
        }
      } else {
      ...
      if (process.env.APOS_LAZY_MINIFY) {
        self.minifyPending = true;
        // Intentionally continue in this function
        // after invoking the callback, to do the
        // minification work in the background
        callback(null);
        return minify(function(err) {
          if (err) {
            self.apos.utils.error('An error occurred during minification. Unminified assets\nwill be served, which wastes bandwidth:\n', err);
          }
          self.minifyPending = false;
        });
      } else {
        return minify(callback);
      }
        ...
        if (!fs.existsSync(masterFile)) {
          fs.writeFileSync(masterFile, _.map(stylesheets, function(stylesheet) {
            ...
            if (keywords.indexOf('inline') !== -1) {
              ...
              // Don't try to do it in Windows Subsystem for Linux, at least until
              // chokidar sounds the all-clear for that (also does not work in fs.watch really)
              ...
              if (!require('is-wsl')) {
                ...
                self.lessCssWatches.push(watcher);
              }
            }
            if (keywords.length) {
    ...
    self.stylesheetsHelper = function(when) {
      if (self.options.minify && !self.minifyPending) {
