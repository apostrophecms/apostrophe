...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // are removed.
    //
    // An array of copies performed, with `from` and `to` properties for
    // each entry, is delivered to the callback as the second argument.
    ...
    self.syncToUploadfs = function(from, to, callback) {
      ...
      function performCopies(callback) {
        return async.eachLimit(copies, 5, function(copy, callback) {
          return self.apos.attachments.uploadfs.copyIn(copy.from, copy.to, function(err) {
            return callback(err);
          });
        }, callback);
      }
