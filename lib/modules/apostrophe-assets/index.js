...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.determineGeneration = function() {
      ...
      self.generationTask = (self.apos.argv._[0] === 'apostrophe:generation');

      var generation;

      if (self.generationTask) {
    ...
    self.apos.templates.addToApos({
      assets: {
        ...
        stylesheets: function(when) {
          if (self.options.minify) {
            return '<link href="' + self.apos.prefix + '/apos-minified/' + when + '-' + self.generation + '.css" rel="stylesheet" />';
          } else {
            if (self.lessMasters && self.lessMasters[when]) {
              return '<link href="' + self.apos.prefix + self.lessMasters[when].web + '" rel="stylesheet" />';
            }
            return _.map(self.filterAssets(self.pushed['stylesheets'], when), function(stylesheet) {
