  ...
  construct: function(self, options) {
    ...
    self.open = function(verb, data, callback) {
      self.api(verb, data, function(result) {
        ...
        if (self.manager.options.canEditTrash) {
          apos.docs.inTrashMixin(self, object);
        }
    ...
    self.saveContent = function(callback) {
      ...
      return async.series({
        ...
        convertTrash: function(callback) {
          return self.convertTrash(piece, callback);
        },
    ...
    self.convertTrash = function(piece, callback) {
      // Only comes into play when canEditTrash is in effect
      if (self.options.canEditTrash) {
        piece.trash = self.inTrash();
      }
      return setImmediate(callback);
    };
