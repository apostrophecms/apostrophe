  ...
  construct: function(self, options) {
    ...
    self.enableSearch = function() {
      ...
      self.$filters.on('keyup', inputSelector, function(e) {
        ...
        // TODO: search only or current word
        ...
        self.api('suggestions', options, function(results) {
          if (results.status === 'ok' && results.data.length) {
      ...
      function getTypingWord (search, cursorIndex) {
        ...
        spacesIndexes.forEach((index, i) => {
          var nextIndex = spacesIndexes[i + 1];
          if (index <= cursorIndex && (nextIndex >= cursorIndex || (!nextIndex && !word))) {
            searchSliceIndexes = [index + 1, nextIndex || search.length];
            word = search.substring(index, nextIndex);
