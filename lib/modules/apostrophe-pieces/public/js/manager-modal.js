...
apos.define('apostrophe-pieces-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableCreateViaUpload = function() {
      if (!self.options.createViaUpload) {
        return;
      }
      ...
      // (see below).
      console.log(getUploader().length);
      getUploader().fileupload({
        ...
        url: self.action + '/create-via-upload',
        start: function (e) {
          // Only way to know this is the beginning of the whole show
          if (!getUploader().fileupload('active')) {
            busy(true);
            self.createViaUploadStatus = { total: 0, unsuitable: 0, errors: 0 };
          }
        },
        ...
        fail: function (e, data) {
          self.createViaUploadStatus.errors++;
          feedback();
        },
          ...
          if (data.result.status === 'ok') {
            ...
            self.createViaUploadStatus.total++;
          } else if (data.result.status === 'unsuitable') {
            self.createViaUploadStatus.unsuitable++;
          } else {
            self.createViaUploadStatus.errors++;
          }
          feedback(data);
      ...
      function feedback(data) {
        if (getUploader().fileupload('active') <= 1) {
          if (self.createViaUploadStatus.errors) {
            alert('Some files were damaged, of an unsuitable type or too large to be uploaded.');
          } else if (self.createViaUploadStatus.unsuitable) {
            ...
            if (self.createViaUploadStatus.total === 1) {
              self.manager.edit(data.result.piece._id);
            }
          }
        }
      }
    ...
    // used to build the query that goes to the server in a create-via-upload
