...
apos.define('apostrophe-pieces-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    self.enableFilters = function() {
      ...
      let trashFilter = _.find(self.filters, { name: 'trash' });
      if (trashFilter) {
        let superTrashHandler = trashFilter.handler;
    ...
    self.reflectBatchOperation = function() {
      ...
      _.each(self.batchOperations, function(batchOperation) {
        ...
        _.each(batchOperation.unlessFilter, function(val, key) {
          let filterVal = self.currentFilters[key];
          if (val === true) {
    ...
    self.enableSearch = function() {
      self.$filters.on('change', '[name="search-' + self.options.name + '"]', function() {
        ...
        self.refresh(function() {
          let $input = self.$filters.find('[name="search-' + self.options.name + '"]');
    ...
    self.enableCheckboxEvents = function() {
      ...
      function selectAllHandler(e) {
        ...
        let $pieces = self.$el.find('[data-piece]');
        if (checked) {
      ...
      self.$el.on('change', '[data-piece] input[type="checkbox"]', function(e) {
        let $box = $(this);
        let id = $box.closest('[data-piece]').attr('data-piece');
        if ($box.prop('checked')) {
          self.addChoice(id);
        } else {
    ...
    self.reflectSelectEverything = function () {
      let $pieces = self.$el.find('[data-piece]');
      let $checked = $pieces.find('input[type="checkbox"]:checked');
      let $selectEverything = self.getSelectEverything();
    ...
    self.reflectSelectEverythingCheckbox = function() {
      let checkIt = (self.getIds().length && self.allIds && (self.allIds.length === self.getIds().length));
    ...
    self.refreshSelectEverything = function() {
      let listOptions = self.getListOptions({ format: 'allIds' });
      apos.ui.globalBusy(true);
    ...
    self.getListOptions = function(options) {
      let listOptions = _.assign({}, options);
    ...
    self.refresh = function(callback) {
      let listOptions = self.getListOptions({ format: 'managePage' });
