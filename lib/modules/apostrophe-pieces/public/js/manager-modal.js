  ...
  construct: function(self, options) {
      ...
      function feedback(data) {
        if (getUploader().fileupload('active') <= 1) {
          if (self.insertViaUploadStatus.errors) {
            ...
            apos.notify('Some uploaded files were unsuitable for the current placement. Note the required minimum size.', { type: 'error' });
          } else {
    ...
    self.enableCheckboxEvents = function() {
      ...
      function selectAllHandler(e) {
        ...
        // temp hack while we refactor modals to use flexbox
        if (self.$el.find('.apos-modal-slide-current .apos-manage-grid-page').length) {
          if (self.$el.find('.apos-modal-footer').css('bottom') === '0px') {
            self.shrinkGrid();
          } else {
            self.growGrid();
          }
    ...
    // shrink and grow make visual adjustments to accommodate the the new Select Everything element
    self.shrinkGrid = function() {
      self.$el.find('.apos-modal-footer').css('bottom', '72px');
      self.$el.find('.apos-modal-body').css('height', 'calc(100% - 144px)');
    };

    self.growGrid = function() {
      self.$el.find('.apos-modal-footer').css('bottom', '0px');
      self.$el.find('.apos-modal-body').css('height', 'calc(100% - 72px)');
    };

    self.adjustModal = function () {
        var gridIds = self.$el.find('[data-piece]').map(function() {
          return $(this).attr('data-piece');
        }).get();

        var checkedIds = self.$el.find('[data-piece]').map(function() {
          if ($(this).find('input[type="checkbox"]').is(':checked')) {
            return $(this).attr('data-piece');
          }
        }).get();

        if (gridIds.length === checkedIds.length) {
          self.getSelectAll().prop('checked', true);
          self.showSelectEverything();
          self.shrinkGrid();
        } else {
        ...
        if (self.choices.length && (self.allIds.length === self.choices.length)) {
          self.getSelectEverything().find('input[name="select-everything"]').prop('checked', true);
    ...
    self.hideSelectEverything = function() {
      ...
      self.getSelectEverything().removeClass('apos-active');
    };
