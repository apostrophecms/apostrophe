...
module.exports = function(self, options) {
  ...
  self.getSuggestions = async function(req, term) {
    const search = self.apos.launder.string(req.body.term).toLowerCase();

    const images = await self.find(req, {
      // $or: [
      // {
      //   title: {$regex: `^${term}`}
      // }
      ...
      // }
      // ]
      // highSearchText: self.apos.utils.searchify(term)
    }).limit(10).autocomplete(search).toArray();

    const suggestions = images.reduce((acc, img) => {
      const title = img.title.toLowerCase();
      const titleMerge = (title.includes(search) && !acc.includes(title)) ? [ img.title ] : [];
      const tagsMerge = img.tags.filter((tag) => !acc.includes(tag) && tag.startsWith(search));
