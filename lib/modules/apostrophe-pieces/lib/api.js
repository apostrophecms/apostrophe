    ...
    if (!self.isAdminOnly()) {
      ...
      if (self.apos.permissions.extended) {
        self.apos.permissions.add({
          ...
          label: 'Admin: Total Control of ' + self.label
        });
        self.apos.permissions.add({
          // one word lowercase to avoid confusing
          // any regexes already out there that
          // match permissions
          value: 'updateany-' + self.name,
          // The labels here are replaced by the new permissions
          // editing UI, but are helpful as documentation or if
          // they step into the light in third party extensions we
          // don't know about
          label: 'Edit Any ' + self.label
        });
        // Applies only to pieces you can also edit
        self.apos.permissions.add({
          value: 'trash-' + self.name,
          label: 'And Trash ' + self.label
        });
        self.apos.permissions.add({
          value: 'insert-' + self.name,
          label: 'Create a New ' + self.label
        });
        // This exists to make it easier to revoke per-piece permissions quickly
        self.apos.permissions.add({
          value: 'update-' + self.name,
          label: 'Edit Specific ' + self.pluralLabel
        });
      } else {
        // legacy permissions
        // can do anything to this type
        self.apos.permissions.add({
          value: 'admin-' + self.name,
          label: 'Admin: ' + self.label
        });
        // this is really "can create, and edit where designated"
        self.apos.permissions.add({
          value: 'edit-' + self.name,
          label: 'Edit: ' + self.label
        });
        // no longer used due to workflow
        self.apos.permissions.add({
