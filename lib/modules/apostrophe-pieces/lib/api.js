...
module.exports = function(self, options) {
  ...
  // This method returns list of suggestions based on highSearchWords array,
  // it uses the autocomplete cursor to request documents, it also checks for filters.
  self.getSuggestions = async function(req, term, filters) {
    const search = self.apos.launder.string(term).toLowerCase();
    const cursor = getCursorWithFilters(filters);
    const images = await cursor.autocomplete(search).limit(15).toArray();

    const suggestions = images.reduce((acc, img) => {
      if (acc.length >= 10) {
        return acc;
      }

      const matchingWords = img.highSearchWords.filter((word) => {
        if (acc.includes(word)) {
          return false;
        }
        return acc.includes(word)
          ? false
    ...
    // adding filter to cursor only if this one exist for the current piece
    function getCursorWithFilters (filters) {
      return Object.entries(filters).reduce((cursor, [filter, value]) => {
        return cursor[filter]
          ? cursor[filter](['0', '1'].includes(value) ? parseInt(value, 10) : value)
          : cursor;
      }, self.find(req, {}));
    };
  };
  ...
  self.composeColumns = function() {
    ...
    if (options.addColumns) {
      ...
      // So we do not inadvertently modify it for other subclasses
      self.options.listProjection = Object.assign({}, self.options.listProjection);
    ...
    options.removeColumns = options.defaultRemoveColumns
  ...
  self.validateSchema = function() {
    ...
    const forbiddenFields = self.options.forbiddenFields || [];

    _.each(schema, function(field) {
      if (_.contains(forbiddenFields, field.name)) {
        throw new Error('Piece type ' + self.name + ': the field name ' + field.name + ' is forbidden');
      }
    });
  };
