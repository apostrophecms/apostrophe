...
var Promise = require('bluebird');call
...
module.exports = function(self, options) {
  ...
  // insert the piece.
  //
  // For convenience, the method resolves with the piece. It's the same piece object
  // you passed, likely with modifications such as the `updatedAt` property.
  ...
  self.update = async function(req, piece, options) {
    ...
    piece.type = self.name;
    await self.emit('beforeUpdate', piece, options);
    await self.emit('beforeSave', piece, options);
    await self.apos.docs.update(req, piece, options);
    await self.afterUpdate(req, piece, options);
    await self.afterSave(req, piece, options);
    return piece;
  };
  ...
  // `update` routes. This function is async and will be awaited
...
>>>>>>> 3.0
  ...
  self.convertInsertAndRefresh = async function(req, input) {
    ...
    await self.afterConvert(req, input, piece);
    await copy();
    ...
    async function copy() {
      ...
      await self.copyExtras(req, copyOf, input, piece);
    }
  };
  ...
  // top level extra areas have already been copied. This
  // function is async and will be awaited.
  //
  // `input` is the raw input object from the browser and
  // you should not assume it is trustworthy. `piece` is
  // the validated, sanitized piece so far.

  self.copyExtras = async function(req, copyFrom, input, piece) {
    return;
>>>>>>> 3.0
  };
  ...
  self.copyExtras = async function(req, copyFrom, piece) {
    return;
  };
