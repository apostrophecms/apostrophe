...
module.exports = function(self, options) {
  ...
  self.createRoutes = function() {
    ...
    self.route('post', 'suggestions', self.apos.docs.requireEditorOfSomething, self.routes.suggestions);
    ...
    self.route('post', 'insert-via-upload', self.requireEditor, self.apos.attachments.middleware.canUpload, self.apos.middleware.files, self.apos.middleware.heicImages, self.routes.insertViaUpload);
  };
  ...
  self.routes.suggestions = async function(req, res) {
    try {
      ...
      if (!term || typeof filters !== 'object') {
        const errorMsg = !term
          ...
          : 'Filters must be an object containing search filters.';

        return self.apiResponse(res, errorMsg);
      }

      const suggestions = await self.getSuggestions(req, term, filters);

      return self.apiResponse(res, null, suggestions);
