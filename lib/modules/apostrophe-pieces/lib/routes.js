  ...
  self.routes.list = function(req, res) {
    ...
    return async.series({
      ...
      if ((!err) && (req.body.format === 'managePage')) {
        ...
        var sort = results.cursor.get('sort');
        if (sort && _.keys(sort)[0]) {
          var simpleSort = {};
          // Put this in a simpler form for nunjucks, allows only
          // one active sort at a time
          simpleSort.name = _.keys(sort)[0];
          simpleSort.order = sort[simpleSort.name];
          results.sort = simpleSort;
        }
        results = {
          filters: self.render(req, 'manageFilters', results),
          headings: self.render(req, 'manageHeadings', results),
