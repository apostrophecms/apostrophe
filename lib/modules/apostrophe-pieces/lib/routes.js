...
module.exports = function(self, options) {
  ...
  self.createRoutes = function() {
    ...
    self.route('post', 'create-via-upload', self.apos.attachments.middleware.canUpload, self.apos.middleware.files, self.routes.createViaUpload);
  };
  ...
  // form fields. Applicable only when the `createViaUpload` option is set
  // for a particular piece type. The first schema field of type attachment is
  // populated.

  self.routes.createViaUpload  = function(req, res) {
    ...
    if (!field) {
      console.error('The pieces module ' + self.options.__meta.name + ' has createViaUpload turned on but the schema does not contain a field of type attachment.');
      res.statusCode = 404;
      return res.send('notfound');
    }
