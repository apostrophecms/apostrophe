...
module.exports = function(self, options) {
  ...
  self.createRoutes = function() {
    ...
    self.route('post', 'unpublish', self.routes.publish);
  ...
  self.routes.publish = function(req, res) {
    return self.batchSimpleRoute(req, function(req, piece, callback) {
  ...
  self.routes.tag = function(req, res) {
    return self.batchSimpleRoute(req, function(req, piece, callback) {
      ...
      piece.tags = _.uniq(self.apos.launder.tags(piece.tags.concat(req.body.tags)));
      return self.update(req, piece, callback);
    });
  };
  ...
  self.routes.untag = function(req, res) {
    return self.batchSimpleRoute(req, function(req, piece, callback) {
      ...
      var removing = self.apos.launder.tags(req.body.tags);
  ...
  // Pass req and a function that accepts (req, piece, callback),
  // does the modification on that one piece (including calling
  ...
  self.batchSimpleRoute = function(req, change) {
    console.log('in batchSimpleRoute');
    var query;
    var singlePiece;
    var res = req.res;
    if (req.body._id) {
    ...
    return async.eachSeries(ids, function(id, callback) {
      return self.findForEditing(req, { _id: id })
        .toObject(function(err, piece) {
          ...
          return change(req, piece, callback);
        }
      );
    }, function(err) {
      if (err) {
