apos.define('apostrophe-ui', {
  ...
  construct: function(self, options) {
    ...
    // think this is going to be depracated
    self.enableMenus = function() {
      var $body = $('body');

      $body.on('click', '[data-apos-admin-bar-logo]', function(e) {
        var $adminBar = $body.find('[data-apos-admin-bar]').first();
        $adminBar.toggleClass('apos-active');
      });

      // TODO all this class selector stuff has to go -jimmy
    ...
    self.enhanceDropdowns = function() {
      $('body').on('click', '[data-apos-dropdown]', function(event) {
        var $label = $(this).find('[data-apos-dropdown-label]');
        if ($label.attr('data-apos-dropdown-label') === 'active') {
          $label.css('width', '');
          $label.attr('data-apos-dropdown-label', '');
        } else {
          $label.css('width', $(this).find('[data-apos-dropdown-items]').outerWidth());
          $label.attr('data-apos-dropdown-label', 'active');
        }
      })
    }
    ...
    self.link = function(sel, verb, subject, callback) {
      ...
      if (subject === null) {
        ...
        attribute = 'data-' + apos.utils.cssName(verb) + '-' + apos.utils.cssName(subject);
      }
