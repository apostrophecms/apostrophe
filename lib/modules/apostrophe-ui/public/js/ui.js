  ...
  construct: function(self, options) {
    ...
    self.enhanceDropdowns = function() {
      $('body').on('click change', '[data-apos-dropdown]', function(event) {
        ...
        if ($this.find('[data-apos-dropdown-button-label]').length) {
          ...
          let $label = $(this).find('[data-apos-dropdown-button-label]');
          if ($label.attr('data-apos-dropdown-button-label') === 'active') {
            ...
            let val = parseInt($label.attr('data-apos-original-right-padding').substring(0, $label.attr('data-apos-original-right-padding').length - 2));
            let extra = ($(this).find('[data-apos-dropdown-items]')[0].getBoundingClientRect().width - $label[0].getBoundingClientRect().width + val);
    ...
    self.enableClickUrl = function() {
      ...
      setTimeout(function() {
        ...
        let matches = hash.match(/^#click-(.*)$/);
        if (matches) {
          let $element = $('[data-' + matches[1] + ']');
          if ($element.length) {
    ...
    self.pushGlobalBusy = function() {
      let counter = self.globalBusyCounter;
    ...
    self.popGlobalBusy = function() {
      let counter = self.globalBusyStack.pop();
      let i;
      for (i = 0; (i < counter); i++) {
    ...
    self.globalLock = function() {
      ...
      let $freezer = $('<div class="apos-global-busy"></div>');
      $('body').append($freezer);
    ...
    self.globalUnlock = function() {
      let elapsed = Date.now() - self.globalLockAt;
      let $freezer = $('.apos-global-busy');
    ...
    self.enhanceColorpicker = function($el, options) {
      ...
      let colorOptions = _.merge(defaults, options || {});
    ...
    self.ajaxInfiniteScrollHandler = function() {
      let $infiniteScroll = $('[data-apos-ajax-infinite-scroll]');
      ...
      $infiniteScroll.each(function() {
        ...
        let scrollBottom = scrollElement.scrollTop + $(window).height();
    ...
    self.ajaxSubmitHandler = function(event) {
      ...
      let action = $form.attr('action') || (window.location.href.replace(/\?.*$/, ''));
    ...
    self.ajaxGo = function(name, url, data, options) {
      data = data || {};
      let $old = $('[data-apos-ajax-context="' + name + '"]');
      let $appendTo = $old.find('[data-apos-ajax-append]');
      let append = false;
      let hasAposAjax = false;
      let oldTop;
    ...
    self.isAjaxUrl = function(url) {
      ...
      let absoluteNew = $anchor[0].href.replace(/[?#].*$/, '');
      let absoluteOld = window.location.href.replace(/[?#].*$/, '');
    ...
    self.replaceUnpreserved = function($old, $new, options) {
      ...
      function autoPreserveText() {
        let $preserve = $old.find('input[name]:focus,textarea[name]:focus');
      ...
      function removeOrphans() {
        let $alive = find($new, 'preserve');
        let $previous = find($old, 'preserve');
      ...
      function tagAncestors($context) {
        ...
        let $preserve = find($context, 'preserve');
        $preserve.each(function() {
          let $el = $(this);
          let level = 0;
