...
apos.define('apostrophe-ui', {
  ...
  construct: function(self, options) {
    ...
    // infinite scroll.
    //
    // This method is throttled. If many calls are made with the same `name`,
    // only the last is guaranteed to be honored, and requests will not be made
    // simultaneously. This means it is safe to submit the form frequently, for
    // instance in response to keyboard events, without fear of out-of-order
    // results or overwhelming the server.

    self.ajaxGo = function(name, url, data, options) {

      self.ajaxTimeouts = self.ajaxTimeouts || {};
      self.ajaxInProgress = self.ajaxInProgress || {};
      self.ajaxLast = self.ajaxLast || {};

      if (self.ajaxInProgress[name]) {
        later();
        return false;
      }

      var now = Date.now();
      if (self.ajaxLast[name] && (now - self.ajaxLast[name] < 100)) {
        later();
        return false;
      }

      self.ajaxInProgress[name] = true;
      ...
      $.ajax({
        ...
        success: function(html) {
          ...
          self.ajaxInProgress[name] = false;
          self.ajaxLast[name] = Date.now();
        },
        error: function(err) {
          self.ajaxError(err);
          self.ajaxInProgress[name] = false;
      ...
      function later() {
        if (self.ajaxTimeouts[name]) {
          clearTimeout(self.ajaxTimeouts[name]);
        }
        self.ajaxTimeouts[name] = setTimeout(function() {
          self.ajaxGo(name, url, data, options);
        }, 100);
      }
