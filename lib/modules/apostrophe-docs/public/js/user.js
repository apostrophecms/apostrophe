apos.define('apostrophe-docs', {
  ...
  construct: function(self, options) {
    ...
    // Obtain a session lock on the given doc _id, interacting with the user
    ...
    // else could know about or have a lock on

    self.lock = function(_id, callback) {
      if (!_id) {
        return callback(null);
      }
      return $.jsonCall(self.action + '/session-lock',
        { 
          _id: _id
        },
        function(result) {
          if (result.status === 'ok') {
            ...
            return self.api(self.action + '/session-lock',
              {
                force: true,
                _id: _id
              },
    ...
    // current user does not have a lock anymore after
    // the call, whether they initially had one or not
    
    self.unlock = function(_id, callback) {
      return $.jsonCall(self.action + '/session-unlock',
        { 
          _id: _id
        },
        function(result) {
