...
module.exports = function(self, options) {
  ...
  self.route('post', 'chooser-choices', function(req, res) {
    ...
    if (field.idsField) {
      ...
      if (field.relationshipsField) {
        ...
        _.each(choices, function(choice) {
          input.relationshipsField[choice.value] = _.omit(choice, 'value', 'label');
        });
      }
    } else {
    ...
    return async.series({
      ...
      return res.send(self.render(req, 'chooserChoices', { choices: receptacle[field.name], outputChooserChoice: manager.outputChooserChoice } ));
    });
  });
