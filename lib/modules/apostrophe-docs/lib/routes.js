...
module.exports = function(self, options) {
  ...
  self.route('post', 'polymorphic-chooser-modal', function(req, res) {
    var typeNames = self.apos.launder.strings(req.body.types);
    var types = _.map(typeNames, function(name) {
      return self.apos.docs.getManager(name);
    });
    return res.send(self.render(req, 'polymorphicChooserModal', { options: self.options, types: types }));
  });
