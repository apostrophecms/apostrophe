  ...
  self.apiRoute('post', 'slug-taken', function(req, res, next) {
    ...
    return self.find(req, criteria).permission(false).trash(null).published(null).projection({ _url: 1, title: 1 }).toObject().then(function(doc) {
      if (doc) {
        ...
        var hint = self.apos.templates.i18n(req, '__ns', 'apostrophe', 'The slug you want to use is currently claimed by the %s %s. Do you want to edit that document in order to change its slug?', self.apos.templates.i18n(req, '__ns', 'apostrophe', label), doc.title);
