...
module.exports = function(self, options) {
  ...
  // Lock the given doc id to the current user's session id, such
  // that other calls to sessionLock for that doc id will
  // fail unless they have the same session id. If
  ...
  self.sessionLock = function(req, id, options, callback) {
    if (!callback) {
      callback = options;
      options = {};
    }
    ...
    if (!options.force) {
      criteria.$or = [
        {
          sessionLock: { $exists: 0 }
        },
        {
          'sessionLock.id': req.sessionID
        }
      ];
    }
  ...
  // Release a document lock set via `sessionLock`.
  
  self.sessionUnlock = function(req, id, callback) {
    if (!id) {
      return callback('no id');
    }
    if (!req.sessionID) {
