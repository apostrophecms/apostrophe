...
module.exports = function(self, options) {
  ...
  self.docBeforeSave = function(req, doc) {
    ...
    if (doc.title) {
      doc.sortTitle = self.apos.utils.sortify(doc.title);
    } else {
      doc.sortTitle = '';
    }
  ...
  // This method provides the back end of the /autocomplete route.
  // For the implementation of the autocomplete() filter see autocomplete.js.
  //
  // "query" must contain a "type" property which is the type of doc
  // we wish to autocomplete.
  //
  // "query" must contain either a "term" property, which is a partial
  // string to be autocompleted, or a "values" property, which is an array
  // of IDs of docs. If neither is provided an empty array is the
  // result delivered to the callback

  self.autocomplete = function(req, query, callback) {
    var criteria = {};
    var type = self.apos.launder.string(query.type);
    ...
    if (query.term !== undefined) {
      ...
      cursor.and({ _id: { $in: self.apos.launder.ids(query.values) } });
    } else {
      return callback(null, []);
    }
    ...
    manager.addExtraAutocompleteCriteria(cursor, query);
  ...
  // Invoked by the apostrophe-doc-versions module.
  // Remove properties that are unsuitable for versioning
  // because they are part of a shared resource
  // (slug collisions are an issue, and doc permissions aren't
  // something the average editor is allowed to roll back
  // necessarily)

  self.docVersionPrune = function(req, version) {
    // Don't let people roll back permissions changes
    delete version.slug;
    delete version.docPermissions;
  };
