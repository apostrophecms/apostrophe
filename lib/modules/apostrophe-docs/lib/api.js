      ...
      if (!result.result.nModified) {
        return self.db.findOne({ _id: id }, { advisoryLock: 1 }, function(err, info) {
          ...
          if (info.advisoryLock.username === req.user.username) {
            return callback('locked-by-me', self.apos.templates.i18n(req, '__ns', 'apostrophe', 'You may be editing that document in another tab or window.\nIf you take control, you could lose work in that tab or window.\nDo you want to take control?'));
          }
          return callback('locked', self.apos.templates.i18n(req, '__ns', 'apostrophe', 
        ...
        function(err, info) {
          ...
          if ((!info.advisoryLock) || (info.advisoryLock._id !== contextId)) {
            if (info.advisoryLock && info.advisoryLock.username === req.user.username) {
              message = self.apos.templates.i18n(req, '__ns', 'apostrophe', 'You are now editing this document in another tab or window.');
              return callback('locked-by-me');
            }
            message = self.apos.templates.i18n(req, '__ns', 'apostrophe', 'Control of this document has been taken by another user.');
