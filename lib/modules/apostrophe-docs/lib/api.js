...
module.exports = function(self, options) {
  ...
  //
  // This method is part of the implementation of the conventional
  // trash can and does not come into play when workflow is active.
  // In that situation `trash` is an ordinary schema field.
    ...
    return async.series({
      find: function(callback) {
        return self.find(req, criteria)
          .published(null)
          .permission((options.permission !== undefined) ? options.permission : self.getTrashPermissionName())
  ...
  self.getTrashPermissionName = function() {
  ...
  self.testInsertPermissions = function(req, doc, options) {
    if (!(options.permissions === false)) {
      if (!self.apos.permissions.can(req, self.getInsertPermissionName(doc), null, doc)) {
        throw new Error('forbidden');
      }
    }
  };

  self.getInsertPermissionName = function(doc) {
    if (self.apos.permissions.extended) {
      return 'insert-' + doc.type;
    } else {
      return 'edit-' + doc.type;
    }
  };
  ...
  self.docBeforeUpdate = function(req, doc, options) {
    if (options.permissions !== false) {
      if (!self.apos.permissions.can(req, self.getUpdatePermissionName(doc), doc)) {
        throw new Error('forbidden');
      }
    }
  };

  self.getUpdatePermissionName = function(doc) {
    return 'updateany-' + doc.type;
  };
      ...
      if (!result.result.nModified) {
        return self.db.findOne({ _id: id }, { advisoryLock: 1 }, function(err, info) {
          ...
          return callback('locked', self.apos.i18n.__(
            ...
            info.advisoryLock.title, info.advisoryLock.username, ago
          ));
        });
      }
      return callback(null);
      ...
      function(err, info) {
        ...
        if ((!info.advisoryLock) || (info.advisoryLock._id !== contextId)) {
          if (info.advisoryLock && info.advisoryLock.username === req.user.username) {
            return callback('locked-by-me', self.apos.i18n.__('You are now editing this document in another tab or window.'));
          }
          return callback('locked', self.apos.i18n.__('Control of this document has been taken by another user, tab or window.'));
        }
        return callback(null);
      }
    );
