...
module.exports = function(self, options) {
  ...
  self.enableCollection = async function(callback) {
    self.db = await self.apos.db.collection('aposDocs');
  };

  self.ensureSlugIndex = async function() {
    const params = self.getSlugIndexParams();
    return await self.db.createIndex(params, { unique: true }, callback);
  };
  ...
  self.ensureTextIndex = async function() {
    return await self.db.ensureIndex({
      highSearchText: 'text', 
      lowSearchText: 'text', 
      title: 'text', 
      searchBoost: 'text' 
  ...
  self.ensurePathLevelIndex = async function(callback) {
    const params = self.getPathLevelIndexParams();
    return await self.db.ensureIndex(params, {});
  };
  ...
  // unique it is made unique. `apostrophe-docs:beforeInsert`,
  // `apostrophe-docs:beforeSave`, `apostrophe-docs:AfterInsert`
  // and `apostrophe-docs:afterSave` events are emitted and
  // awaited. They receive `(req, doc, options)`.
  //
  ...
  self.insert = async function(req, doc, options) {
    options = options || {};
    await self.emit('beforeInsert', req, doc, options);
    await self.emit('beforeSave', req, doc, options);
    await self.denormalizePermissions(req, doc, options);
    await self.insertBody(req, doc, options);
    await self.emit('afterInsert', req, doc, options);
    await self.emit('afterSave', req, doc, options);
    await self.emit('afterLoad', req, [ doc ]);
    return doc;
  };
  ...
  // `afterUpdate` and `afterSave` events are emitted by
  // this module.
  //
  ...
  self.update = async function(req, doc, options) {
    options = options || {};
    await self.emit('beforeUpdate', req, doc, options);
    await self.emit('beforeSave', req, doc, options);
    await self.denormalizePermissions(req, doc, options);
    await self.updateBody(req, doc, options);
    await self.emit('afterUpdate', req, doc, options);
    await self.emit('afterSave', req, doc, options);
    await self.emit('afterLoad', req, [ doc ]);
    return doc;
  };
  ...
  self.denormalizePermissions = async function(req, doc, options) {
    ...
    return await self.emit('afterDenormalizePermissions', req, doc, options);
  };
  ...
  // edit the document.
  //
  // The module emits the `beforeTrash` and `afterTrash` events
  // with `(req, doc)`.
  //
  ...
  self.trash = async function(req, idOrCriteria, options) {
    ...
    const doc = await self.find(req, criteria)
      ...
      .toObject();
    await self.emit('beforeTrash', req, doc);
    await self.trashBody(req, doc);
    await self.emit('afterTrash', req, doc);
  };
  ...
  self.trashBody = async function(req, doc) {
    doc.trash = true;
    return await self.db.updateOne({
      _id: doc._id
    }, {
      $set: {
        trash: true
  ...
  self.rescue = async function(req, idOrCriteria, options) {
    ...
    const doc = await self.find(req, criteria)
      ...
      .permission((options.permission !== undefined) ? options.permission : 'edit-doc')
      .toObject();
    await self.emit('beforeRescue', req, doc);
    await self.rescueBody(req, doc);
    await self.emit('afterRescue', req, doc);
  };
  ...
  self.rescueBody = async function(req, doc) {
    delete doc.trash;
    return await self.db.updateOne({ _id: doc._id }, { $unset: { trash: 1 } });
  };
  ...
  self.deleteFromTrash = async function(req, idOrCriteria) {
    ...
    return await self.db.remove(criteria);
  };
  ...
  self.on('beforeInsert', 'testPermissionsAndAddIdAndCreatedAt', function(req, doc, options) {
  ...
  self.on('beforeSave', 'ensureSlugSortifyAndUpdatedAt', function(req, doc, options) {
    self.ensureSlug(doc);
  ...
  self.on('beforeUpdate', 'checkPermissionsBeforeUpdate', async function(req, doc, options) {
    if (options.permissions !== false) {
  ...
  self.updateBody = async function(req, doc, options) {
    return await self.retryUntilUnique(req, doc, async function() {
      return await self.db.updateOne({ _id: doc._id }, self.apos.utils.clonePermanent(doc));
    });
  };
  ...
  self.insertBody = async function(req, doc, options) {
    return await self.retryUntilUnique(req, doc, async function() {
      return await .db.insert(self.apos.utils.clonePermanent(doc));
    });
  };
  ...
  self.lock = async function(req, id, contextId, options) {
    ...
    if (!result.result.nModified) {
      ...
      var ago = Math.ceil((now - info.advisoryLock.updatedAt.getTime()) / 1000 / 60);
