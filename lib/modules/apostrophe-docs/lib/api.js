...
module.exports = function(self, options) {
  ...
  // Lock the given doc id to a given `req.htmlPageId`, such
  // that other calls to `htmlPageLock` for that doc id will
  // fail unless they have the same `req.htmlPageId`. If
  ...
  //
  // `req.htmlPageId` is like a session id, except that
  // it is specific to an HTML page during its lifetime
  // in a particular browser. Every time you navigate
  // between pages you get a new, unique `htmlPageId`.
  // It is passed as a header in every request
  // during the lifetime of that page.
  
  self.htmlPageLock = function(req, id, options, callback) {
    if (!callback) {
      callback = options;
      options = {};
    }
    ...
    if (!options.force) {
      criteria.$or = [
        {
          htmlPageLock: { $exists: 0 }
        },
        {
          'htmlPageLock.id': req.htmlPageId
        }
      ];
    }
    ...
    return self.db.update(criteria, {
      ...
      if (!modified) {
        // This can't really distinguish between a doc not found
        // and a doc already locked, but Apostrophe never
        // deletes docs, so that shouldn't be an issue
        return callback('locked');
      }
      // Is the doc nonexistent or 
      return callback(null);
    });
  };

  // Release a document lock set via `htmlPageLock`.
  
  self.htmlPageUnlock = function(req, id, callback) {
    if (!id) {
      return callback('no id');
    }
    if (!req.htmlPageId) {
  ...
  self.htmlPageUnlockAll = function(req, callback) {
    if (!req.htmlPageId) {
      return callback('no htmlPageId');
    }
    return self.db.update({
