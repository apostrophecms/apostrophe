...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.lowLevelMongoCursor = function(req, criteria, projection, options) {
      ...
      if (projection) {
        // MongoDB prior to 4.4 allows both "workflowLastCommitted" and "workflowLastCommitted.1" to
        // appear in the projection, provide bc for all the code that relied on that. If we have both
        // we only need the larger one

        // A shallow clone is safe enough because dot notation projection keys always have simple values
        projection = Object.assign({}, projection);
        for (const key of Object.keys(projection)) {
          const subkeys = key.split('.');
          // we are not interested in the last one because we would be comparing the key to itself
          subkeys.pop();
          let parentkey = '';
          for (const subkey of subkeys) {
            ...
            parentkey += subkey;
            if (_.has(projection, parentkey)) {
              // dropping redundant projection of key in favor of parentkey
