...
module.exports = function(self, options) {
  ...
  _.defaults(cursor, {
    ...
    toMongo: function(callback) {
      ...
      return async.waterfall([
        ...
        function(callback) {
          ...
          mongo.skip(cursor.get('skip'));
          mongo.limit(cursor.get('limit'));
          mongo.sort(cursor.get('sort'));
    ...
    finalize: function(callback) {
      ...
      cursor.finalizeSort();
      cursor.finalizePublished();
      cursor.finalizeTrash();
      cursor.finalizeOrphan();
      return async.series({
    ...
    finalizePublished: function() {
      ...
      if (published === null) {
        console.error('I think it is null');
        return;
      }
      if (published || (published === undefined)) {
        console.error('I think it is ' + published);
        cursor.and({
          published: true
        });
        return;
      }
      console.error('I think it is false');
      cursor.and({
        published: { $ne: true }
      });
      console.log(cursor.get('criteria'));
    },
