...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    //
    // `finalize` will be awaited. So will `after`.
    //
    // If `finalize` returns the string `refinalize`,
    // the entire series of finalizers is invoked
    ...
    self.addFilter('joins', {
      ...
      after: function(results, callback) {
        ...
        return async.eachSeries(_.keys(resultsByType), function(type, callback) {
          ...
          return self.apos.schemas.join(self.get('req'), manager.schema, self.get('joins'), resultsByType[type]);
        }, callback);
      }
    });
