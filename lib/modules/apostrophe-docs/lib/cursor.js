...
//
// ```
// return await apos.docs.find(req).
//   and({ age: { $gte: 30 } }).
//   published(true).
//   toArray();
// ```
...
//
// ```
// return await apos.docs.getManager('apostrophe-blog').
//   find().
//   limit(10).
//   toArray();
// ```
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    if (self.__meta.name === 'apostrophe-users-cursor') {
      console.trace('users cursor created');
    }
    self.state = {};
    self.filters = {};
    self.afters = {};
    ...
    // object which can  have `set`, `prefinalize`, `finalize`
    // and `after` functions and a default value, `def`. You provide
    ...
    // by `self.get('nameOfFilter')`.
    //
    // `finalize` defaults to doing nothing. Usually your finalizer
    ...
    // up so far.
    //
    // `prefinalize` is used in edge cases where
    // some modification of the criteria is needed
    // before the finalizers are invoked. For instance,
    // `autocomplete` uses it to set up `search`
    // after performing an initial query for
    // complete words.
    //
    // If the filter has still not been called at
    // prefinalization time, the filter is called
    // with `def` before prefinalizers and finalizers
    // are invoked.
    //
    // `prefinalize`, `finalize` and `after` may be
    ...
    self.addFilter = function(name, definition) {
      ...
      const set = definition.set || function(value) {
        self.set(name, value);
      };
    ...
    self.addFilter('addLateCriteria', {
      set: function(c) {
        let lateCriteria = self.get('lateCriteria');
        if (!lateCriteria) {
    ...
    self.addFilter('projection', {
      finalize: function() {
        let projection = self.get('projection');
        if (!projection) {
          projection = {};
        }
    ...
    self.projectUrlField = function(add) {
      ...
      let manager = type && self.apos.docs.getManager(type);
      let fields;
      if (manager) {
    ...
    self.projectJoinField = function(key, add) {
      ...
      const manager = type && self.apos.docs.getManager(type);
      let schema;
      let field;
      if (manager) {
    ...
    self.addFilter('permission', {
      finalize: function() {
        let typeSuffix = '-' + (self.get('type') || 'doc');
        let permission = self.get('permission');
        if (permission !== false) {
          if (permission && (!permission.match(/-/))) {
            permission = permission + typeSuffix;
          }
          self.and(self.apos.permissions.criteria(self.req, permission || ('view-' + typeSuffix)));
        }
      },
      after: function(results) {
        // In all cases we mark the docs with ._edit if
        // the req is permitted to do that
        self.apos.permissions.annotate(self.req, 'edit-doc', results);
      }
    });
    ...
    self.addFilter('autocomplete', {
      prefinalize: require('./autocomplete.js')(self),
      safeFor: 'public',
      launder: function(s) {
    ...
    self.addFilter('published', {
      def: true,
      finalize: function() {
        const published = self.get('published');
        if (published === null) {
          return;
        }
        if (published) {
    ...
    self.addFilter('explicitOrder', {
      ...
      finalize: function() {
        ...
        const criteria = {};
        const values = self.get('explicitOrder');
        const property = self.get('explicitOrderProperty');
        if (!values.length) {
      ...
      after: function(results) {
        ...
        const property = self.get('explicitOrderProperty');
        const temp = self.apos.utils.orderById(values, results, property);
        let i;
        // Must modify array in place
        for (i = 0; (i < temp.length); i++) {
          results[i] = temp[i];
        }
        const skip = self.get('explicitOrderSkip');
        const limit = self.get('explicitOrderLimit');
      ...
      after: async function(results) {
        ...
        for (let type of _.keys(resultsByType)) {
          ...
          if (manager && manager.schema) {
            console.log('XXXX', resultsByType[type]);
            await self.apos.schemas.join(self.req, manager.schema, resultsByType[type], self.get('joins'));
          }
        }
      }
    });
    ...
    //
    // The method receives `(req, docs)`. All of the docs will be of
    ...
    self.addFilter('addUrls', {
      ...
      after: async function(results) {
        const req = self.req;
        const val = self.get('addUrls');
        if (!val) {
          return;
        }
        const byType = _.groupBy(results, 'type');
        const interesting = _.filter(_.keys(byType), function(type) {
        ...
        for (let type of interesting) {
          await self.apos.docs.getManager(type).addUrls(req, byType[type]);
        }
      }
    });
    ...
    //
    // If `distinctCounts(true)` has been called on this cursor,
    // you can obtain counts for each unique value via
    // `self.get('distinctCounts')` after this method returns.
    ...
    self.toDistinct = async function(property) {
      ...
      async function distinctCounts() {
        ...
        const results = await self.db.aggregate(pipeline);
        const counts = {};
        _.each(results, function(doc) {
          counts[doc._id] = doc.count;
        });
        self.set('distinctCounts', counts);
        return _.map(results, '_id');
      }
    };
    ...
    //
    // However, if there is no choices function or no filter at all,
    ...
    self.toChoices = async function(property, options) {
      options = options || {};
      if (options.counts) {
        self.distinctCounts(true);
      }
      let choicesFn;
      const filter = self.filters[property];
      if (filter) {
        if (filter.choices) {
    ...
    self.toObject = async function() {
      let limit = self.get('limit');
      self.set('limit', 1);
      const result = (await self.toArray())[0];
    ...
    self.toArray = async function() {
      console.log('> > > before toMongo', self.req.params);
      const mongo = await self.toMongo();
      console.log('> > > before mongoToArray', self.req.params);
      const docs = await self.mongoToArray(mongo);
      console.log('> > > before after docs', self.req.params);
      await self.after(docs);
      console.log('> > > after all the things', self.req.params);
      return docs;
    };

    // Returns a MongoDB cursor. You can use this
    ...
    self.toMongo = async function() {
      ...
      return self.lowLevelMongoCursor(self.req, self.get('criteria'), self.get('projection'), {
        skip: self.get('skip'),
    ...
    self.lowLevelMongoCursor = function(req, criteria, projection, options) {
      console.log('LLMC args: ', require('util').inspect(criteria, { depth: 10 }), projection, options);
      const mongo = self.db.find(criteria).project(projection);
    ...
    self.clone = function() {
      const cursor = self.apos.createSync(self.__meta.name, options);
    ...
    //
    // If the cursor has already been finalized, nothing happens.
    //
    // First, defaults are applied. Then,
    // the `refinalize` function of each filter
    // that has one is awaited. Next, the `finalize`
    // function of each filter that has one is awaited.
    ...
    self.finalize = async function() {
      ...
      const names = Object.keys(self.filters);
      for (let name of names) {
        if (self.get(name) === undefined) {
          const filter = self.filters[name];
          self.set(name, filter.def);
        }
      }
      for (let name of names) {
        const filter = self.filters[name];
        if (filter.prefinalize) {
          await filter.prefinalize();
        }
      }
      for (let name of names) {
        const filter = self.filters[name];
        if (filter.finalize) {
          await filter.finalize();
        }
      }
    ...
    self.mongoToArray = async function(mongo) {
      ...
      console.log(id, self.req.url, self.req.params);
      const result = await mongo.toArray();
      console.log(id, self.req.url, self.req.params);
      return result;
    };
    ...
    self.after = async function(results) {
      // Since "afters," in some cases, invoke filters, we have to
      // set self._finalized to false so these invocations don't
      // throw an exception.
      let originalFinalized = self._finalized;
      self._finalized = false;
      for (let name of _.keys(self.filters)) {
        const filter = self.filters[name];
        if (filter.after) {
          await filter.after(results);
        }
      }
    ...
    self.finalizeSort = function() {
      ...
      if (sort) {
        ...
        let manager = self.apos.docs.getManager(self.get('type') || 'apostrophe-page');
        if (!(manager && manager.schema)) {
          return;
        }
    ...
    self.finalizeDefaultSort = function() {
      let sort = self.get('defaultSort') || { title: 1 };
      if ((!self.get('search')) && (sort === 'search')) {
        // A search is not present, and yet `{ sort: 'search' }`
