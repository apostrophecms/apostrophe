...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // previous doc based on the sort.
    //
    ...
    self.nextOrPrevious = function(verb) {
      ...
      _.each(sort, function(val, key) {
        if (val === 1) {
          var clause = _.clone(leftHand);
          if (direction === 1) {
            clause[key] = { $gte: doc[key] };
          } else {
            clause[key] = { $lte: doc[key] };
          }
