  ...
  construct: function(self, options) {
      ...
      finalize: function() {
        ...
        if (permission !== false) {
          self.set('criteria', {
            $and: [
              self.get('criteria') || {},
              self.apos.permissions.criteria(self.get('req'), permission || 'view-page')
            ]
          });
        }
      }
    ...
    self.toArray = function(callback) {
      return async.waterfall([
        self.toMongo,
        self.mongoToArray,
        self.markPermissionsAndLoadWidgets,
        self.loadDocsEvent,
      ], callback);
    };
    ...
    self.markPermissionsAndLoadWidgets = function(results, callback) {
      ...
      return async.eachSeries(results, function(doc, callback) {
        return setImmediate(function() {
          return self.docs.loadWidgets(req, doc, callback);
        });
      }, function(err) {
    ...
    self.loadDocsEvent = function(results, callback) {
      var callbacks = [];
      self.apos.emit('loadDocs', self, results, callbacks);
