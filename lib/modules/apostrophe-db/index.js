...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    //
    // Takes a collection as an argument because this ensures we patch
    // the prototype actually being used for collections — since a
    // database connection can be injected into apostrophe, we cannot assume
    // the driver we obtained via `require` is actually in play.

    self.bcPatch = function() {
      // Don't use self.keepalive because someone clever may have thought
      // they could disable that. Do reuse that collection name though as
      // it's perfect when we just need a collection object for the sake of
      // patching its prototype.
