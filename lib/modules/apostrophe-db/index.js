...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.connectToMongo = function(callback) {
      ...
      self.apos.db = {
        collection(name, cb) {
          const dbDir = `${self.apos.rootDir}/data/db`;
          if (!fs.existsSync(dbDir)) {
            fs.mkdirSync(dbDir);
          }
          const c = new Nedb({filename: `${dbDir}/${name}.db`, autoload: true });
          const superFind = c.find.bind(c);
          c.find = (...rest) => {
            const cursor = superFind(...rest);          
            cursor.__criteria = rest[0];
            cursor.count = (...rest) => {
              return c.count.apply(c, [ cursor.__criteria, ...rest ]);
            };
            cursor.toArray = (...rest) => {
              return cursor.exec(...rest);
            };
            cursor.nextObject = (cb) => {
              if (cursor._fetched) {
                return cb(null, cursor._fetched.shift() || null);
              } else {
                return cursor.exec((e, r) => {
                  if (e) return cb(e);
                  cursor._fetched = r;
                  return cb(null, cursor._fetched.shift() || null);
                });
            ...
            cursor.project = cursor.projection;
            const superExec = cursor._exec.bind(cursor);
            cursor._exec = (...rest) => {
              cursor.project = cursor._project;
              return superExec(...rest);
            };
            promisifyMethods(cursor, 'toArray', 'count', 'nextObject');
            return cursor;
          };
          const superInsert = c.insert.bind(c);
          c.insert = (...rest) => {
            return superInsert(...rest);
          };
          const superEnsureIndex = c.ensureIndex.bind(c);
          c.ensureIndex = (props, options, cb) => {
            ...
            if (keys.length > 1) {
              if (options.unique) {
                console.warn(`⚠️ composite index on ${keys} ignored, but has unique option, so this will probably break things`);
              } else {
                console.warn(`>>> composite index on ${keys} ignored`);
              }
              return cb(null);
            }
            return superEnsureIndex({
              fieldName: keys[0],
              ...(options || {})
            }, cb);
          };
          c.distinct = (prop, criteria, cb) => {
            if ((typeof criteria) === 'function') {
              cb = criteria;
              criteria = {};
            }
            return c.find(criteria).toArray((err, docs) => {
              if (err) {
                return cb(err);
              }
              const d = {};
              ...
              return cb(null, Object.keys(d));
            });
          };
          const superUpdate = c.update.bind(c);
          c.update = (...rest) => {
          ...
          promisifyMethods(c, 'insert', 'update', 'remove', 'ensureIndex', 'distinct');
          c.findWithProjection = c.find;
          if (cb) return cb(null, c);
          return c;
        },
        listCollections() {
          console.error('Our bluff was called, implement listCollections for nedb');
          process.exit(1);
        }
      };
...
// Add optional promise support to methods, like mongo has

function promisifyMethods(object, ...methods) {
  for (const method of methods) {
    const _super = object[method].bind(object);
    object[method] = (...rest) => {
      ...
      return new Promise((resolve, reject) => {
        return _super(...(rest.concat([ function(err, result) {
          if (err) {
            return reject(err);
          } else {
