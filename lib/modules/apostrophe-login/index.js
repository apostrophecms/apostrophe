  ...
  construct: function(self, options) {
                ...
                return self.verifyLogin(req.body.username, password, function(err, user) {
                  ...
                  return Promise.try(function() {
                    ...
                    return res.redirect('/password-reset?' + qs.stringify({
                      reset: reset,
                      errors: [ self.apos.templates.i18n(req, '__ns', 'apostrophe', 'Your password must be changed for the following reasons:') ].concat(errors)
                    }));
                  }).catch(function(err) {
                    self.apos.utils.error(err);
                    return fail(errors);
      ...
      if ((self.options.localLogin !== false) && (self.options.passwordReset || self.options.resetLegacyPassword)) {
        ...
        self.apos.app.get('/password-reset', function(req, res) {
          ...
          return self.apos.users.find(adminReq, criteria).toObject().then(function(user) {
            ...
            self.apos.utils.error(err);
            return res.redirect(`${self.getLoginUrl()}?message=${self.apos.templates.i18n(req, '__ns', 'apostrophe', 'That reset code was not found. It may have expired. Try resetting again.')}`);
          });
        ...
        self.apos.app.post('/password-reset', function(req, res) {
          ...
          return Promise.try(function() {
            ...
            return res.redirect(`${self.getLoginUrl()}?message=${self.apos.templates.i18n(req, '__ns', 'apostrophe', 'Your password has been reset. Please log in.')}`);
          }).catch(function(err) {
            self.apos.utils.error(err);
    ...
    self.sendPasswordResetEmail = function(req, user) {
      ...
      return Promise.try(function() {
        ...
        return self.email(req, 'passwordResetEmail', { user: user, url: url, site: site }, {
          to: user.email,
          subject: self.apos.templates.i18n(req, '__ns', 'apostrophe', self.options.passwordResetSubject || ('Your request to reset your password on ' + site))
        });
      });
    };
    ...
    self.checkPasswordRules = function(req, password) {
      ...
      errors = errors.map(function(error) {
        return self.apos.templates.i18n(req, '__ns', 'apostrophe', error);
      });
      return errors;
    };
