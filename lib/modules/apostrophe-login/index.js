    ...
    self.addRoutes = function() {
      ...
      if ((self.options.localLogin !== false) && self.options.passwordReset) {
        ...
        self.apos.app.get('/password-reset-request', function(req, res) {
          ...
          // before certain fixes would expect (this is why we still pass a string and not
          // a flag, and why we call it `message`)
          return self.sendPage(req, 'passwordResetRequest', { message: req.query.error ? 'that is not a valid email address or username' : undefined });
        });
        ...
        self.apos.app.post('/password-reset-request', function(req, res) {
          ...
          return self.apos.users.find(req, {
            ...
            return res.redirect('/login?message=' + req.__('An email message has been sent to you with instructions to reset your password. Be sure to check your spam folder if you do not see it in the next few minutes.'));
          }).catch(function(err) {
        ...
        self.apos.app.get('/password-reset', function(req, res) {
          ...
          return self.apos.users.find(req, {
            passwordReset: reset,
            passwordResetAt: { $gte: new Date(Date.now() - 86400 * 14) }
          }).toObject().then(function(user) {
            if (!user) { 
              throw 'notfound';
            }
        ...
        self.apos.app.post('/password-reset', function(req, res) {
          ...
          return self.apos.users.find(req, {
            passwordReset: reset,
            passwordResetAt: { $gte: new Date(Date.now() - 1000 * 60 * 60 * (self.options.passwordResetHours || 48)) }
          }).toObject().then(function(user) {
            if (!user) {
              throw new Error('notfound');
            }
            user.password = password;
            return self.apos.users.update(self.apos.tasks.getReq(), user);
          }).then(function(user) {
