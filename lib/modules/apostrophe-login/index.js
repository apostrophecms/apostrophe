...
module.exports = {
  ...
  construct: function(self, options){
    // Ensure our own instance of Passport. In unit tests we make many instances
    // of Apostrophe which must be independent, and it's a good code smell anyway
    // to be clean in this way. -Tom
    ...
    self.enableLocalStrategy = function(){
      self.passport.use(new LocalStrategy(function(username, password, done){
        var req = self.apos.tasks.getReq();
        self.apos.users.find(req, {username: username}).toObject(function(err, user){
          if(err){
            return done(err);
          }
          if(!user){
