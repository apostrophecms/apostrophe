...
// We use a rate limit on the POST /login-totp route to protect against brute force,
// You can modify the rate limit parameters in the configuration object as done below.
//
// If this option is set to an object, you may specify sub-options:
//
// `totp: { groups: true, rateLimitMaxRequests: 100, rateLimitMinutes: 10 }`
//
...
module.exports = {
  ...
  construct: function(self, { throttle, totp }) {
    ...
    self.requireTotp = function(req, res, next) {
      ...
      var safelist = [ '/login-totp', '/setup-totp', '/confirm-totp', '/login', '/logout', '/modules/apostrophe-areas/editor' ];
      // const regex = /^\/modules\/apostrophe-.+/;

      if (
        _.contains(safelist, req.path)
        // || regex.test(req.path)
      ) {
        return next();
      }
