    ...
    self.addRoutes = function() {
      ...
      if (self.options.totp) {
        ...
        self.apos.app.get('/setup-totp', function(req, res) {
          ...
          if (req.user.totp && req.user.totp.confirmed) {
            return res.redirect('/login-totp');
          }

          var encodedKey;
          var period = (req.user.totp && req.user.totp.period) || 30;
          var otpUrl;
          var qrImage;

          var key = (req.user.totp && req.user.totp.key) || self.randomKey(10);
        ...
        self.apos.app.post('/confirm-totp', function(req, res) {
          if (!req.user) {
            return res.redirect('/login');
          }
          if (!req.user.totp) {
