  ...
  construct: function(self, options) {
    ...
    self.verifyLogin = function(username, password, callback) {
      ...
      return self.apos.users.find(req, {
        ...
        return self.apos.users.verifyPassword(user, password, function(err) {
          ...
          console.log('=============> HERE <================')
          return self.disableIfInactive(user).then(function(user) {
            console.log('=============> user <================')
    ...
    self.verifyTotp = function(user, done) {
      return self.apos.users.safe.findOne({
       ...
       /*  return self.disableIfInactive(
          userSafe,
          done,
          {
            key: userSafe && userSafe.totp && userSafe.totp.key,
            period: userSafe && userSafe.totp && userSafe.totp.period
          }
        ); */
        done(null, userSafe && userSafe.totp && userSafe.totp.key, userSafe && userSafe.totp && userSafe.totp.period);
      }).catch(function(err) {
        return done(err);
      });
    };
      ...
      if (self.apos.users.options.disableInactiveAccounts) {
        ...
        if (user.lastLogin) {
          ...
          if (!isUserNeverDisabled) {
            ...
            var now = moment();
            var diff = lastLogin.diff(now, 'days');
            if (diff >= usersOptions.inactivityDuration) {
              console.log('=============> inactivity <================')
              return self.apos.docs.db.update({ _id: user._id }, { $set: { disabled: true, trash: false, lastLogin: '' } })
                .then(function() {
                  // return callback(new Error('Account disabled due to inactivity. Please, refer to the administrator of the site for assistance.'), false);
                  return new Error('Account disabled due to inactivity. Please, refer to the administrator of the site for assistance.');
                }).catch(function(err) {
...
/*       if (totp && totp.key && totp.period) {
        return callback(null, totp.key, totp.period);
      }
      return callback(null, user); */
    };
