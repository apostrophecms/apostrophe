...
module.exports = {
  ...
  construct: function(self, { totp }) {
    ...
    self.addRoutes = function() {
      ...
      if (totp) {
        const totpApiLimiter = expressRateLimit({
          windowMs: (totp.rateLimitMaxRequests || 15) * 60 * 1000, // 15 minutes
          max: totp.rateLimitMinutes || 100,
          handler: (req, res) => {
            // self.apos.notify('Too many requests, please retry later.', { type: 'error' });
            return self.sendPage(req, 'login-totp', { error: 'Too many requests' });
          }
        });
        ...
        self.apos.app.post('/login-totp', totpApiLimiter, function(req, res, next) {
          console.log('=============> IN LOGIN TOTP POST <================');
