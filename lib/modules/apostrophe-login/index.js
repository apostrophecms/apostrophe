...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Check if the first character about the option `loginUrl` is `/`
    // and if is not present, add the `/` in the first position.
    // If the option is not present, return the default url `/login`
    self.fetchLoginUrl = function() {
      if (self.options.loginUrl) {
        return self.options.loginUrl && self.options.loginUrl.startsWith('/') ? self.options.loginUrl : `/${self.options.loginUrl}`
      }
      else {
        return '/login'
      }
    ...
    self.addRoutes = function() {
      if (self.options.localLogin !== false) {
        self.apos.app.get(self.fetchLoginUrl(), function(req, res) {
          if (req.user) {
