...
module.exports = {
  ...
  construct: function(self, { throttle, totp }) {
    const { allowedAttempts = 3, perMinutes = 1, lockoutMinutes = 1 } = throttle || {};
    ...
    // Check that user didn't already exceed the throttle limit before continuing
    self.checkThrottleAttempts = async function (user) {
      const { safe } = self.apos.users;
    ...
    // In case of login error, add an attempt to the user,
    // and check if the user must be locked out depending of attempts number
    self.addThrottleAttempt = async function (user, error) {
      const { safe } = self.apos.users;

      await addAttempt();

      const lockout = await lockoutIfNeeded();

      await self.cleanUpThrottleAttempts(user, true);

      return lockout ? 'throttle' : error;
      ...
      async function lockoutIfNeeded() {
        ...
        const unallowedAttemps = userSafe.attempts && userSafe.attempts
          .filter(attempt => (attempt >= perSince)).length >= allowedAttempts;
    ...
    // Clean user attempts, if it happens during a login fail we only remove the old attempts
    // when the login succeed, we remove all attempts.
    self.cleanUpThrottleAttempts = async function(user, loginFailed = false) {
      const { safe } = self.apos.users;

      const updateOptions = loginFailed ? {
        $pull: {
          attempts: {
            $lt: getPerSince()
          }
    ...
    self.verifyPassword = function(user, password, callback) {
      ...
      async function body() {
        await self.checkThrottleAttempts(user);
        await attempt();
      }
      ...
      async function attempt() {
        ...
        try {
          ...
          await self.cleanUpThrottleAttempts(user);
        } catch (e) {
          const err = await self.addThrottleAttempt(user, e);
    ...
    self.verifyTotp = function(user, done) {
      ...
      return self.apos.users.safe.findOne({
        ...
        return self.checkIfActive(userSafe, function(err) {
          ...
          return done(
            null,
            userSafe && userSafe.totp && userSafe.totp.key,
            userSafe && userSafe.totp && userSafe.totp.period
          );
        });
      }).catch(function(err) {
        return done(err);
    ...
    self.requireTotp = function(req, res, next) {
      ...
      if (_.contains(safelist, req.path)) {
        return next();
      }
            ...
            if (errors.length) {
              if (self.options.resetLegacyPassword) {
                ...
                return self.verifyLogin(req.body.username, password, function(err, user) {
                  if (err || (!user)) {
                    if (err === 'throttle') {
                      errors.unshift(self.getThrottleLoginErr(req, lockoutMinutes));
                    }
                    return fail(errors);
                  }
            ...
            self.passport.authenticate('local', function(err, user, info) {
              if (err) {
                if (err === 'throttle') {
                  ...
                  err = new Error(self.getThrottleLoginErr(req, lockoutMinutes));
                }
                return res.redirect(`${self.getLoginUrl()}?` + qs.stringify({
                  message: err.message,
                  error: '1'
      ...
      if (totp) {
        ...
        self.apos.app.post('/login-totp',
          async function(req, res, next) {
            ...
            try {
              await self.checkThrottleAttempts(req.user);

              return next();
            } catch (err) {
              return res.redirect('/login-totp?' + qs.stringify({
                error: self.getThrottleLoginErr(req, lockoutMinutes)
              }));
            }
          }, async (req, res, next) => {
            self.passport.authenticate('totp', async (err, user, info) => {
              const error = err || (!user && 'That code was not correct.');

              if (error) {
                const errorMsg = await self.addThrottleAttempt(req.user, error);

                return res.redirect('/login-totp?' + qs.stringify({
                  error: errorMsg === 'throttle'
                    ? self.getThrottleLoginErr(req, lockoutMinutes)
                    : errorMsg
                }));
              }
