...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.verifyLogin = function(username, password, callback) {
      ...
      return self.apos.users.find(req, {
        ...
        return self.apos.users.verifyPassword(user, password, function(err) {
          ...
          return self.disableInactiveAccount(err, user, callback);
        });
      });
    };

    self.verifyTotp = function(user, done) {
      self.disableInactiveAccount([], user, done);
    ...
    self.disableInactiveAccount = function(err, user, callback) {
      if (user.lastLogin) {
        ...
        if (diff > 90) {
          var req = self.apos.tasks.getReq();
          var user = Object.assign(user, { disabled: true, trash: false, lastLogin: '' });
