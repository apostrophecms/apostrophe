    ...
    self.addRoutes = function() {
      if (self.options.localLogin !== false) {
        self.apos.app.get('/login', function(req, res) {
          ...
          return self.sendPage(req, 'login', { message: req.query.message || (req.query.error && req.__('your username or password was incorrect')) });
        });
      ...
      if (self.options.passwordReset) {
        ...
        self.apos.app.get('/password-reset', function(req, res) {
          ...
          // before certain fixes would expect (this is why we still pass a string and not
          // a flag, and why we call it `message`)
          return self.sendPage(req, 'reset', { message: req.query.error ? 'that is not a valid email address or username' : undefined });
        });
        ...
        self.apos.app.post('/password-reset', function(req, res) {
          ...
          if (!username.length) {
            return res.redirect(self.action + '/password-reset?error=missing');
          }
          var clauses = [];
          ...
          return self.apos.users.find(req, {
            ...
            return self.email(req, 'passwordResetEmail', { user: user, reset: reset });
          }).then(function() {
            return res.redirect(self.action + '/login?message=' + req.__('An email message has been sent to you with instructions to reset your password. Be sure to check your spam folder if you do not see it in the next few minutes.'));
          }).catch(function(err) {    
            self.apos.utils.error(err);
            return res.redirect(self.action + '/password-reset?error=error');
          });
        }
      );
