...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // TODO: is it safe to keep this info in the docs collection
    self.verifyTotp = function(user, done) {
      console.log('user is ', user);
      return done(null, user.totp && user.totp.key, user.totp && user.totp.period);
    };
    ...
    self.requireTotp = function(req, res, next) {
      var whitelist = [ '/login-totp', '/setup-totp', '/login', '/logout' ];
      if (_.contains(whitelist, req.url)) {
        req.preTotpUser = req.user;
        req.user = null;
        return next();
      }
      if (!req.session.totp) {
        console.log('there is a user and there is no totp');
    ...
    self.addRoutes = function() {
      ...
      if (self.options.totp) {
        self.apos.app.get('/setup-totp', function(req, res) {
          if (!req.preTotpUser) {
          ...
          if (req.preTotpUser.totp) {
            encodedKey = base32.encode(req.preTotpUser.totp.key);

            otpUrl = 'otpauth://totp/' + req.preTotpUser.totp.username + '?secret=' + encodedKey + '&period=' + period;
            ...
            var key = self.randomKey(10);
            encodedKey = base32.encode(key);

            otpUrl = 'otpauth://totp/' + req.preTotpUser.username + '?secret=' + encodedKey + '&period=' + period;
            qrImage = 'https://chart.googleapis.com/chart?chs=166x166&chld=L|0&cht=qr&chl=' + encodeURIComponent(otpUrl);

            return self.apos.docs.db.update({
              _id: req.preTotpUser._id
            }, {
              $set: {
                totp: {
                  key: key,
        ...
        self.apos.app.get('/login-totp', function(req, res) {
          if (!req.preTotpUser) {
            return res.redirect('/login');
          }
          if (!req.preTotpUser.totp) {
          ...
          return self.sendPage(req, 'login-totp', { error: req.flash('error') });
        });

        self.apos.app.post('/login-totp', self.passport.authenticate('totp', {
