...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.disableIfInactive = function(err, user, callback) {
      if (self.apos.users.options.disableInactiveAccounts) {
        ...
        if (user.lastLogin) {
          ...
          var isUserWhitelisted = user._groups.some(function(group) {
            return usersOptions.whitelistGroups.includes(group.title);
          });
          ...
          if (!isUserWhitelisted) {
            ...
            if (diff >= usersOptions.inactivityDuration) {
              var req = self.apos.tasks.getReq();
              user = Object.assign({}, user, { disabled: true, trash: false, lastLogin: '' });
              return self.apos.users.update(req, user)
                .then(function() {
                  return callback('Account disabled due to inactivity. Please, refer to the administrator of the site for assistance.', false);
                }).catch(function(err) {
