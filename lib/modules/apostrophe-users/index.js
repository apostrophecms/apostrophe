...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.ensureSafe = async function() {
    ...
    self.ensureSafeCollection = async function() {
      self.safe = await self.apos.db.collection('aposUsersSafe');
    };

    // Index the safe.

    self.ensureSafeIndexes = async function() {
      await self.safe.ensureIndex({ username: 1 }, { unique: true });
      await self.safe.ensureIndex({ email: 1 }, { unique: true, sparse: true });
    };
    ...
    self.afterConvert = async function(req, piece) {
    ...
    self.on('apostrophe-docs:beforeInsert', 'insertSafe', async function(req, doc, options) {
      return await self.insertOrUpdateSafe(req, doc, 'insert');
    });

    // For security, on **ANY** update of a doc, we check to see if it is
    // an `apostrophe-user` and, if so, hash the password, remove it from the doc
    // and store the hash in the safe instead.
    self.on('apostrophe-docs:beforeUpdate', 'updateSafe', async function(req, doc, options) {
      return await self.insertOrUpdateSafe(req, doc, 'update');
    });

    // Insert or update a user's record in the safe, which stores the
    // password hash completely outside of the `aposDocs` collection.
    // First checks to be sure this is an `apostrophe-user` and returns
    // immediately if not. Invoked on promise events.

    self.insertOrUpdateSafe = async function(req, doc, action) {
      ...
      if (action === 'insert') {
        await self.safe.insert(safeUser);
      } else {
        const changes = {
          $set: safeUser
        };
        ...
        await self.safe.update({ _id: safeUser._id }, changes);
      }
    };
    ...
    self.hashPassword = async function(doc, safeUser) {
    ...
    self.hashSecrets = async function(doc, safeUser) {
    ...
    self.hashSecret = async (doc, safeUser, secret) {
      if (!doc[secret]) {
        return;
      }
      await require('util').promisify(self.pw.hash)(doc[secret]);
      delete doc[secret];
    ...
    self.verifyPassword = async function(user, password) {
      return await self.verifySecret(user, 'password', password);
    };
    ...
    self.verifySecret = async function(user, secret, attempt) {
      ...
      if (!(await verify(safeUser[secret + 'Hash'], attempt))) {
        throw 'invalid';
      }
    };
    ...
    self.forgetSecret = async function(user, secret) {
      const changes = {
        $unset: {}
      };
      changes.$unset[secret + 'Hash'] = 1;
      await self.safe.update({
        _id: user._id
      }, changes);
    };

    const superDeduplicateTrash = self.deduplicateTrash;
    // Reflect email and username changes in the safe after deduplicating in the piece
    self.deduplicateTrash = async function(req, piece) {
      await superDeduplicateTrash(req, piece);
      await self.insertOrUpdateSafe(req, piece, 'update');
    };

    const superDeduplicateRescue = self.deduplicateRescue;
    // Reflect email and username changes in the safe after deduplicating in the piece
    self.deduplicateRescue = async function(req, piece) {
      await superDeduplicateRescue(req, piece);
      await self.insertOrUpdateSafe(req, piece, 'update');
    };
    ...
    self.ensureGroups = async function() {
      if (!options.groups) {
        return;
      }
    ...
    self.ensureGroup = async function(group) {
      ...
      const _group = await self.apos.groups.find(req, criteria).toObject();
      if (_group) {
        group._id = _group._id;
        if (group.permissions) {
          _group.permissions = group.permissions;
        }
        return await self.apos.groups.update(req, _group);
      }

      return await self.apos.groups.insert(req, group);

    };

    const superRequirePiece = self.requirePiece;
    ...
    self.addFromTask = async function() {
      ...
      const req = self.apos.tasks.getReq();
      // find the group
      const group = await self.apos.groups.find(req, { title: groupname });
      if (!group) {
      ...
      return await self.apos.users.insert(req, {
        username: username,
    ...
    self.apos.tasks.add('apostrophe-users', 'change-password',
      ...
      async function(apos, argv) {
        return await self.changePasswordFromTask();
      }
    );

    // Implement the `apostrophe-users:change-password` task.

    self.changePasswordFromTask = async function() {
      const argv = self.apos.argv;
      if (argv._.length !== 2) {
      ...
      const user = await self.apos.users.find(req, { username: username }).toObject(function(err, _user) {
      if (!user) {
      ...
      return await self.update(req, user);
    };

    // A route which accepts a `username` POST parameter and responds
    // with a 200 status code if that username is
    // available, otherwise a `{ available: false }`.
    // If `status` is not `ok` then an error occurred. Used to help
    // users discover available usernames when creating accounts.

    self.apiRoute('post', 'unique-username', async function(req) {
      const username = self.apos.launder.string(req.body.username);
      const user = self.find(req, { username: username }).projection({ _id: 1, username: 1 }).toObject();
      if (user) {
        throw 'conflict';
      }
    });
