    ...
    options.arrangeFields = [
      {
        name: 'basics',
        label: 'Basics',
        fields: [ 'firstName', 'lastName', 'title', 'username', 'email', 'password', 'slug', 'group', '_groups', 'disabled', 'slug' ].concat(options.apos.docs.trashInSchema ? [ 'trash' ] : [])
      }
  ...
  construct: function(self, options) {
    ...
    self.route('post', 'unique-username', function(req, res) {
      ...
      return self.find(req, { username: username }).projection({ _id: 1, username: 1 }).toObject(function(err, user) {
        return self.apiReply(req, err, function() {
