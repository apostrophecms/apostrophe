...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // On insert of a users doc, we hash the password, remove it from the doc
    // and store the hash in the safe instead.
    self.on('apostrophe-users:beforeInsert', 'insertSafe', async function(req, doc, options) {
      return self.insertOrUpdateSafe(req, doc, 'insert');
    });

    // On update of a users doc, we hash the password, remove it from the doc
    // and store the hash in the safe instead.
    self.on('apostrophe-users:beforeUpdate', 'updateSafe', async function(req, doc, options) {
    ...
    self.ensureGroup = async function(group) {
      ...
      if (_group) {
        ...
        return self.apos.groups.update(req, _group);
      }

      return self.apos.groups.insert(req, group);
    ...
    self.addFromTask = async function() {
      ...
      const req = self.apos.tasks.getReq();
      // find the group
      const group = await self.apos.groups.find(req, { title: groupname }).toObject();
      if (!group) {
