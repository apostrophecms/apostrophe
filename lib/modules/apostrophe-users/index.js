...
//
// If set, this must be an object with `neverDisabledGroups` and
// `inactivityDuration` subproperties. `neverDisabledGroups` must be
// an array of group names that should NOT be disabled due to
// not having logged in recently, like `[ 'admin' ]`. `inactivityDuration`
// must be an integer number of days. Users who have not logged in
// in more than `inactivityDuration` days will not be permitted to
// log in again until an admin clears the `disabled` flag via
// "Manage Users." You must set these sub-options, there are no defaults.
