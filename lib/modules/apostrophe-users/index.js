...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.route('post', 'change-password-modal', function(req, res) {
      if (!req.user) {
        return res.status(404).send('notfound');
      }
      return res.send(self.render(req, 'changePassword', {}));
    });

    self.route('post', 'change-password', function(req, res) {
      if (!req.user) {
        return res.status(404).send('notfound');
      }
      if (!_.find(self.schema, { name: 'password' })) {
        return res.status(404).send('notfound');
      }
      // The "confirm" field is checked browser side and has no security implications, leave that alone
      var existing = self.apos.launder.string(req.body.existing);
      var password = self.apos.launder.string(req.body.password);
      var user;
      if (!(existing && password)) {
        return res.send({ status: 'invalid' });
      }
      return async.series([ existing, update ], function(err) {
        if (err) {
      ...
      function existing(callback) {
        return self.verifyPassword(req.user.username, existing, callback);
      }

      function fetch(callback) {
        // We ought to be able to skip this and just self.update req.user, however just in case
        // someone is overriding to add a restricted projection to deserializeUser let's avoid
        // ruining their day by fully fetching the user object before updating it
        return self.find(req, { _id: req.user._id }).permissions(false).toObject(function(err, _user) {
          if (err) {
            return callback(err);
          }
          user = _user;
      ...
      function update(callback) {
        user.password = password;
        return self.update(req, user, { permissions: false }, callback); 
      }
    });
    ...
    self.pushAssets = function() {
      superPushAssets();
      self.pushAsset('script', 'change-password-modal', { when: 'user' });
    };
    ...
    self.addAdminBarItems = function() {
      // Do not add "change password" if the password field was removed from the users schema
      if (_.find(self.schema, { name: 'password' })) {
        self.apos.adminBar.add('apostrophe-change-password', 'Change Password');
        self.apos.adminBar.group({
          label: 'Account',
          last: true,
          items: [ 'apostrophe-change-password', 'apostrophe-login-logout' ]
        });
      }
    };
