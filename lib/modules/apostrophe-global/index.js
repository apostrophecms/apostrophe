...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Just one permission makes sense for apostrophe-global because there is
    // just one instance and it always already exists
    self.addPermissions = function() {
      if (self.apos.permissions.extended) {
        self.apos.permissions.add({
          value: 'updateany-global',
          label: 'Edit Global Preferences and Content',
          description: 'Allows users to edit the global preferences of the site, as well as editing shared headers, shared footers, etc.'
        });
      } else {
        self.apos.permissions.add({
          value: 'admin-global',
          label: 'Admin: Global Preferences and Content'
        });
      }
    };
