...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.findGlobal = async function(req) {
      return await self.find(req, { slug: self.slug })
        .permission(false)
        .toObject();
    };

    // Initialize the `global` doc, if necessary.

    self.on('apostrophe:modulesReady', 'initGlobal', async function() {
      ...
      if (!existing) {
        const _new = { slug: self.slug, published: true, type: self.name };
    ...
    self.enableMiddleware = function() {
      ...
      ] : []).concat([ self.addGlobalToDataMiddleware ]);
    };

    // Used only when `separateWhileBusyMiddleware` is in effect
    self.whileBusyMiddleware = async function(req, res, next) {
      const global = await self.find(req, { slug: self.slug })
        .permission(false)
        .areas(false)
        .joins(false)
        .toObject();
      ...
      try {
        await self.checkWhileBusy(req, _global);
      } catch (err) {
        if (err === 'busy') {
    ...
    self.checkWhileBusy = async function(req, _global) {
      ...
      async function considerGlobal() {
        return await considerLock(propName, lockName);
      }
      ...
      async function considerLocale() {
        ...
        return await considerLock(localePropName, localeLockName);
      }
    ...
    self.addGlobalToDataMiddleware = function(req, res, next) {
    ...
    self.addGlobalToData = async function(req) {
      if (req.data.global) {
        return;
      }
      ...
      if (self.options.deferWidgetLoading) {
        await return self.apos.areas.loadDeferredWidgets(req);
      }
    };

    self.busyTryAgainSoon = async function(req) {
    ...
    self.whileBusy = async function(fn, options) {
      let locked = false;
      let marked = false;
    ...
    const superGetBrowserData = self.getBrowserData;
    ...
    self.getEditControls = function(req) {
      const controls = superGetEditControls(req);
