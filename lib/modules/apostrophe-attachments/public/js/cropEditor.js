apos.define('apostrophe-attachments-crop-editor', {
  ...
  construct: function(self, options) {
    ...
    self.initCropper = function() {
      ...
      if (options.minSize) {
        _.assign(cropperOpts, {
          ...
          dragMode: 'crop',
          autoCrop: false,
          naturalWidth: self.attachment.width,
          naturalHeight: self.attachment.height,
          cropmove: function(e) {
    ...
    self.testMinimumSize = function(e) {
      if (self.attachment.extension === 'svg') {
        return true;
      }
    ...
    self.getCropperData = function() {
      ...
      if (self.attachment.extension === 'svg') {
        // cropper gets confused, defaults to funny "natural size" for svg,
        // convert back to the 1600 pixel width used as a basis in uploadfs
        var data = self.$cropImage.cropper("getData", true);
        var natural = self.$cropImage.cropper("getImageData", true);
        data.x = scale(data.x);
        data.y = scale(data.y);
        data.width = scale(data.width);
        data.height = scale(data.height);
        return data;
      }
      return self.$cropImage.cropper("getData", true);
      function scale(x) {
        return x * 1600 / natural.naturalWidth;
      }
  };
