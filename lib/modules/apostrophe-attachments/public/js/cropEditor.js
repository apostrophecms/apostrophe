apos.define('apostrophe-attachments-crop-editor', {
  ...
  construct: function(self, options) {
    ...
    self.initCropper = function() {
      let cropperOpts = { viewMode: 1, autoCropArea: 1, checkCrossOrigin: false };
    ...
    self.testMinimumSize = function(e) {
      let minWidth = self.options.minSize[0];
      let minHeight = self.options.minSize[1];
      let data = $(e.target).cropper('getData');
    ...
    self.zoom = function(e) {
      ...
      if (e.ratio > e.oldRatio) { // Zooming In
        let displayedWidth = $(e.target).cropper('getImageData').width;
        let displayedHeight = $(e.target).cropper('getImageData').height;
        let newNaturalWidth = displayedWidth / e.ratio;
        let newNaturalHeight = displayedHeight / e.ratio;
