apos.define('apostrophe-attachments-crop-editor', {
  ...
  construct: function(self, options) {
    ...
    self.afterShow = function() {
      ...
      var jcropArgs = {};
      if (options.crop) {
        jcropArgs.setSelect = self.cropToJcrop(options.crop);
      }
      if (options.minSize) {
        jcropArgs.minSize = [ self.scaleDown(options.minSize[0]), self.scaleDown(options.minSize[1]) ];
      }
      if (options.aspectRatio) {
        jcropArgs.aspectRatio = options.aspectRatio[0] / options.aspectRatio[1];
      }
      var $contentEl = self.$el.find('.apos-modal-content').first();
      jcropArgs.boxWidth = $contentEl.width();
      jcropArgs.boxHeight = $contentEl.height();
