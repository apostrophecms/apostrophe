apos.define('apostrophe-attachments', {
  ...
  construct: function(self, options) {
    ...
    self.populate = function(object, name, $field, $el, field, callback) {
      ...
      let $trashButton = $fieldset.find('[data-apos-trash]');

      // A bulk upload button is provided separately at the array level
      // if multipleUpload: true is set on an array schema field. -Tom

      let $uploaderButton = $fieldset.find('[data-apos-uploader-target]');
      let $input = $fieldset.find('input[type="file"]');
      ...
      $input.fileupload({
        ...
        always: function (e, data) {
          ...
          let file = data.result.file;
          let groups = field.fileGroups || (field.fileGroup && [ field.fileGroup ]);
          if (groups) {
            if (!_.includes(groups, file.group)) {
              extensions = [];
          ...
          if (extensions) {
            if (!_.includes(extensions, file.extension)) {
              apos.notify("The file must be of one of the following types: %s", extensions.join(', '), { type: 'error' });
              return;
            }
          }
    ...
    self.addHandlers = function() {
      apos.ui.link('apos-crop', 'attachment', function($el, _id) {
        let $existing = $el.closest('[data-existing]');

        let attachment = $existing.data('existing');
        let field = $existing.data('field');
        let $fieldset = $el.closest('[data-name]');
    ...
    self.updateExisting = function($fieldset, info, field) {
      let $existing = $fieldset.find('[data-existing]');
      if (!info) {
      ...
      let $preview = $existing.find('[data-preview]');
