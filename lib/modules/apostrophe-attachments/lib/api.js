...
module.exports = function(self, options) {
  ...
  self.recomputeAllDocReferences = function(callback) {
    ...
    function attachments(callback) {
      if (self.db.initializeUnorderedBulkOp) {
        return bulk(callback);
      } else {
        return manual(callback);
      }
      function bulk(callback) {
        var op = self.db.initializeUnorderedBulkOp();
        var count = 0;
        _.each(attachmentUpdates, function(updates, id) {
          const c = op.find({ _id: id });
        ...
        if (!count) {
          // op.execute will throw an error if there are no updates
          return callback(null);
        }
        return op.execute(callback);
      }
      function manual(callback) {
        return async.eachSeries(Object.keys(attachmentUpdates), function(id, callback) {
          return self.db.update({ _id: id }, attachmentUpdates[id], callback);
        }, callback);
      }
    }
