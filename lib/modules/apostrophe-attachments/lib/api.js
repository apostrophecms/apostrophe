...
module.exports = function(self, options) {
  ...
  // Returns true if, based on the provided `focalPoint` object,
  // a valid focal point has been specified. If the object is falsy
  // or the x coordinate is not a number this method returns false.
  // You may pass a relationship object, as found in
  // apostrophe-images-widgets, or just a focal point object.
  
  self.hasFocalPoint = function(focalPoint) {
    return focalPoint && typeof(focalPoint.x) === 'number';
  };

  // Return a point with top and left properties, as percentages, suitable for use
  // in displaying the location of the focal point or implementing
  // CSS to center the display on the focal point. **These figures are
  // percentages**, relative to the **cropped** image. That is, if
  // the image is cropped and the focal point is in the upper left corner
  // of the cropped region, the returned point is { top: 0, left: 0 }.
  //
  // A crop may be provided as either attachment.crop or options.crop.
  //
  // Available as a helper.

  self.focalPointToOffset = function(attachment, options) { ... }
    
  };

  {%- if apos.attachments.relationshipHasFocalPoint(relationship) -%}
    {%- set focalPoint = apos.attachments.focalPointToOffset(choice.attachment, relationship) -%}
