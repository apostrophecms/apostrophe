...
module.exports = function(self, options) {
  ...
  self.getMissingAttachmentUrl = function() {
    ...
    self.apos.warn('Template warning: Impossible to retrieve the attachment url since it is missing, a default icon has been set. Please fix this ASAP!');
        ...
        return async.eachSeries(sizes, function(size) {
          ...
          return method(path, function(err) {
            if (err) {
              ...
              self.apos.warn('Unable to set permissions on ' + path + ', possibly it does not exist');
            }
            return callback(null);
          });
        }, callback);
