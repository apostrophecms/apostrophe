...
module.exports = function(self, options) {
  ...
  self.insert = function(req, file, options, callback) {
    ...
    function body(callback) {
      ...
      async function sanitize(callback) {
      ...
      return async.series([ permissions, length, md5, sanitize, upload, remember ], function(err) {
  ...
  self.each = function(criteria, limit, each, callback) {
    ...
    if ((typeof last) !== 'function') {
      return require('util').promisify(callback => {
        return self.each.apply(self, [...arguments, callback ]);
      })();
    }
  ...
  self.addSvgSanitizationMigration = () => {
    self.apos.migrations.add(self.__meta.name + 'svg-sanitization', async () => {
      return self.each({
        extension: 'svg',
        sanitized: {
          $ne: true
        ...
        const tempFile = self.uploadfs.getTempPath() + '/' + self.apos.utils.generateId() + '.' + attachment.extension;
        ...
        try {
          ...
          self.apos.utils.error(`Warning: unable to sanitize SVG file ${uploadfsPath}`);
        }
      });
    });
  };  
    ...
    const commands = [
      ...ids.length ? [
        [
          ...
          {
            $addToSet: { [!doc.trash ? 'docIds' : 'trashDocIds']: doc._id }
          }
        ],
        [
          {
          ...
          {
            $pull: { [!doc.trash ? 'trashDocIds' : 'docIds']: doc._id }
          }
        ],
        [
          {
        ...
        // This is the only command that must be run when
        // the document now has no attachments
        [
          {
            $or: [
              {
  ...
  self.addFixOrientationsMigration = function() {
    self.apos.migrations.add(self.__meta.name + '.fixOrientations', function() {
      return self.apos.migrations.each(
        self.db,
        { group: 'images', type: 'attachment' },
        5,
        function(attachment) {
          const $set = {};
          const $unset = {};
