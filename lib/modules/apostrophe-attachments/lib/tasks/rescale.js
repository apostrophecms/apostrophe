              ...
              async.series([
                ...
                function(callback) {
                  return self.uploadfs.copyOut(originalFile, tempFile, function(err) {
                    if (err) {
                      self.apos.error('WARNING: could not access ' + originalFile + ', perhaps it was deleted');
                      return fileCallback(null);
                    }
                ...
                function(callback) {
                  if (!argv['crop-only']) {
                    return self.uploadfs.copyImageIn(tempFile, originalFile, function(err) {
                      if (err) {
                        self.apos.error('WARNING: could not work with ' + tempFile + ' even though copyOut claims it is there');
                        return fileCallback(null);
                      }
                      return callback(null);
                    });
