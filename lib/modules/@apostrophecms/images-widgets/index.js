...
//
// See [adding editable content to pages](../../tutorials/getting-started/adding-editable-content-to-pages.html#code-@apostrophecms/images-code) for a thorough discussion of the options that can be passed to the widget.

const _ = require('lodash');

module.exports = {
  extend: '@apostrophecms/pieces-widgets',
  options: { label: 'Image(s)' },
  beforeSuperClass(self, options) {
    ...
    const superAlterFields = options.alterFields || function (schema) {
    };
    options.alterFields = function (schema) {
      const join = _.find(schema, { name: '_pieces' });
      if (join) {
