// `@apostrophecms/pieces-widgets` provides widgets that display display pieces of a
// particular type around the site.
//
// You will `extend` this module in new modules corresponding to your modules
// that extend `@apostrophecms/pieces`.
//
  ...
  beforeSuperClass(self, options) {
    ...
    if (!self.pieces) {
      if (!options.piecesModuleName) {
        throw new Error('The module ' + self.__meta.name + ' extends @apostrophecms/pieces-widgets, but the piecesModuleName option is not set, and we can\'t guess it from the name of your module.');
      } else {
  ...
  methods(self, options) {
    return {
      ...
      async load(req, widgets) {
        // Since we are overriding the default load method of `@apostrophecms/widgets`, make
