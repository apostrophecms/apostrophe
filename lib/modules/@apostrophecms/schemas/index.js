...
module.exports = {
  ...
  init(self, options) {
    ...
    self.addFieldType({
      ...
      convert: async function (req, field, data, object) {
        const _id = self.apos.launder.id(data[field.name] && data[field.name]._id) || self.apos.utils.generateId();
      ...
      getWidgetOptionsForPath(field, path) {
        ...
        const widgetTypeManager = self.apos.areas.getManager(type);
        ...
        const fieldType = self.apos.schemas.fieldTypes[subField.type];
    ...
    self.addFieldType({
      ...
      convert: async function (req, field, data, object) {
        ...
        const output = {
          metaType: 'array',
          _id: self.apos.launder.id(data && data._id) || self.apos.utils.generateId()
        };
        ...
        for (const datum of data.entries) {
          const result = {};
          result._id = self.apos.launder.id(datum._id) || self.apos.utils.generateId();
        ...
        if (Array.isArray(field.withType)) {
          _.each(field.withType, function (type) {
            if (!_.find(self.apos.docs.managers, { name: type })) {
              fail('withType property, ' + type + ', does not match the "name" property of any doc type. You have to set it as the name of the joined module.');
            }
          });
        } else {
          if (!_.find(self.apos.docs.managers, { name: field.withType })) {
            fail('withType property, ' + field.withType + ', does not match the "name" property of any doc type. You have to set it as the name of the joined module.');
          }
        }
      }
    });
  ...
  methods(self, options) {
    return {
      ...
      // requirements of a superclass. See the @apostrophecms/schemas documentation
      ...
      compose(options) {
        ...
        _.each(schema, function (field) {
          ...
          // template name (relative to the @apostrophecms/schemas module)
      ...
      // fetch its definition via apos.schemas.getFieldById().
      register(schema) {
        for (const field of schema) {
          // _id needs to be consistent across processes
          field._id = self.apos.utils.md5(JSON.stringify(_.omit(field, '_id', 'group')));
      ...
      implementPatchOperators(patch, existingPage) {
        ...
        if (patch.$push) {
          ...
          _.each(patch.$pullAll, function(val, key) {
            ...
            self.apos.utils.set(patch, key, _.differenceWith(self.apos.utils.get(patch, key) || [], Array.isArray(val) ? val : [], function(a, b) {
          ...
          _.each(patch.$move, function(val, key) {
            ...
            const existing = self.apos.utils.get(patch, key) || [];
        ...
        _.each(patch, function(val, key) {
          if (key.charAt(0) !== '$') {
            key = self.apos.utils.resolveAtReference(existingPage, key);
            // Simple replacement with a dot path
            if (key.indexOf('.') !== -1) {
              cloneOriginalBase(key);
              self.apos.utils.set(patch, key, val);
            }
          }
        });
        function append(data) {
          _.each(data, function(val, key) {
            ...
            if (val && val.$each) {
              ...
              let existing = self.apos.utils.get(patch, key) || [];
              ...
              self.apos.utils.set(patch, key, updated);
            } else {
              let existing = self.apos.utils.get(patch, key) || [];
              existing.push(val);
              self.apos.utils.set(patch, key, existing);
            }
          });
        }
        function cloneOriginalBase(key) {
          if (key.charAt(0) === '@') {
            ...
            const result = self.apos.utils.findNestedObjectAndDotPathById(existingPage, _id);
            if (!result) {
              throw self.apos.error('invalid', {
                '@path': key
              });
      ...
      subsetSchemaForPatch(schema, patch) {
        ...
        return self.apos.schemas.subset(schema, _.map(_.keys(patch).concat(operatorKeys()), idFieldToSchemaField));
