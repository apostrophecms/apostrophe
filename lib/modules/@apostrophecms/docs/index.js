...
module.exports = {
  ...
  restApiRoutes(self, options) {
    return {
      // PATCH /api/v1/@apostrophecms/docs/_id works for any document type,
      ...
      async patch(req, _id) {
        const doc = await self.find(req, { _id }).published(null).project({ type: 1, slug: 1 }).toObject();
        if (!doc) {
          throw self.apos.error('notfound');
        }
        if (self.apos.pages.isPage(doc)) {
          return self.apos.pages.patch(req, _id);
        } else {
    ...
    return {
      '@apostrophecms/doc-type:beforeInsert': {
        ...
        ensureAreaAndWidgetIds(req, doc, options) {
          console.log('walking doc');
          self.apos.areas.walk(doc, area => {
            if (!area._id) {
              console.log('generating area id');
              area._id = self.apos.utils.generateId();
            }
            for (const item of (area.items || [])) {
              if (!item._id) {
                console.log('generating widget id');
