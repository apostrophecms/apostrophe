...
module.exports = {
  ...
  methods(self, options) {
    return {
      ...
      addManagerModal() {
        self.apos.modals.add(self.__meta.name, self.getComponentName('managerModal', 'ApostrophePiecesManagerModal'), { moduleName: self.__meta.name });
      },
      // Add `._url` properties to the given pieces, if possible.
      // The default implementation does nothing, however
      // [@apostrophecms/pieces-page-type](../@apostrophecms/pieces-page-type/index.html) will
      // call `setAddUrls` to point to [its own `addUrlsToPieces` method](../@apostrophecms/pieces-page-type/index.html#addUrlsToPieces).
      async addUrls(req, pieces) {
      },
      // Called by [@apostrophecms/pieces-page-type](../@apostrophecms/pieces-page-type/index.html) to
      ...
      // See [the `addUrlsToPieces` method of `@apostrophecms/pieces-page-type`](../@apostrophecms/pieces-page-type/index.html#addUrlsToPieces).
      setAddUrls(fn) {
        self.addUrls = fn;
      },
      ...
      async batchSimpleRoute(req, name, change) {
        ...
        await self.apos.modules['@apostrophecms/jobs'].run(req, one, { labels: { title: batchOperation.progressLabel || batchOperation.buttonLabel || batchOperation.label } });
      ...
      async convertUpdateAndRefresh(req, input, _id) {
        return self.apos.locks.withLock(`@apostrophecms/${_id}`, async () => {
      ...
      // it to also be called by the universal @apostrophecms/docs
      // PATCH route.
      patch(req, _id) {
    ...
    return {
      getBrowserData(_super, req) {
        ...
        _.defaults(browserOptions.components, {
          ...
          managerModal: 'ApostrophePiecesManagerModal'
        });
