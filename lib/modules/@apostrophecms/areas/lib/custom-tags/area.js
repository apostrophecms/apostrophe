  ...
  return {
    ...
    async run(req, doc, name, context) {
      ...
      if (!area) {
        ...
        if (docId) {
          let mainDoc = await self.apos.docs.db.findOne({ _id: docId });
          if (!mainDoc) {
            throw self.apos.error('notfound');
          }
          let docDotPath;
          try {
            docDotPath = (doc._id === docId) ? '' : self.apos.utils.findNestedObjectAndDotPathById(mainDoc, doc._id).dotPath;
          } catch (e) {
          ...
          await self.apos.docs.db.updateOne({
            ...
            $set: {
              [areaDotPath]: self.apos.utils.clonePermanent(area)
            }
          });
          mainDoc = await self.apos.docs.db.findOne({ _id: docId });
          // Prevent race condition
          area._id = self.apos.utils.get(mainDoc, areaDotPath)._id;
        }
      }
