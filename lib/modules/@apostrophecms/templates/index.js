  ...
  methods(self, options) {
    return {
      ...
      async renderPageForModule(req, template, data, module) {
        ...
        let args = {
          outerLayout: decorate ? '@apostrophecms/templates:outerLayout.html' : '@apostrophecms/templates:refreshLayout.html',
          permissions: req.user && req.user._permissions || {},
          when: scene,
          js: {
      ...
      // `{% component '@apostrophecms/templates:inject', 'location', 'prepend' %}` call, with the same location,
      ...
      prepend(location, componentName) {
        if (typeof componentName !== 'string') {
          throw new Error('Do not pass a function to apos.templates.prepend. Pass a fully qualified component name, i.e. module-name:async-component-name');
        }
