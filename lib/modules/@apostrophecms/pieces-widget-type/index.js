// `@apostrophecms/pieces-widget-type` provides widgets that display display pieces of a
  ...
  beforeSuperClass(self, options) {
    ...
    if (!self.pieces) {
      if (!options.piecesModuleName) {
        throw new Error('The module ' + self.__meta.name + ' extends @apostrophecms/pieces-widget-type, but the piecesModuleName option is not set, and we can\'t guess it from the name of your module.');
      } else {
  ...
  methods(self, options) {
    return {
      ...
      async loadOtherJoins(req, widgets) {
        ...
        return self.apos.schemas.join(req, schema, widgets, undefined);
      },
