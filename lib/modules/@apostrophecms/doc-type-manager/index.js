...
module.exports = {
  ...
  init(self, options) {
    ...
    //
    // For suffixes, @apostrophecms/pages will take care of adding and removing
  ...
  methods(self, options) {
    return {
      ...
      find(req, criteria, projection) {
        ...
        self.apos.schemas.addQueryBuilders(self.schema, query);
      ...
      // Used by `@apostrophecms/versions` to label changes that
      ...
      async autocomplete(req, query) {
        ...
        _query.applySafeBuilders(query.field.builders || {});
      ...
      async addSearchIndexListener() {
        self.on('@apostrophecms/search:index', 'searchIndexBySchema', async function (doc, texts) {
      ...
      async convert(req, input, doc, options = {
        ...
        let schema = self.apos.docs.getManager(options.type || self.name).allowedSchema(req);
        let copyOf;
        ...
        await self.apos.schemas.convert(req, schema, input, doc);
        if (copyOf) {
      ...
      // of @apostrophecms/pieces often extend this to remove more default filters
      ...
      async findOneForEditing(req, criteria, options = { annotate: false }) {
        ...
        if (options.annotate) {
          self.apos.attachments.all(doc, { annotate: true });
        }
        return doc;
      },
  ...
  queries(self, query) {
    return {
      builders: {
        ...
        // a default.
        //
        // This query builder is called by @apostrophecms/pieces based on its
        ...
        //
        // Used by `@apostrophecms/pieces` and `@apostrophecms/pieces-pages`.
        ...
        // `tags` is not needed here because it comes naturally from
        // the schema field.
        ...
        // are performed (dot notation). See `@apostrophecms/schemas`
      ...
      methods: {
        ...
        // `.markBuildersSafe([ 'free' ])` marks the `free` query builder, which must
        // have a `launder` method, as being safe to populate via `applySafeBuilders`
        // for this particular query. Chainable.

        markBuildersSafe(builders) {
        ...
        // skipping all builders not declared as safe for the domain specified by `safeFor` (either
        // "public" or "manage").
        //
        ...
        applySafeBuilders(params, safeFor) {
        ...
        // laundering the data in any way. ALWAYS use `applySafeBuilders` instead for anything
        ...
        finalizeSort() {
          ...
          if (sort) {
            ...
            // has a manager thanks to @apostrophecms/any-page-manager. Use that as a default
            // so that we always get a manager object

            let manager = self.apos.docs.getManager(query.get('type') || '@apostrophecms/page');
            if (!(manager && manager.schema)) {
              return;
            }
