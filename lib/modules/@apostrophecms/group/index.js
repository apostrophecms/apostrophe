// Provide a way to group [@apostrophecms/users](../@apostrophecms/users/index.html) together
// and assign permissions to them. This module is always active "under the hood," even if
// you take advantage of the `groups` option of `@apostrophecms/users` to skip a separate
...
// `@apostrophecms/users` module. That option configures a hardcoded list of groups
...
module.exports = {
  ...
  methods(self, options) {
    return {
      ...
      // Adds an admin bar button if and only if the `@apostrophecms/users` module
      ...
      addToAdminBarIfSuitable() {
        if (self.apos.users.options.groups) {
          ...
          self.apos.adminBar.add(self.__meta.name, self.pluralLabel, self.isAdminOnly() ? 'admin' : 'edit-' + self.name, { after: '@apostrophecms/users' });
        }
      },
      enableAddGroupTask() {
        self.apos.tasks.add(self.__meta.name, 'add', 'Usage: node app @apostrophecms/groups:add groupname permission1 permission2...\n\nTo list available permissions, run:\n\nnode app @apostrophecms/permissions:list', async function (apos, argv) {
          ...
          _.each(permissions, function (permission) {
            if (!_.includes(_.map(self.apos.permissions.getChoices(), 'value'), permission)) {
              throw 'The permission ' + permission + ' does not exist. Use node app @apostrophecms/permissions:list to list available permissions.';
            }
          });
