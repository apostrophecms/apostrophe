      ...
      'apostrophe:modulesReady': {
        ...
        async manageOrphans() {
          ...
          async function manage() {
            ...
            self.apos.utils.error('* Listing them as "types" for "@apostrophecms/pages", OR');
            self.apos.utils.error('* Providing an appropriate subclass of @apostrophecms/pieces.');
            self.apos.utils.error();
  ...
  methods(self, options) {
    return {
      find(req, criteria, projection) {
        return self.apos.modules['@apostrophecms/any-page-manager'].find(req, criteria, projection);
      },
      getBrowserData(req) {
        if (!req.user) {
          return false;
      ...
      async serveDeliver(req, err) {
        ...
        if (!req.template) {
          if (req.data.page) {
            ...
            // @apostrophecms/pages/views/pages
      ...
      // `req.remainder` is then utilized by modules like `@apostrophecms/custom-pages`
      // to implement features like dispatch, which powers the
      // "permalink" or "show" pages of `@apostrophecms/pieces-pages`
      ...
      // Registers a manager for a specific page type that doesn't already have one via `@apostrophecms/custom-pages`,
      // `@apostrophecms/pieces-pages`, etc. Invoked by `modulesReady` via `registerGenericPageTypes` and
      // `manageOrphans`
