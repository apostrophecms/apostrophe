...
// to have one (see [@apostrophecms/pages](../@apostrophecms/pages/index.html)).
//
...
// template of `@apostrophecms/pages` contains this element:
//
...
// take an array argument and push new type names on it. `@apostrophecms/pieces` modules
  ...
  methods(self, options) {
    return {
      ...
      async indexPage(req) {
        ...
        // Finesse so we can use applySafeBuilders but we still support q, which is
        ...
        const cursor = self.apos.docs.find(req, {}).applySafeBuilders(req.query, 'public').perPage(self.perPage);
        if (self.filters) {
      ...
      async indexTaskOne(req, doc) {
        ...
        return self.apos.docs.db.updateOne({ _id: doc._id }, doc);
      },
