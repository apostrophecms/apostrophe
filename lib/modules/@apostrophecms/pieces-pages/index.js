// `@apostrophecms/pieces-pages` implements "index pages" that display pieces of a
...
module.exports = {
  ...
  methods(self, options) {
    return {
      ...
      // `@apostrophecms/pieces-cursor` in your subclass of
      // `@apostrophecms/pieces`.
      ...
      filterByIndexPageTags(query, page) {
        if (page.withTags && page.withTags.length) {
          query.and({ tags: { $in: page.withTags } });
        }
      },
      ...
      // invoking `self.showPage`. See [@apostrophecms/custom-pages](../@apostrophecms/custom-pages/index.html)
      ...
      chooseParentPage(pages, piece) {
        const property = self.options.chooseParentPageBy || 'tags';
        // The "tags" property was moved to "withTags" to distinguish
        // what a page *shows* from what a page *is*
        const pageProperty = property === 'tags' ? 'withTags' : property;
        const tags = piece[property] || [];
        let bestScore;
        let best = null;
        _.each(pages, function (page) {
          let score = 0;
          const pageTags = page[pageProperty] ? page[pageProperty] : [];
          if (!pageTags.length) {
            score = 1;
          }
          const intersect = _.intersection(tags, pageTags);
          const diff = _.difference(tags, pageTags);
      ...
      // Aliased as the `addUrls` method of [@apostrophecms/pieces](../@apostrophecms/pieces/index.html), which
