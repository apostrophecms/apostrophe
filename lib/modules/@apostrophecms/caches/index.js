...
module.exports = {
  ...
  methods(self, options) {
    return {
      ...
      constructCache(name) {
        return {
          ...
          clear: async function () {
            return self.cacheCollection.remove({ name: name });
          }
        };
      },
      ...
      addClearCacheTask() {
        self.apos.tasks.add('@apostrophecms/caches', 'clear', 'Usage: node app @apostrophecms/caches:clear cachename cachename2...\n\n' + 'Clears caches by name. If you are using apos.caches in your own code you will\n' + 'know the cache name. Standard caches include "@apostrophecms/migrations",\n' + '"minify" and "oembed". Normally it is not necessary to clear them.', async function (apos, argv) {
