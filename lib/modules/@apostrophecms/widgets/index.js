...
// [@apostrophecms/pieces-widgets](../@apostrophecms/pieces-widgets/index.html) and
...
// and plural if it displays more than one thing, like `@apostrophecms/pieces`.
...
// subclass of `@apostrophecms/custom-pages` or `@apostrophecms/pieces-pages`, as well.
//
...
module.exports = {
  options: { playerData: true },
  init(self, options) {
    
    self.template = options.template || 'widget';
    
    if (!self.options.label) {
      throw new Error('You must specify the label option when subclassing @apostrophecms/widgets');
    }
