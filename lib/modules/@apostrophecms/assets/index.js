...
${appImports.invokeCode}
        ...
        const storybookDataFile = path.join(__dirname, '../../../../', '.storybook/components.json');
        await fs.remove(storybookDataFile);

        const storybookData = {};
        componentImports.paths.forEach(function(filepath) {
          const key = path.basename(filepath);
          storybookData[key] = filepath;
        });

        fs.writeFileSync(storybookDataFile, JSON.stringify(storybookData, null, 4));
