...
// Provide services for database migration. The `@apostrophecms/migrations:migrate` task
...
module.exports = {
  ...
  methods(self, options) {
    return {
      // Add a migration function to be invoked when the @apostrophecms/migrations:migrate task is invoked.
      ...
      async eachDoc(criteria, limit, iterator) {
        ...
        return self.each(self.apos.docs.db, criteria, limit, iterator);
      },
      ...
      // to run the `@apostrophecms/migrations:migrate` task as part of
      ...
      addSortify(migrationNamePrefix, criteria, field) {
        self.add(migrationNamePrefix + ':' + field + '-sortified', async function () {
          ...
          await self.eachDoc({ $and: clauses }, 5, async function (doc) {
            ...
            await self.apos.docs.db.updateOne({ _id: doc._id }, { $set: $set });
          });
        });
      },
      ...
      // the @apostrophecms/migrations:migrate task
