...
module.exports = {
  ...
  construct: function(self, options) {
      ...
      if (!routes[key]) {
        self.apos.app[method](self.action + '/' + path, function(req, res) {
          ...
          if ((typeof responder) !== 'string') {
            responder = null;
            middleware = routes[key].slice(0);
          } else {
            middleware = routes[key].slice(1);
          }
    ...
    self.htmlResponder = function(req, err, result) {
      if ((!req) || (!req.res)) {
        return internal('Looks like no req was passed to htmlResponder, you probably do not need to call this method yourself, it is called for you when you use htmlRoute');
      }
      if (err) {
        if ((typeof err) !== 'string') {
          if (self.apos.docs.isUniqueError(err)) {
    ...
    self.logError = function(req, err) {
      if (!(req && req.res && req.ip && req.baseUrl)) {
