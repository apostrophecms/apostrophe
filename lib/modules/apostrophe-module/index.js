...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.renderAndSend = function(req, name, data) {
      try {
        return req.res.send(self.render(req, name, data));
      } catch (e) {
        self.logError(req, e);
        return req.res.status(500).send('error');
      }
    };
    ...
    // A utility for sending JSON API errors or, if no error occurred, generating and sending
    // API responses.
    //
    // If `err` is falsy then `succeedFn` is called with no arguments. The return value
    // must be an object, and if it has no `status` property it is automatically set
    // to `ok`. Then it is sent as a JSON response to the client.
    //
    // If an `err` is present and it is not a string, it is logged and then converted
    // to the string `'error'` to avoid leaking sensitive information.
    // 
    // The browser then receives a JSON object with its `status` set to the `err` string.
    //
    // Note that the status code is always 200 as this method implements the Apostrophe 2.x
    // internal API convention, not REST.

    self.apiReply = function(req, err, succeedFn) {
      if ((!req) || (!req.res)) {
        return internal('Looks like you forgot to pass req to apiReply, which expects (req, err, succeedFn)');
      }
      if (err) {
        ...
        return req.res.send({ status: err });
      } else {
        if ((typeof succeedFn) !== 'function') {
          return internal('Looks like you forgot to pass a success function to apiReply, which expects (req, err, succeedFn). Your success function takes no arguments should return an object to be sent as the JSON response');
        }
        return req.res.send(Object.assign({ status: 'ok' }, succeedFn()));
      }
      function internal(s) {
        self.logError(req, new Error(s));
        return req.res.send({ status: 'error' });
      }
    };
    ...
    self.logError = function(req, err) {
      self.apos.utils.error(req.method + ' ' + req.url + ': ' + err.toString());
    };
