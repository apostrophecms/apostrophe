...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.browserSingleton = function(req, options) {
    ...
    //
    // If the `req` option is omitted, `pushBrowserCall` is used;
    ...
    //
    // When `req` is not given, the singleton is created only for logged-in users unless
    // `req.scene` is set to `user`.
    //
    ...
    self.pushCreateSingleton = function(req) {
      if (!req.user) {
        return;
      }
      ...
      if (req) {
        ...
        self.apos.push.browserCall(req.scene ? req.scene : 'user', 'apos.createModule(?, ?, ?)', self.__meta.name, options, self.options.alias);
      }
    };
