...
module.exports = function(self, options) {
  ...
  self.emit = async function(name /* , arg1, arg2... */) {
    const args = Array.prototype.slice.call(arguments, 1);
    // Find the event handlers registered under the name
    // of the module doing the emitting, or its subclasses.

    // TODO perf hit, although just a synchronous one.
    // At some point we could memoize this, but how late
    // do we have to wait before considering that safe
    // (no more modules will be defined)?

    const subclassesAndSelf = Object.keys(self.apos.eventHandlers).filter(name => {
      if (!self.apos.modules[name]) {
        // apostrophe itself is a special case, it's not a module but
        // it is an emitter
        return self.__meta.name === name;
      }
      const chain = self.apos.modules[name].__meta.chain;
      return chain.find(entry => entry.name === self.__meta.name);
    });

    // Now invoke the handlers
    for (const n of subclassesAndSelf) {
      const eh = self.apos.eventHandlers[n] || {};
      if (eh[name]) {
        for (const h of eh[name]) {
          const module = self.apos.modules[h.moduleName];
          const method = module[h.methodName];
