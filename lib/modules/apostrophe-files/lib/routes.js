...
module.exports = function (self, options) {
  ...
  self.route('get', 'media-library', function(req, res) {
  ...
  self.route('get', 'show-normal-view', function(req, res) {
    return self.getOne(req, { id: launder.id(req.query.id), browsing: true ] }, function(err, file) {
      if (err || (!file)) {
        res.statusCode = 404;
  ...
  self.route('get', 'show-edit-view', function(req, res) {
    return self.getOne(req, { id: launder.id(req.query.id), browsing: true ] }, function(err, file) {
  ...
  self.route('get', 'index', function(req, res) {
    return self.browse(req, req.query, function(err, results) {
      if (err || (!file) || (!file._edit)) {
        res.statusCode = 404;
        return res.send('notfound');
      }
      return res.send(self.render('mediaLibraryPage.html', { file: file }));
    });
  });
};
