...
module.exports = function (self, options) {
  ...
  self.route('post', 'upload', function(req, res) {
  ...
  self.route('get', 'browse', function(req, res) {
    ...
    return self.browse(req, req.query, function(err, result) {
      ...
      return res.send(self.render('index.html', result));
    });
  });
  ...
  self.route('post', 'media-library', function(req, res) {
    ...
    return res.send(self.render('mediaLibrary', data));
  });

  self.route('post', 'show-normal-view', function(req, res) {

    if (!req.body.id) {
      // We want the default normal view
      return res.send(self.render('showNormalView.html', { file: {} }));
    }
    ...
    return self.getOne(req, { id: launder.id(req.body.id), browsing: true }, function(err, file) {
      ...
      return res.send(self.render('showNormalView.html', { file: file }));
    });
  });

  self.route('post', 'show-edit-view', function(req, res) {
    return self.getOne(req, { id: launder.id(req.body.id), browsing: true }, function(err, file) {
      if (err || (!file) || (!file._edit)) {
        res.statusCode = 404;
        return res.send('notfound');
      }
      return res.send(self.render('showEditView.html', { file: file }));
    });
  });

  // Renders one page of items in the media library

  self.route('post', 'index', function(req, res) {
    // Convert page number to skip/limit
    req.body.skip = (Math.max(req.body.page, 1) - 1) * self.options.perPage;
    req.body.limit = self.options.perPage;
    return self.browse(req, req.body, function(err, results) {
      if (err || (!file) || (!file._edit)) {
        res.statusCode = 404;
        return res.send('notfound');
      }
      return res.send(self.render('index.html', { file: file }));
    });
  });
