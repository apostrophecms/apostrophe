apos.define('apostrophe-media-library', {
  ...
  construct: function(self, options) {
    ...
    self.enableTags = function() {
      if (self.options.browseByTag) {
        $.jsonCall(apos.files.action + '/tags', {}, function(results) {
  ...
  self.enableNormalView = function() {
  ...
  self.showItem = function(itemId) {
    ...
    self.$show.attr('data-item', item);

    $.jsonCall(self.action + '/edit-view', { id: itemId }, function(result) {
  ...
  self.saveItem = function(callback) {
    ...
    $.jsonCall(self.action + '/annotate-files', [ {
      _id: item._id,
  ...
  self.findItem = function(item) {
    ...
    $.each($items, function() {
      var $item = $(this);
      if ($item.attr('data-item')._id === item._id) {
        result = $item;
      }
    });
    return result;
