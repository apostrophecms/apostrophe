...
module.exports = {
      ...
      post: {
        ...
        async editVirtualArea(req) {
          ...
          for (let item of items) {
            ...
            const widget = await manager.sanitize(req, item, widgetOptions);
            widget._edit = true;
          ...
          return self.render(req, 'virtualArea', { options: options });
        },
        async renderWidget(req) {
          ...
          if (!self.apos.utils.isBlessed(req, options, 'widget', type)) {
            throw self.blessedError(widget, options);
          }
  ...
  methods(self, options) {
    return {
      ...
      blessedError(item, widgetOptions) {
