...
module.exports = {
    ...
    return {
      post: {
        ...
        async renderWidget(req) {
          ...
          const path = self.apos.launder.string(req.body.path);
          const type = self.apos.launder.string(req.body.type);
          const options = self.getWidgetOptionsForPath(path);
  ...
  methods(self, options) {
    return {
      ...
      // `{% area %}` and `{% singleton %}` custom tags.
      async renderArea(req, area, options) {
        // Ability to disable editing for a specific request
        if (req.disableEditing) {
      ...
      docBeforeUpdate(req, doc, options) {
      ...
      // named field.
      restoreDisallowedFields(req, item, oldItem) {
        let manager = self.getWidgetManager(item.type);
        if (!manager) {
      ...
      //
      // home.body.0:apostrophe-rich-text
      //
      // Nested video widget in layout widget:
      //
      // home.body.1:two-column.left.0:apostrophe-video
      //
      ...
      //
      // home.address.fancy.0:apostrophe-rich-text
      //
      ...
      //
      // home.addresses.0.fancy.0:apostrophe-rich-text
      //
