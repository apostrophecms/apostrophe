...
export default function() {
  ...
  async function save() {
    ...
    try {
      for (let docId of docIds) {
        const areaNames = Object.keys(itemsByParentId[docId]);
        for (let areaName of areaNames) {
          const items = itemsByParentId[docId][areaName];
    ...
    function patchSubareas(items) {
      for (let item of items) {
        const _id = item._id;
        const kidsByAreaName = itemsByParentId[_id] || {};
        const areaNames = Object.keys(kidsByAreaName);
        for (let areaName of areaNames) {
          const items = kidsByAreaName[areaName];
          item[areaName] = {
            type: 'area',
            items: items
          };
  ...
  function updateParentIdMap(areaApp, items) {
    itemsByParentId[areaApp.parentId] = itemsByParentId[areaApp.parentId] || {};
    itemsByParentId[areaApp.parentId][areaApp.areaName] = items;
  }
  ...
  function createAreaApp(el) {
    ...
    return new Vue({
      ...
      mounted() {
        updateParentIdMap(this, data.items);
      },
      methods: {
        changed(items) {
