...
export default function() {
  ...
  async function save() {
    ...
    try {
      ...
      for (let docId of docIds) {
        ...
        for (let areaName of areaNames) {
          ...
          await axios.create({
            ...
            window.apos.areas.action + '/save-area',
            {
              docId: docId,
              dotPath: areaName,
              items: items,
      ...
      console.error('Error saving contextual content: ' + e.toString());
    }
  ...
  function updateParentIdMap(areaApp, items, options) {
    const parentId = areaApp.parentId || areaApp.docId;
    dataByParentId[parentId] = dataByParentId[parentId] || {};
    dataByParentId[parentId][areaApp.areaName] = dataByParentId[parentId][areaApp.areaName] || {};
    dataByParentId[parentId][areaApp.areaName].items = items;
    if (options) {
      dataByParentId[parentId][areaApp.areaName].options = options;
    }
  }
