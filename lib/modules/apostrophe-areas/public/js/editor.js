...
apos.define('apostrophe-area-editor', {
  ...
  construct: function(self, options) {
    ...
    self.editRichText = function($richText) {
      ...
      self.doneEditingRichText(function() {
        ...
        var config = {
          ...
          on: {
            ...
            instanceReady: function(ck) {
              ...
              ck.editor.$a2Item = $richText.closest('[data-widget]');
        ...
        instance.on('blur', function(){
          ...
          if (html.length !== 0) {
            $richText.parents('[data-type="apostrophe-rich-text"]').removeClass('apos-empty');
          } else {
            $richText.parents('[data-type="apostrophe-rich-text"]').toggleClass('apos-empty', true);
          }
    ...
    self.getWidgetData = function($widget, withContent) {
      ...
      if (withContent) {
        console.log('TODO check with tom on the following code -Jimmy');
        var type = $widget.attr('data-widget');
        var manager = apos.areas.getWidgetManager(type);
        if (manager && manager.options) {
          if (manager.options.content) {
            if (manager.options.contentSelector) {
              data.content = $widget.find(manager.contentSelector).text();
            } else {
  ...
  construct: function(self, options) {
    ...
    self.enableOnEnhance = function() {
      apos.on('enhance', function() {
        apos.log('enhance event seen in editors');
        self.enableAll();
      });
    };
