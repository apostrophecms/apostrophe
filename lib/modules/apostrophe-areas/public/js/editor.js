...
apos.define('apostrophe-areas-editor', {
  ...
  construct: function(self, options) {
    ...
    // obtains a session lock and starts autosaving of
    // the area, then invokes the given function.
    // However this method passes on exactly the same
    // arguments given to it to `fn` on success, and does
    // not invoke `fn` at all on failure. Part of the
    // implementaiton of `startAutosavingHandler`.
    
    self.startAutosavingThen = function(fn) {
      console.trace();
      var args = Array.prototype.slice.call(arguments);
      console.log('calling startAutosaving');
    ...
    // with the given function. Useful as an event handler.
    
    self.startAutosavingHandler = function(fn) {
      return function() {
        return self.startAutosavingThen(fn);
      }
    };
