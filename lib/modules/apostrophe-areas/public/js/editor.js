...
apos.define('apostrophe-areas-editor', {
  ...
  construct: function(self, options) {
    ...
    self.registerClickHandlers = function() {
      ...
      self.$el.on('change', '[data-schema-widget-control]', self.startAutosavingHandler(self.changedSchemaWidgetControl));
    };
    ...
    self.updateAllSchemaWidgetControlChoices = function($widget) {
      var data = apos.areas.getWidgetData($widget);
      $widget.find('.apos-area-widget-controls:first [data-schema-widget-control]').each(function() {
        var $control = $(this);
        var name = $control.attr('name');
        if ($control.attr('data-type') === 'checkboxes')  {
