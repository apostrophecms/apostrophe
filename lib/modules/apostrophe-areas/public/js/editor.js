  ...
  construct: function(self, options) {
    ...
    self.init = function() {
      ...
      self.$el.on('click', '[data-edit-item]', function(event) {
        ...
        if ($item.attr('data-type') === 'apostrophe-rich-text') {
      ...
      self.$el.on('click', '[data-rich-text]:not(.apos-widget [data-rich-text])', function(event) {
        ...
        // is a link
        var $apostrophe-rich-text = $(this);
        var $link = $(event.target).closest('a');
        if ($link.length) {
          var state = $apostrophe-rich-text.data('aposState');
          if (!state) {
    ...
    self.editRichText = function($apostrophe-rich-text) {
      // Remove any initial "click to type" content when we
      // start actual editing
      self.removeInitialContent(self.$el);

      if (self.$activeapostrophe-rich-text && ($apostrophe-rich-text.attr('id') === self.$activeapostrophe-rich-text.attr('id'))) {
      ...
      self.doneEditingRichText(function() {
        ...
        self.$activeapostrophe-rich-text = $apostrophe-rich-text;
        var id = $apostrophe-rich-text.attr('id');
        if (!id) {
          // Must have one for ckeditor
          id = apos.generateId();
          $apostrophe-rich-text.attr('id', id);
        }
        $apostrophe-rich-text.attr('contenteditable', 'true');
        var toolbar = [];
        ...
        var config = {
          ...
          on: {
            ...
            instanceReady: function(ck) {
              ...
              ck.editor.$a2Item = $apostrophe-rich-text.closest('[data-widget]');
        ...
        var instance = self.instances[id] = CKEDITOR.inline(id, config);
        var itemActions = $(instance.element.$).parent().find('[data-widget]-actions');

        instance.on('focus', function(){
          itemActions.hide();
          $apostrophe-rich-text.data('aposState', 'focused');
        });
        ...
        instance.on('blur', function(){
          ...
          self.doneEditingRichText(function() {});
          $apostrophe-rich-text.data('aposState', 'blurred');
          // stuart
          var html = $apostrophe-rich-text.html();
          if (html.length !== 0) {
            $apostrophe-rich-text.parents('[data-type="apostrophe-rich-text"]').removeClass('apos-empty');
          } else {
            $apostrophe-rich-text.parents('[data-type="apostrophe-rich-text"]').toggleClass('apos-empty', true);
          }
        ...
        setTimeout(function() {
          ...
          // apparent reason
          if ($apostrophe-rich-text.html() === "" || $apostrophe-rich-text.html() === ' ') {
            $apostrophe-rich-text.html('<div>&nbsp;</div>');
          }
          instance.focus();
        }, 100);
    ...
    self.doneEditingRichText = function(callback) {
      if (!self.$activeapostrophe-rich-text) {
        return callback();
      }

      var id = self.$activeapostrophe-rich-text.attr('id');
      var instance = self.instances[id];
      var data = instance.getData();
      instance.destroy();
      delete self.instances[id];
      self.$activeapostrophe-rich-text.removeAttr('contenteditable');
      self.$activeapostrophe-rich-text.html(data);
      self.$activeapostrophe-rich-text.data('aposState', undefined);
      self.$activeapostrophe-rich-text = undefined;
      return callback();
    };
    ...
    self.addTopContentMenu = function(callback) {
      $.post(self.contentMenuUrl || '/apos-editor-2/content-menu',
        { controls: self.options.controls, apostrophe-rich-text: self.options.apostrophe-rich-text, addLabel: self.options.addLabel },
        function(data) {
        var $menu = $(data);
    ...
    self.getDroppableAreas = function($draggable) {
      var apostrophe-rich-text;
      if ($draggable.hasClass('apos-rich-text-item')) {
        apostrophe-rich-text = true;
      }
      ...
      if (betweenAreas) {
        ...
        $areas = $('[data-area]:not([data-text-only])').filter(function() {
          ...
          if ((!editor.limitReached()) || ($draggable.data('areaEditor') === editor)) {
            if ((apostrophe-rich-text && (editor.options.apostrophe-rich-text !== false)) || _.contains(editor.options.controls, $draggable.attr('data-type'))) {
              return true;
            }
          }
        });
    ...
    self.serialize = function() {
      var items = [];
      $el.find(self.selItems).each(function() {
        var $item = $(this);
        var item;
        if (!self.isText($item)) {
...
// such as enabling the editor at appropriate times
...
apos.define('apostrophe-area-editors', {
  ...
  construct: function(self, options) {
    ...
    self.enableCkeditor = function() {
      ...
      CKEDITOR.plugins.addExternal('split', apos.prefix + apos.areas.options.action + '/js/ckeditorPlugins/split/', 'plugin.js');
    ...
    self.enableAll = function() {
      $('[data-area]').each(function() {
        ...
        var instance = apos.create('apostrophe-area-editor', { $el: $el });
        instance.init();
      });
    };
