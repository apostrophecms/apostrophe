apos.define('apostrophe-areas', {
  ...
  construct: function(self, options) {
    ...
    self.editSingleton = function($singleton) {
      ...
      if (manager.edit) {
        ...
        if (!$widget.length) {
          ...
          $widget.attr('data', JSON.stringify({ type: options.type, _id: apos.utils.generateId() }));
    ...
    self.monitorAndSaveSingleton = function($singleton) {
      ...
      var lastJson = JSON.stringify(data);
      function pass(sync) {
        if (!$widget.closest('body').length) {
          // If we are no longer in the DOM, stop watching.
          // This prevents these from sticking around forever.
          // - Tom and Sam
          return;
        }
        ...
        if (json !== lastJson) {
          lastJson = json;
          return self.saveSingleton($singleton, docId, dotPath, data, options, sync ? null : schedule);
        } else {
          if (!sync) {
            schedule();
          }
    ...
    // Call with no callback for a synchronous save. -Tom
