...
module.exports = function(self, options) {
  ...
  self.warnMissingWidgetType = function(name) {
    ...
    if (!self.missingWidgetTypes[name]) {
      self.apos.error('WARNING: widget type ' + name + ' exists in content but is not configured');
  ...
  self.renderWidget = function(req, type, data, options, callback) {
    ...
    try {
      ...
      self.apos.error('^^^^ LOOK UP HERE FOR THE ERROR IN YOUR WIDGET OUTPUT METHOD');
      return callback(e);
    }
  };
