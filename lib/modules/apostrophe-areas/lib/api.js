...
module.exports = function(self, options) {
  ...
  self.lockSanitizeAndSaveArea = function(req, areaInfo, callback) {
    if (typeof(areaInfo) !== 'object') {
      return callback(new Error('areaInfo is not an object'));
    }
    var items = Array.isArray(areaInfo.items) ? areaInfo.items : [];
    var docId = launder.id(areaInfo.docId);
    var dotPath = launder.string(areaInfo.dotPath);
    var doc;
    ...
    function htmlPageLock(callback) {
      ...
      return self.apos.docs.htmlPageLock(req, docId, callback);
    }
