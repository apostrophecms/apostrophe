...
module.exports = function(self, options) {
  ...
  // Print warning message about a bogus widget type — only once per run per type.
  self.warnBogusWidgetType = function(name) {
    if (!self.bogusWidgetTypes) {
      self.bogusWidgetTypes = {};
    }
    if (!self.bogusWidgetTypes[name]) {
      console.error('WARNING: widget type ' + name + ' exists in content but is not configured');
      self.bogusWidgetTypes[name] = true;
    }
  };
  ...
  self.sanitizeItems = function(req, items, callback) {
    ...
    return async.eachSeries(items, function(item, callback) {
      ...
      if (!manager) {
        self.warnBogusWidgetType(item.type);
        return setImmediate(callback);
      }
