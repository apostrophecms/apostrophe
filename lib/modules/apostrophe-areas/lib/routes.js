...
module.exports = function(self, options) {
  ...
  self.apiRoute('post', 'save-area', function(req, res, next) {
    return self.lockSanitizeAndSaveArea(req, req.body, function(err, info) {
      if (err === 'locked-by-me') {
        return next('locked', null, { message: self.apos.templates.i18n(req, '__ns', 'apostrophe', 'You are now editing this document in another tab or window.') });
      } else if (err === 'locked') {
        return next('locked', null, { message: self.apos.templates.i18n(req, '__ns', 'apostrophe', 'Another user has taken control of this document.') });
      } else {
        return next(err);
      }
    });
