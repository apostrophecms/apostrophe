...
module.exports = function(self, options) {
  ...
  self.route('post', 'save-area', function(req, res) {
    return self.lockSanitizeAndSaveArea(req, req.body, function(err) {
      if (err) {
  ...
  self.route('post', 'save-areas-and-unlock', function(req, res) {
    ...
    if (!Array.isArray(areas)) {
      return fail(new Error('invalid, no areas property'));
    }
    return async.series([
    ...
    function lockSanitizeAndSaveAreas(callback) {
      return async.eachSeries(req.areas, function(areaInfo, callback) {
    ...
    function unlockAll(callback) {
      ...
      return self.apos.docs.htmlPageUnlockAll(req, callback);
    }
  });
