...
module.exports = function(self, options) {
  ...
  self._check = function(req, action, event, _true, _false, object, then) {
    ...
    // the effective type name of the object for permissions purposes 

    if ((type === 'doc') && object) {
      type = self.getEffectiveTypeName(object);
    }
  ...
  // Return the effective type name of an object for permissions checks.
  // Normally this is the `type` property, however for pages it is
  // the generic `apostrophe-page`, because pages can change type.

  self.getEffectiveTypeName = function(object) {
    if (self.apos.pages.isPage(object)) {
