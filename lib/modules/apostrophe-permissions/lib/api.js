...
module.exports = function(self, options) {
  ...
  // Enumerate all permissions to be listed in the user interface for
  // assigning permissions to a group. DO NOT CALL BEFORE `modulesReady`.
  ...
  self.enumerate = function() {
    ...
    if (self.options.permissions) {
      ...
      // All doc type managers that have been registered, have a label
      // and don't have the adminOnly option set automatically get
      // permissions in the list
      _.each(self.apos.docs.managers, function(manager, name) {
        if ((!manager.isAdminOnly()) && (manager.label)) {
          permissions.push({
            value: 'admin-' + self.apos.utils.cssName(name),
            label: 'Admin: ' + manager.label
          });
          permissions.push({
            value: 'edit-' + self.apos.utils.cssName(name),
            label: 'Edit: ' + manager.label
          });
          permissions.push({
            value: 'submit-' + self.apos.utils.cssName(name),
            label: 'Submit: ' + manager.label
          });
        }
      });
      // A chance for any module to extend the list further
      self.apos.emit('permissionsEnumerate', permissions);
      self.permissionsCache = permissions;
      return permissions;
    }
  };
