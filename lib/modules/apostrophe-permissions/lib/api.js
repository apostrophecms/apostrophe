...
module.exports = function(self, options) {
  ...
  //
  // For bc, if the action's verb is `update`, both `_edit: true`
  // and `_update: true` are set.
  ...
  self.permissions = self.options.extended ? [
    {
      value: 'admin',
      label: 'Admin: Total Control of Everything',
      description: 'Allows users to do anything, including creating more users and groups.'
    },
    {
      value: 'guest',
      label: 'View content marked "Login Required"',
      description: 'Allows users to view content of various types when it is marked "Login Required." You can also use the "Certain People" setting for fine-tuned privacy.'
    },
  ] : [
  ...
  //
  // If a permission with the same `value` is added again, only
  // the new one is kept.
  //
  // Permissions with the `last` property set to `true` move
  // to the end of the list just before the end of initialization.

  self.add = function(permission) {
    self.permissions = self.permissions.filter(function(p) {
      return p.value !== permission.value;
    });
  ...
  self.impliedBy = options.impliedBy || self.extended ? {
    view: [ 'updateany', 'update', 'admin' ],
    update: [ 'updateany', 'admin' ]
  } : {
  ...
  self._check = function(req, action, event, _true, _false, object, newObject, then) {
    ...
    if (self.extended) {
      if (verb !== 'view') {
        if (!(permissions[verb + '-' + type] || permissions[verb + 'any-' + type])) {
  ...
  // Given a permission name or array of names, this method appends the
  // user ID and the user's group IDs to each one and returns the resulting
