      ...
      criteria: function(req, permissions, verb) {
        ...
        if (verb === 'view') {
          ...
          if (req.user) {
            ...
            addSpecificEditPermissions('edit');
          }
        } else {
          // Not view permissions
          ...
          addSpecificEditPermissions(effectiveVerb);
        }
        ...
        function getActionableTypes(methodName, verb) {
          ...
          _.each(self.apos.docs.managers, function(manager, type) {
            ...
            if (permissions[0] === 'update-turkey') {
              console.log(req.user._permissions, permissions);
            }
            ...
            if (!foundMissing) {
              types.push(type);
              if (permissions[0] === 'update-turkey') {
                console.log('types is now: ', types);
              }
            }
          });
          if (!types.length) {
        ...
        function addSpecificEditPermissions(effectiveVerb) {
          if (self.extended) {
            clauses.push({
              $or: [
                {
