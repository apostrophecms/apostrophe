    ...
    doc: {
      ...
      specific: function(req, permissions, verb, type, object, newObject) {
        ...
        if (verb === 'view') {
          ...
          var editPermissionName = self.extended ? 'update' : 'edit';
          if (req.user && _.intersection(self.userPermissionNames(req.user, editPermissionName), object.docPermissions).length) {
            return true;
          }
        } else {
          ...
          if (req.user) {
            ...
            clauses.push({
              $or: [
                {
                  docPermissions: {
                    $in: self.userPermissionNames(req.user, self.extended ? 'update' : 'edit')
                  },
                  type: {
                    $in: getEditableTypes()
                  }
