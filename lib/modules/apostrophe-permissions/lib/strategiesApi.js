      ...
      criteria: function(req, permissions, verb) {
        ...
        if (verb === 'view') {
          ...
          if (req.user) {
            ...
            addSpecificEditPermissions(self.extended ? 'update' : 'edit');
          }
        } else {
          // Not view permissions
          ...
          // specific permission.
          addSpecificEditPermissions(verb);
        }

        if (!clauses.length) {
          // empty $or is an error in modern mongodb
          return {};
        }
        ...
        function getAlwaysTypes(verb) {
          return _.union(getActionableTypes('getAlwaysPermissionNames', verb), getActionableTypes('getAdminPermissionName'));
        }
        ...
        function getActionableTypes(methodName, verb) {
          ...
          _.each(self.apos.docs.managers, function(manager, type) {
            ...
            const foundMissing = _.find(permissions, function(permission) {
        ...
        function addSpecificEditPermissions(verb) {
          var effectiveVerb = verb;
          if (self.extended) {
            if ((verb === 'update') || (verb === 'trash')) {
              effectiveVerb = 'edit';
            }
            clauses.push({
              $or: [
