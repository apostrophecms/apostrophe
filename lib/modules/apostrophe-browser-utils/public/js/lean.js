...
(function() {
  ...
  function monitor(xmlhttp, callback) {
    xmlhttp.addEventListener("load", function() {
      ...
      if (responseHeader === 'application/json; charset=utf-8') {
        ...
        if (maybeJson(trimResponse, callback)) {
          try {
            data = JSON.parse(this.responseText);
          } catch (e) {
            try {
        ...
        } else if (!maybeJson(trimResponse, callback)) {
          try {
            data = this.responseText;
          } catch (e) {
            return callback(e);
  ...
  function maybeJson (json, callback) {
    if (typeof json !== 'string') {
      return callback(new Error('You forgot to pass a string as the first argument.'));
    }
    var cases = [ 'true', 'false', 'null', '"', '-', '[', '{' ];
    return cases.some(function(element) { return json.split(' ')[0] === element; });
  }
