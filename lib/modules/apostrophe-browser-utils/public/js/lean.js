...
(function() {
  ...
  function monitor(xmlhttp, callback) {
    xmlhttp.addEventListener("load", function() {
      ...
      // Trim the response so we can accurately test;
      var trimResponse = this.responseText.trim();
      var data;
      // If the response header is `application/json`, immediately try
      // to JSON.parse(), and if it fails, report the error.
      if (responseHeader === 'application/json; charset=utf-8') {
        try {
          data = JSON.parse(this.responseText);
        } catch (e) {
          return callback(e);
        }
      // If there is no explicit content-type header,
      // test the trimmed response against our isJson() method.
      // If it returns `true`, try to JSON.parse() it.
      // And if it fails, try to JSON.stringify() it, and if that fails,
      // report an error.
      } else {
        if (isJSON(trimResponse, callback)) {
          try {
            data = JSON.parse(this.responseText);
          } catch (e) {
            try {
        ...
        // If the trimmed response returns false from our
        // isJson() method, JSON.stringify it. If it fails,
        // report the error.
        } else if (!isJSON(trimResponse, callback)) {
          try {
            data = JSON.stringify(this.responseText);
          } catch (e) {
            return callback(e);
  ...
  // the data passed to the function is formatted
  // JSON and returns `true`, if so. False, if not

  function isJSON (json, callback) {
    if (!json) {
      return callback(new Error('You forgot to pass JSON data as the first argument.'));
    }
    var cases = [ true, false, null, '"', '-', '[', '{' ];
    cases.forEach(function(element) {
      if (json[0] === element) {
        return setImmediate(null, true);
      }
      return setImmediate(null, false);
    });
  }
