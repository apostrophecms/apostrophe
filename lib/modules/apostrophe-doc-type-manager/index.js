  ...
  methods(self, options) {
    return {
      ...
      find(req, criteria, projection) {
        ...
        self.apos.schemas.addQueryBuilders(self.schema, {
          override: [ 'tags' ]
        }, query);
      ...
      async autocomplete(req, query) {
        ...
        _query.projection(self.getAutocompleteProjection(), query);
  ...
  queries(self, query) {
    return {
      builders: {
        ...
        autocomplete: {
          prefinalize: require('./autocomplete.js')(self, query),
          safeFor: 'public',
          launder(s) {
      ...
      methods: {
        ...
        async toDistinct(property) {
          ...
          if (!query.get('distinctCounts')) {
            return self.apos.docs.db.distinct(property, query.get('criteria'));
          } else {
          ...
          async function distinctCounts() {
            ...
            const results = await self.apos.docs.db.aggregate(pipeline).toArray();
            const counts = {};
        ...
        lowLevelMongoCursor(req, criteria, projection, options) {
          const mongo = self.apos.docs.db.find(criteria).project(projection);
        ...
        finalizeSort() {
          ...
          if (sort) {
            ...
            let manager = self.apos.docs.getManager(query.get('type') || 'apostrophe-page');
            if (!(manager && manager.schema)) {
              return;
            }
