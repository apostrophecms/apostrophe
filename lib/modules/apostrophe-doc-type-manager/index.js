  ...
  methods(self, options) {
    return {
      ...
      find(req, criteria, projection) {
        ...
        for (const name of self.__meta.chain.map(entry => name)) {
        ...
        self.apos.schemas.addFilters(self.schema, {
          override: [ 'tags' ]
        }, query);
        return query;
      },
  ...
  queries(self, query) {
    return {
      builders: {
        ...
        // `.defaultSort({ title: 1 })` changes the default value for the `sort` filter.
        ...
        // use of the `page` filter to indicate the current page number.
        //
        ...
        permission: {
          finalize(query) {
            ...
            if (permission !== false) {
              ...
              query.and(self.apos.permissions.criteria(query.req, permission || ('view-' + typeSuffix)));
            }
          },
          after: function(query, results) {
            // In all cases we mark the docs with ._edit if
            // the req is permitted to do that
            self.apos.permissions.annotate(query.req, 'edit-doc', results);
          }
        },
        ...
        autocomplete: {
          prefinalize: require('./autocomplete.js')(self, options),
          safeFor: 'public',
          launder(query, s) {
        ...
        tags: {
          ...
          launder(query, s) {
            ...
            return s.map(value => self.apos.launder.string(value));
          }
        },
          ...
          async after(query, results) {
            ...
            for (let type of _.keys(resultsByType)) {
              ...
              if (manager && manager.schema) {
                await self.apos.schemas.join(query.req, manager.schema, resultsByType[type], query.get('joins'));
              }
            }
          }
        },
        ...
        // `.previous({ doc object goes here })`. If set to a doc object, this filter will limit
        ...
        // `.next({ doc object goes here })`. If set to a doc object, this filter will limit
          ...
          after(results) {
            for (const result of results) {
              if (result.slug && self.apos.pages.isPage(result)) {
                const url = self.apos.pages.getBaseUrl(query.req);
          ...
          async after(results) {
            ...
            if (req.deferWidgetLoading && (!req.loadingDeferredWidgets)) {
              ...
              for (const type of types) {
                let manager = self.apos.areas.getWidgetManager(type);
                if (!manager) {
      ...
      methods: {
        ...
        // have a `launder` method, as being safe to populate via `queryStringToQueryBuilders`
        ...
        async toDistinct(query, property) {
          ...
          if (!query.get('distinctCounts')) {
            return self.db.distinct(property, query.get('criteria'));
          } else {
        ...
        async toChoices(query, property, options) {
          ...
          let choicesFn;
          const filter = self.queryBuilders[property];
          if (filter) {
            if (filter.choices) {
        ...
        async toMongo() {
          ...
          return query.lowLevelMongoCursor(self.req, query.get('criteria'), query.get('projection'), {
            skip: query.get('skip'),
      ...
      clone() {
        ...
        subquery.state = _.cloneDeep(self.state);
        return subquery;
      },
