...
module.exports = {
  ...
  init(query, options) {
    query.name = query.options.name;
    
    if (!options.name) {
      throw new Error('apostrophe-doc-type-manager requires name option in module ' + query.__meta.name);
    }
    ...
    query.trashPrefixFields = ['slug'];
    query.trashSuffixFields = [];
    query.patchAdminPermissionInSchema();
    query.composeSchema();
    query.apos.docs.setManager(query.name, query);
    query.addSearchIndexListener();
    query.enableBrowserData();
  },
  apiRoutes(query, options) {
    return {
      post: {
        async chooser(req) {
          const browse = !!req.body.browse;
          const autocomplete = !!req.body.autocomplete;
          return query.render(req, 'chooser', {
            browse: browse,
        ...
        async chooserChoices(req) {
          ...
          if (!query.apos.utils.isBlessed(req, _.omit(field, 'hints'), 'join')) {
            throw 'notfound';
          }
          ...
          await query.apos.schemas.convert(req, [_.omit(field, 'readOnly')], 'form', input, receptacle);
          await query.apos.schemas.join(req, [field], receptacle, true);
          const choiceTemplate = field.choiceTemplate || query.choiceTemplate || 'chooserChoice.html';
          const choicesTemplate = field.choicesTemplate || query.choicesTemplate || 'chooserChoices.html';
          ...
          const markup = query.render(req, choicesTemplate, {
            choices: choices,
        ...
        async relationshipEditor(req) {
          const field = req.body.field;
          if (!query.apos.utils.isBlessed(req, field, 'join')) {
            throw 'notfound';
          }
          return query.render(req, field.relationshipTemplate || query.relationshipTemplate || 'relationshipEditor', { field: field });
        },
        async autocomplete(req, res) {
          ...
          const term = query.apos.launder.string(req.body.term);
          return query.autocomplete(req, {
            field: field,
            term: term
          });
        }
    ...
    return {
      'afterTrash': {
        deduplicateTrash(req, doc) {
          ...
          const $set = {};
          _.each(query.trashPrefixFields, function (name) {
          ...
          _.each(query.trashSuffixFields, function (name) {
            if (typeof doc[name] !== 'string') {
              // Presumably a sparse index
              return;
            }
          ...
          return query.apos.docs.db.updateOne({ _id: doc._id }, { $set: $set });
        }
      },
      'afterRescue': {
        async deduplicateRescue(req, doc) {
          ...
          for (const field of query.trashPrefixFields.concat(query.trashSuffixFields)) {
          ...
          async function checkOne(name) {
            ...
            const found = await query.apos.docs.db.findOne(query, { _id: 1 });
            if (found) {
              delete $set[name];
            }
          }
          async function update() {
            ...
            return query.apos.docs.db.updateOne({ _id: doc._id }, action);
          }
        }
      }
    };
  ...
  methods(query, options) {
    return {
      addTrashPrefixFields(fields) {
        query.trashPrefixFields = query.trashPrefixFields.concat(fields);
      },
      removeTrashPrefixFields(fields) {
        query.trashPrefixFields = _.difference(query.trashPrefixFields, fields);
      },
      addTrashSuffixFields(fields) {
        query.trashSuffixFields = query.trashSuffixFields.concat(fields);
      },
      removeTrashSuffixFields(fields) {
        query.trashSuffixFields = _.difference(query.trashSuffixFields, fields);
      },
      ...
      getEditPermissionName() {
        return query.isAdminOnly() ? 'admin' : 'edit-' + query.name;
      },
      // Returns the minimum permission name that should be checked for
      // to determine if this user has full admin privileges for
      // this doc type
      getAdminPermissionName() {
        return query.isAdminOnly() ? 'admin' : 'admin-' + query.name;
      },
      ...
      find(req, criteria, projection) {
        ...
        for (const name of query.__meta.chain.map(entry => name)) {
          if (query.queries[name]) {
            const result = fn(query, query);
            if (result.builders) {
          ...
          if (query.extendQueries[name]) {
            wrap(query.builders, query.extendQueries[name].builders || {});            
            wrap(query.methods, query.extendQueries[name].methods || {});            
          }
        }
        // Add query builders for each field in the schema. Doing it now allows
        // normal overrides of these generic query builders by subclasses
        query.apos.schemas.addQueryBuilders(query.schema, {
          override: [ 'tags' ]
        }, query);
      ...
      allowedSchema(req) {
        let disabled;
        let type;
        const schema = _.filter(query.schema, function (field) {
          return !field.permission || query.apos.permissions.can(req, field.permission);
        });
        ...
        if (typeIndex !== -1) {
          ...
          // on locale settings
          disabled = query.apos.pages.getOption(req, 'disabledTypes');
          if (disabled) {
      ...
      composeSchema() {
        ...
        query.schema = query.apos.schemas.compose(query.options);
        ...
        _.each(query.schema, function (field) {
          if (_.includes(forbiddenFields, field.name)) {
            throw new Error('Doc type ' + query.name + ': the field name ' + field.name + ' is forbidden');
          }
        });
        query.patchAdminPermissionInSchema();
      },
      ...
      patchAdminPermissionInSchema() {
        ...
        _.each(fieldNames, function (fieldName) {
          const field = _.find(query.schema, { name: fieldName });
          if (field) {
            field.permission = query.getAdminPermissionName();
          }
        });
      },
      ...
      async autocomplete(req, query) {
        ...
        _query.projection(query.getAutocompleteProjection(), query);
        ...
        if (query.term && query.term.toString && query.term.toString().length) {
          let term = query.apos.launder.string(query.term);
        ...
        // query builders in API calls like this one, knowing that the field definition
        // the browser sent us is one the developer created
        query.applyBuildersSafely(query.field.builders || {});
        ...
        if (query.values) {
          docs = query.apos.utils.orderById(query.values, docs);
        }
        const results = _.map(docs, doc => {
          const response = {
            label: query.getAutocompleteTitle(doc, query),
            value: doc._id
          };
      ...
      async addSearchIndexListener() {
        query.on('apostrophe-search:index', 'searchIndexBySchema', async function (doc, texts) {
          if (doc.type !== query.name) {
            return;
          }
          await query.apos.schemas.indexFields(query.schema, doc, texts);
        });
      },
      ...
      addSortifyMigration(field) {
        ...
        return query.apos.migrations.addSortify(query.__meta.name, { type: query.name }, field);
      },
      getBrowserData(req) {
        if (!req.user) {
          return;
        ...
        data.schema = query.allowedSchema(req);
        return data;
      }
    };
  },
  queries(query, query) {
    return {
      builders: {
        ...
        and: {
          set(c) {
            ...
            const criteria = currsor.get('criteria');
            if (!criteria) {
              query.criteria(c);
            } else {
              query.criteria({
        ...
        // `.projection({...})` sets the MongoDB projection. You can also
            ...
            for (const [ key, val] of Object.entries(projection)) {
              ...
              if (key.toString().substr(0, 1) === '_') {
                ...
                if (!query.projectComputedField(key, add)) {
                  query.apos.utils.warn(query.__meta.name + ': a projection cannot find a computed field (' + key + ') unless it is _url\nor the name of a forward join in the schema for the type being found.\nThis does not mean the field will not work, but it is on you to know\nwhat fields power it, or if they are even coming from the doc itquery.');
                } else {
                  // We don't get the _ property itquery
        ...
        // a default.
        //
        // This query builder is called by apostrophe-pieces based on its
        ...
        skip: {
          launder(query, s) {
            return query.apos.launder.integer(s, 0, 0);
          },
          safeFor: 'public'
        },
        ...
        perPage: {
          def: undefined,
          launder(query, i) {
            return query.apos.launder.integer(i, 1, 1);
          },
          safeFor: 'manage'
        },
        ...
        permission: {
          finalize(query) {
            ...
            if (permission !== false) {
              ...
              query.and(query.apos.permissions.criteria(query.req, permission || ('view-' + typeSuffix)));
            }
          },
          after: function(query, results) {
            // In all cases we mark the docs with ._edit if
            // the req is permitted to do that
            query.apos.permissions.annotate(query.req, 'edit-doc', results);
          }
        },
        ...
        // must exist in the title, tags or other text schema fields of
        ...
        autocomplete: {
          prefinalize: require('./autocomplete.js')(query, options),
          safeFor: 'public',
          launder(query, s) {
            return query.apos.launder.string(s);
          }
        },
          ...
          finalize(query) {
            ...
            if (search) {
              if (query.get('regexSearch')) {
                ...
                // A query builder like the `geo` query builder of apostrophe-places
                // has warned us that `$near` or another operator incompatible
                // with `$text` is present. We must dumb down to regex search
                query.and({
                  highSearchText: query.apos.utils.searchify(search)
                });
              } else {
        ...
        // `.tag('blue')` limits results to those which include the
        // tag `blue` in their `tags` property.
        ...
        // `.tags([ 'blue', 'red' ])` limits results to those which include at least one
        ...
        tags: {
          ...
          launder(query, s) {
            ...
            return s.map(value => query.apos.launder.string(value));
          }
        },
        ...
        trash: {
          ...
          safeFor: 'manage',
          launder(s) {
            return query.apos.launder.booleanOrNull(s);
          },
          choices() {
        ...
        explicitOrder: {
          ...
          after(query, results) {
            ...
            const property = query.get('explicitOrderProperty');
            const temp = query.apos.utils.orderById(values, results, property);
            let i;
        ...
        addUrls: {
          ...
          async after(query, results) {
            ...
            for (let type of interesting) {
              await query.apos.docs.getManager(type).addUrls(req, byType[type]);
            }
          }
        },
        ...
        pageUrl: {
          ...
          after(results) {
            for (const result of results) {
              if (result.slug && query.apos.pages.isPage(result)) {
                const url = query.apos.pages.getBaseUrl(query.req);
                result._url = url + query.apos.prefix + result.slug;
              }
            }
          }
        },
        ...
        areas: {
          ...
          async after(results) {
            ...
            for (const doc of results) {
              const areasInfo = [];
              query.apos.areas.walk(doc, function(area, dotPath) {
              ...
              if (areasInfo.length) {
                ...
                if (req.areasLoadedFor[doc._id] >= 5) {
                  query.apos.utils.warn('WARNING: reached maximum area loader recursion level. Doc _id is ' + doc._id + '. Are you using projections for all joins?');
                  return;
                }
            ...
            if (req.deferWidgetLoading && (!req.loadingDeferredWidgets)) {
              ...
              for (const type of types) {
                let manager = query.apos.areas.getWidgetManager(type);
                if (!manager) {
                  query.apos.areas.warnMissingWidgetType(type);
        ...
        // indexed by id in `_originalWidgets`. This query builder
        ...
        originalWidgets: {
          ...
          after(results) {
            for (const result of results) {
              ...
              query.apos.areas.walk(result, function(area, dotPath) {
                for (const item of area.items || []) {
      ...
      methods: {
        ...
        // `.safeQueryBuilders([ 'free' ])` marks the `free` query builder, which must
        // have a `launder` method, as being safe to populate via `applyBuildersSafelyqueryStringToQueryBuilders`
        // for this particular query. Chainable.

        safeQueryBuilders(builders) {
          for (const builder of builders) {
            query.builders[builder].safeFor = 'public';
          }
          return query;
        },
        ...
        async toDistinct(query, property) {
          await query.finalize();

          if (!query.get('distinctCounts')) {
            return query.db.distinct(property, query.get('criteria'));
          } else {
          ...
          async function distinctCounts() {
            ...
            const results = await query.db.aggregate(pipeline).toArray();
            const counts = {};
        ...
        async toChoices(query, property, options) {
          options = options || {};
          if (options.counts) {
        ...
        async toMongo() {
          ...
          if (query.get('log') || process.env.APOS_LOG_ALL_QUERIES) {
            query.apos.utils.log(require('util').inspect(criteria, { depth: 20 }));
          }
        ...
        projectUrlField(add) {
          ...
          let manager = type && query.apos.docs.getManager(type);
          let fields;
          if (manager) {
            fields = manager.getUrlFields();
          } else {
            fields = query.apos.docs.getDefaultUrlFields();
          }
        ...
        // generic query obtained from `apos.docs.find`
      ...
      lowLevelMongoCursor(req, criteria, projection, options) {
        const mongo = query.db.find(criteria).project(projection);
      ...
      clone() {
        const subquery = query.find(query.req);
      ...
      handleFindArguments({ req, criteria, projection}) {
        if (!req || !req.res) {
      ...
      // Invoke builders whose names are present in a query object (often from req.query), skipping all
      // builders not declared as safe for the given domain (such as "public" or "manage").
      // Builders declaring themselves as safe must implement a `launder` function to
      // clean up the data. Never trust a browser.
      //
      // If `safeFor` is not specified or is set to `manage`, we assume any query builder that
      // has a launder method is suitable for our use.

      applyBuildersSafely(query, safeFor) {
        for (const [name, value] of query) {
          ...
          if (safeFor && safeFor !== 'manage' && safeFor !== query.builders[name].safeFor) {
            return;
          }
      ...
      finalizeSort() {
        ...
        if (sort) {
          ...
          let manager = query.apos.docs.getManager(query.get('type') || 'apostrophe-page');
          if (!(manager && manager.schema)) {
            return;
          }
