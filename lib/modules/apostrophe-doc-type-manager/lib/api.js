...
module.exports = function(self, options) {
  ...
  // admin bar menu item visibility. When extended permissions
  // are not in effect, also determines which types are
  // candidates for queries that could involve updates.
  // When extended permissions are in effect, the return value
  // may be an array, as both insert- and update- imply a need
  // for admin bar menu item access
  ...
  self.getEditPermissionName = function() {
    ...
    if (self.apos.permissions.extended) {
      return [ 'insert-' + self.name, 'update-' + self.name, 'updateany-' + self.name ];
    } else {
  ...
  // this doc type. When extended permissions are not in effect,
  // also used to help build query criteria
