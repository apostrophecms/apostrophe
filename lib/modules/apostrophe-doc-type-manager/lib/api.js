...
module.exports = function(self, options) {
  ...
  self.deduplicateTrash = async function(req, doc) {
    ...
    return await self.apos.docs.db.update({ _id: doc._id }, { $set: $set });
  };
  ...
  self.deduplicateRescue = async function(req, doc) {
    let deduplicateKey = doc.workflowGuid || doc._id;
    let prefix = 'deduplicate-' + deduplicateKey + '-';
    let suffix = '-deduplicate-' + deduplicateKey;
    let $set = {};
    ...
    for (field of self.trashPrefixFields.concat(self.trashSuffixFields)) {
      await checkOne(field);
    }
    await update();

    async function checkOne(name) {
      const query = { type: self.name, _id: { $ne: doc._id } };
      ...
      found = await self.apos.docs.db.findOne(query, { _id: 1 });
      if (found) {
        delete $set[name];
      }
    }
    ...
    async function update() {
      ...
      return await self.apos.docs.db.update({ _id: doc._id }, action);
    }
  };
  ...
  self.find = function(req, criteria, projection) {
    return self.createRelatedTypeSync('cursor', {
      apos: self.apos,
      module: self,
      req: req,
      criteria: criteria,
  ...
  self.patchAdminPermissionInSchema = function() {
    const fieldNames = [ '_editUsers', '_editGroups' ];
  ...
  self.autocomplete = async function(req, query) {
  ...
  self.addSearchIndexListener = async function() {
