...
module.exports = function(self, options) {
  ...
  // When extended permissions are not in effect- returns the minimum permission name that
  // should be checked for to determine if this user has some edit privileges for
  // this doc type (not necessarily every instance of it). Not used when
  // extended permissions are in effect. Used to build query criteria by type
  ...
  // Returns true if the admin bar menu item for this piece type should be displayed,
  // based on the permissions of `req`. May be called without `req` from a template.

  self.isAdminBarNeeded = function(req) {
    if (!req) {
      req = self.apos.templates.contextReq;
    }
    if (self.apos.permissions.extended) {
      // Test whether we have at least one of these permissions
      return _.find([ 'insert-' + self.name, 'update-' + self.name, 'updateany-' + self.name ], function(permission) {
        return self.apos.permissions.can(req, permission);
      });
    } else {
      return self.apos.permissions.can(req, 'edit-' + self.name);
    }
  };
  ...
  self.getSometimesPermissionNames = function(verb) {
    ...
    if (verb === 'trash') {
      return [ 'update-' + self.name, 'trash-' + self.name ];
    }
  ...
  self.getAlwaysPermissionNames = function(verb) {
    ...
    if (verb === 'trash') {
      return [ 'updateany-' + self.name, 'trash-' + self.name ];
    }
    return verb + 'any-' + self.name;
  };

  // When extended permissions are not in effect, returns the minimum permission
  // name that should be checked for to determine if this user has full admin
  // privileges for this doc type. Used to build query criteria by type
