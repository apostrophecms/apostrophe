    ...
    self.enableSortable = function () {
      var sortableOptions = {
        ...
        update: function(event, ui) {
          ...
          self.$choices.find('[data-chooser-choice]').each(function () {
            var $choice = $(this);
            proposedChoices.push({ value: $choice.attr('data-chooser-choice') });
          });

          // account for all original items
          _.each(proposedChoices, function(proposed) {
            var matched = false;
            _.each(self.choices, function(choice) {
              if (proposed.value === choice.value) {
                matched = true;
              }
            });
            if (matched === false) {
