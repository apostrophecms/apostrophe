apos.define('apostrophe-doc-type-manager-chooser', {
  ...
  construct: function(self, options) {
    ...
    self.add = function(_id) {
      let existing = _.find(self.choices, { value: _id });
      if (existing) {
    ...
    self.remove = function(_id, refresh) {
      if (self.removed) {
        ...
        let removed = _.find(self.choices, { value: _id });
        if (removed) {
    ...
    self.refresh = function(options) {
      ...
      self.refreshing++;
      let $currentInlineRelationships;
      ...
      return self.api('chooser-choices', { choices: self.choices, field: self.field, validate: true }, function(result) {
        ...
        return async.eachSeries(self.choices || [], function(choice, callback) {
          let $current = $currentInlineRelationships.filter('[data-relationship-inline="' + choice.value + '"]');
          let $new = self.$choices.find('[data-relationship-inline="' + choice.value + '"]');
          if ($current.length) {
          ...
          let compare = JSON.stringify(self.choices);
    ...
    self.convertInlineRelationships = function(callback) {
      return async.eachSeries(self.choices, function(choice, callback) {
        let $relationship = self.$choices.find('[data-relationship-inline="' + choice.value + '"]');
    ...
    self.enableLinks = function() {
      self.link('apos-delete', 'item', function($button, _id) {
        let index = _.findIndex(self.choices, { value: _id });
        if (index === -1) {
          return;
        }
        self.remove(_id, false);
        let $choices = self.$el.find('[data-choices]>.apos-chooser-choice');
      ...
      self.link('apos-relate', 'item', function($button, _id) {
        let choice = _.find(self.choices, { value: _id });
        if (!choice) {
          return;
        }
        let editorType = self.field.relationshipEditor || self.manager.getToolType('relationship-editor');
    ...
    self.enableAutocomplete = function() {
      ...
      if (self.options.pluralLabel) {
        let placeholder = self.$autocomplete.attr('placeholder');
        if (placeholder) {
      ...
      self.$autocomplete.autocomplete({
        ...
        response: function(event, ui) {
          ...
          let filtered = _.filter(content, function(datum) {
    ...
    self.enableSortable = function () {
      let sortableOptions = {
        ...
        update: function(event, ui) {
          let $choices = self.$el.find('[data-chooser-choice]');
    ...
    self.decorateManager = function(manager, options) {
      ...
      let superBeforeShow = manager.beforeShow;
      ...
      let superEnableCheckboxEvents = manager.enableCheckboxEvents;
      ...
      manager.getIds = function() {
        return _.map(manager.chooser.choices, 'value');
      };

      let superReflectChoicesInCheckboxes = manager.reflectChoicesInCheckboxes;
      ...
      let superManagerSave = manager.save;
      ...
      let superManagerCancel = manager.cancel;
    ...
    self.pieceIsRelevant = function(piece) {
      if (Array.isArray(self.field.withType)) {
        return _.includes(self.field.withType, piece.type);
      } else {
