  ...
  construct: function(self, options) {
    ...
    self.enableBreadcrumbEvents = function() {
      self.$el.on('click', '[data-modal-breadcrumb]:first .apos-modal-breadcrumb-item', function() {
        let $item = $(this);
        let index = $item.data('aposBreadcrumbIndex');
        let slides = self.getSlides();
        let cancels = slides.length - index - 1;
    ...
    self.resetEl = function() {
      ...
      self.$shell = self.$el;
      let $content = self.$el.find('[data-modal-content]:first');
      self.$el = $content;
    };
    ...
    self.getSlideableAncestorEl = function() {
      let stack = apos.modalSupport.stack;
      if (!stack.length) {
        return false;
      }
      let $slideableAncestorEl = stack[stack.length - 1];
    ...
    self.getLastSlide = function() {
      let $slides = self.$el.findSafe('[data-modal-content]', '[data-modal-content]');
      if ($slides.length <= 1) {
        return self;
      }
      let $last = $slides.eq($slides.length - 1);
    ...
    self.getSlides = function() {
      let $slides = self.$el.find('[data-modal-content]');
      let slides = [];
    ...
    self.slideIn = function() {
      ...
      let $currentControls = self.$slideableAncestorEl.find('[data-modal-controls]:visible:first');
      ...
      let $currentFilters = self.$slideableAncestorEl.find('[data-modal-filters]:visible:first');
      self.$previousFilters = $currentFilters;
      $currentFilters.hide();
      $currentFilters.after(self.$modalFilters);

      let $currentInstructions = self.$slideableAncestorEl.find('[data-modal-instructions]:visible:first');
      self.$previousInstructions = $currentInstructions;
      $currentInstructions.hide();
      $currentInstructions.after(self.$instructions);

      let $wrapper = self.$slideableAncestorEl.find('[data-modal-contents]:first');
    ...
    self.indicateCurrentModal = function(adding) {
      if (adding) {
        ...
        let top = apos.modalSupport.getLatestModal();
        if (top !== self) {
      ...
      let latest = apos.modalSupport.getLatestModal();
      if (latest) {
    ...
    self.resizeContentHeight = function() {
      ...
      let $container = self.getSlideableAncestorEl() || self.$el;

      let headerHeight = $container.find('.apos-modal-header').outerHeight() || 0;
      let breadcrumbHeight = $container.find('.apos-modal-breadcrumb').outerHeight() || 0;
      let footerHeight = $container.find('.apos-modal-footer').outerHeight() || 0;

      let contentHeight = 'calc(100vh - 50px - ' + (headerHeight + breadcrumbHeight) + 'px)';
      ...
      let $body = $content.find('.apos-modal-body');
      ...
      let $chooser = $content.find('.apos-chooser');
      if ($chooser.length) {
    ...
    self.refreshBreadcrumb = function() {
      let $ancestor = self.$slideableAncestorEl;
      if (!$ancestor) {
      ...
      let ancestor = $ancestor.data('aposModal');
      let $breadcrumb = $ancestor.find('[data-modal-breadcrumb]');
      $breadcrumb.html('');
      let slides = ancestor.getSlides();
    ...
    self.slideOut = function() {
      ...
      let $slides = self.$slideableAncestorEl.findSafe('[data-modal-content]', '[data-modal-content]');
    ...
    self.stackPop = function() {
      ...
      let $current = apos.modalSupport.getTopModalOrBody();
      ...
      let $blackoutContext = apos.modalSupport.getTopModalOrBody();
      let $blackout = $blackoutContext.find('.apos-modal-blackout');
    ...
    self.applyBlackout = function() {
      ...
      let $blackoutContext = apos.modalSupport.getTopModalOrBody();
      let $blackout = $('<div class="apos-modal-blackout"></div>');
      if ($blackoutContext.prop('tagName') === 'BODY') {
        var interval = setInterval(function() {
          let contextHeight = $(document).height();
    ...
    self.focusFirstFormElement = function() {
      ...
      let $firstElm = self.$el.find('.apos-modal-body form fieldset:first input');
...
apos.modalSupport = {
  ...
  closeTopModal: function() {
    let topModal = apos.modalSupport.getTopModalOrBody();
