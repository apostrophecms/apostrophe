...
apos.define('apostrophe-images-manager-modal', {
  ...
  construct: function(self, options) {
    ...
    let superEnableButtonEvents = self.enableButtonEvents;
    self.enableButtonEvents = function() {
      ...
      self.$el.on('dblclick', '[data-edit-dbl-' + options.name + ']', function() {
        let id = $(this).attr('data-edit-dbl-' + options.name);
    ...
    let superAfterRefresh = self.afterRefresh;
    ...
    let superDisplayChoiceInCheckbox = self.displayChoiceInCheckbox;
    self.displayChoiceInCheckbox = function(id, state) {
      let $checkbox = superDisplayChoiceInCheckbox(id, state);
