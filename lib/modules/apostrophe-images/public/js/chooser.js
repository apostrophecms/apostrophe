...
apos.define('apostrophe-images-chooser', {
  ...
  construct: function(self, options) {
    self.finalize = function(callback) {
      // autocrop if needed
      let aspectRatio = self.field.hints && self.field.hints.aspectRatio;
      if (!aspectRatio) {
        return callback(null);
      }
      aspectRatio = aspectRatio[0] / aspectRatio[1];
      let manager = apos.docs.getManager(self.field.withType);
      return self.get(function(err, choices) {
        ...
        return async.eachSeries(choices, function(choice, callback) {
          ...
          function crop(callback) {
            ...
            let height = attachment.width / aspectRatio;
            let left = 0;
            let top = (attachment.height - height) / 2;
    ...
    self.link('apos-focal-point', 'item', function($button, _id) {
      ...
      let editorType = self.field.focalPointEditor || self.manager.getToolType('focal-point-editor');
