...
module.exports = function(self, options) {
  ...
  self.enableHelpers = function() {
    ...
    self.addHelpers({
      getImageAlt: function(image) {
        if (options.enableAltField) {
          // If the alt property doesn't exist, we want to set an empty string
          return image.alt || '';
        }
  ...
  self.getSuggestions = async function(req, term, filters) {
    ...
    const { published, trash, orientation, fileType, mediaSource } = filters;
    const publishedVal = getFilterValue(published);
    const trashVal = getFilterValue(trash);
    ...
    const images = await self.find(req, {})
      ...
      .autocomplete(search).limit(15).toArray();
