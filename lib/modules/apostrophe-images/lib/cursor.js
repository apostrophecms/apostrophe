...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // the available images extensions come from
    // used media sources in the project (eg. `apostrophe-media-sources-unsplash`).
    // Accessible programmatically using self.find(req).mediaSources('apostrophe')`.
    // Available options are: `jpg`, `png`, `gif` and `svg` if `svgImages` is enabled.

    self.addFilter('mediaSources', {
      finalize: function() {
        const mediaSources = self.get('mediaSources');
        if (!mediaSources) {
          return;
        }

        const criteria = {
          'attachment.mediaSources': mediaSources
        };
      ...
      choices: function(callback) {
        const {choices} = options.module.filters
          .find((filter) => filter.name === 'mediaSources');
