...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    options.addFields = [
      ...
      options.apos.permissions.extended ?
      {
        type: 'permissions',
        name: 'permissions',
        label: 'Permissions'
  ...
  construct: function(self, options) {
    ...
    self.modulesReady = function() {
      ...
      if (!self.apos.permissions.extended) {
        ...
        self.setPermissionsDefaults();
      }
      self.addToAdminBarIfSuitable();
    };

    // For non-extended permissions. Extended permissions fields set up their own choices
    //
    ...
    //
    // Patch the default value of the permissions field via the getDefaults method of the
    // field type now, before we send the schema to the browser side, so it doesn't need
    // a redundant implementation of getDefaults
    self.setPermissionsDefaults = function() {
      var permissions = _.find(self.schema, { name: 'permissions' });
      if (!permissions) {
        return;
      }
      if (permissions.type !== 'permissions') {
        return;
      }
      permissions.def = self.apos.schemas.fieldTypes[permissions.type].getDefault();
    };
