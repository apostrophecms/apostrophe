...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    if (self.options.baseUrl && (!self.apos.baseUrl)) {
      self.apos.error('WARNING: you have baseUrl set as an option to the `apostrophe-express` module.');
      self.apos.error('Set it as a global option (a property of the main object passed to apostrophe).');
      self.apos.error('When you do so other modules will also pick up on it and make URLs absolute.');
    }
  },
  ...
  construct: function(self, options) {
    ...
    self.sessions = function() {
      ...
      if (sessionOptions.secret === 'you should have a secret') {
        self.apos.error('WARNING: No session secret provided, please set the `secret` property of the `session` property of the apostrophe-express module in app.js');
      }
    ...
    self.addListenMethod = function() {
      self.apos.listen = function() {
        ...
        if (port === undefined) {
          port = 3000;
          self.apos.log("I see no data/port file, port option, forcePort option, or PORT environment variable, defaulting to port " + port);
        }
        ...
        if (address === undefined) {
          address = false;
          self.apos.log('I see no data/address file, address option, forceAddress option, or ADDRESS environment variable, listening on all interfaces');
        }
        ...
        if (address !== false) {
          self.apos.log('Listening on http://' + address + ':' + port);
          self.server = self.apos.baseApp.listen(port, address);
        } else {
          self.apos.log('Listening at http://localhost:' + port);
