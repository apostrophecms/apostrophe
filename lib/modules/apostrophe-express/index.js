...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.requiredMiddleware = function() {
      ...
      // in app.js
      let bodyParserOptions = _.extend({
        json: {
          limit: '16mb'
        },
        urlencoded: {
    ...
    self.useModuleMiddleware = function(when) {
      self.apos.app.use(function(req, res, next) {
        const middleware = self.moduleMiddlewareByWhen[when];
        let i = 0;
        return nextMiddleware();
        function nextMiddleware() {
          if (i < middleware.length) {
            middleware[i++](req, res, nextMiddleware);
          } else {
            return next();
          }
        }
    ...
    self.enableCsrf = async function() {
    ...
    self.compileCsrfExceptions = async function() {
      let list = (self.options.csrf && self.options.csrf.minimumExceptions) || [ '/login', '/password-reset', '/password-reset-request' ];
      list = list.concat((self.options.csrf && self.options.csrf.exceptions) || []);
      await self.emit('csrfExceptions', list);
    ...
    self.listen = async function() {
      ...
      if (port === undefined) {
        port = 3000;
        self.apos.utils.log("I see no data/port file, port option, forcePort option, or PORT environment variable, defaulting to port " + port);
      }
      ...
      if (address === undefined) {
        address = false;
        self.apos.utils.log('I see no data/address file, address option, forceAddress option, or ADDRESS environment variable, listening on all interfaces');
      }
    ...
    self.on('apostrophe:run', 'listenIfNotTask', async function(isTask) {
      if (isTask) {
        return;
      }
      await self.listen();
      // Emit the apostrophe-express:afterListen event
      self.emit('afterListen');
    });

    // On the apostrophe:destroy event, destroys the HTTP server object, freeing the port.

    self.on('apostrophe:destroy', 'destroyServer', async function() {
    ...
    // modules can respond to the `csrfExceptions` e vent.
    //
    self.on('apostrophe:afterInit', 'addCsrfAndFindModuleMiddleware', async function() {
    ...
    self.findModuleMiddleware = function() {
      ...
      _.each(self.apos.modules, function(module, name) {
        ...
        let prop = module.expressMiddleware;
      ...
      do {
        let beforeIndex = _.findIndex(labeledList, function(item) {
          return item.before;
        });
        if (beforeIndex === -1) {
          break;
        }
        let item = labeledList[beforeIndex];
        let otherIndex = _.findIndex(labeledList, { module: item.before });
      ...
      _.each(labeledList, function(item) {
        let prop = item.middleware;
        let when = item.when || 'afterRequired';
