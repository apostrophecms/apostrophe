...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.csrf = function(req, res, next) {
      ...
      if (!self.csrfCookieName) {
        self.csrfCookieName = (self.options.csrf && self.options.csrf.name) ||
      ...
      if ((req.method === 'GET') || (req.method === 'HEAD') || (req.method === 'OPTIONS') || (req.method === 'TRACE')) {
        ...
        res.cookie(self.csrfCookieName, token);
      } else {
        ...
        if ((!req.cookies[self.csrfCookieName]) || (req.get('X-XSRF-TOKEN') !== req.cookies[self.csrfCookieName]) || (req.session['XSRF-TOKEN'] !== req.cookies[self.csrfCookieName])) {
          res.statusCode = 403;
