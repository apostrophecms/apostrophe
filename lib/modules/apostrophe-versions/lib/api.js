...
module.exports = function(self, options) {
    ...
    return async.series({
      ...
      compare: function(callback) {
        ...
        function compare(i) {
          ...
          return self.compare(doc, versions[i + 1], versions[i], function(err, changes) {
            if (err) {
              return callback(err);
            }
  ...
  self.compare = function(doc, version1, version2, callback) {
    ...
    return setImmediate(function() {
      return callback(null, compareObjects(schema, version1.doc, version2.doc));
    });
    ...
    function compareObjects(schema, version1, version2) {
      ...
      _.each(version1, function(val, key) {
        ...
        if (change) {
          change.changes = compare(field, change.old, change.current);
          changes.push(change);
        }
      });
      ...
      function compare(field, old, current) {

        if (field.type === 'array') {

          return compareArrays(field.schema, old, current);
          ...
          var indexer = self.apos.schemas.indexers[field.type];
          if (indexer) {
      ...
      _.each(version1.items, function(widget1) {
        ...
        if (!newVersion) {
          changes.push({
            ...
            old: widget1,
            manager: manager,
            changes: compareObjects(manager.schema, widget1, { type: widget1.type })
          });
          importantChanges++;
          return;
        }
        if (!_.isEqual(newVersion, widget1)) {
          ...
          if (manager.compare) {
            change.changes = manager.compare(widget1, newVersion);
          } else if (manager.schema) {
            change.changes = compareObjects(manager.schema, widget1, newVersion);
          }
          importantChanges++;
          changes.push(change);
        }
      ...
      _.each(version2.items, function(widget2) {
        ...
        if (!oldVersion) {
          changes.push({
            ...
            current: widget2,
            manager: manager,
            changes: compareObjects(manager.schema, { type: widget2.type }, widget2)
          });
          importantChanges++;
        }
      });
    ...
    function compareArrays(schema, version1, version2) {
      version1 = version1 || [];
      version2 = version2 || [];
      var changes = [];
      var importantChanges = 0;
      _.each(version1, function(item1) {
        var newVersion = _.find(version2, { _id: item1._id });
        if (!newVersion) {
          changes.push({
            action: 'remove',
            old: item1,
            ...
            changes: compareObjects(schema, item1, { type: item1.type })
          });
          importantChanges++;
          return;
        }
        if (!_.isEqual(newVersion, item1)) {
          ...
          change.changes = compareObjects(schema, item1, newVersion);
          importantChanges++;
          changes.push(change);
        }
      });
      _.each(version2, function(item2) {
        var oldVersion = _.find(version1, { _id: item2._id });
        if (!oldVersion) {
          changes.push({
            action: 'add',
            current: item2,
            ...
            changes: compareObjects(schema, { type: item2.type }, item2)
          });
          importantChanges++;
        }
      });
      ...
      if ((version1.length === version2.length) && (!importantChanges)) {
        ...
        var oldRank;
        var currentRank;
        var moved = _.find(version2, function(widget2, i) {
          if (ranksById[widget2._id] !== i) {
            oldRank = ranksById[widget2._id];
            currentRank = i;
            return true;
          }
