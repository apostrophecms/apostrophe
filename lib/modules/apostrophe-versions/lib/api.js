...
module.exports = function(self, options) {
  ...
  self.ensureIndexes = function() {
    return self.db.createIndex({ docId: 1, createdAt: -1 });
  };

  self.on('apostrophe-doc-type-manager:afterSave', 'addVersion', async function(req, doc, options) {

    let pruned = self.apos.utils.clonePermanent(doc);
    let version = {
  ...
  self.pruneOldVersions = async function(doc) {

    let now = new Date();

    let last = null;
    let cursor = self.db.findWithProjection({ createdAt: { $lt: now }, docId: doc._id }, { createdAt: 1, _id: 1, author: 1 }).sort({ createdAt: -1 });
    const next = Promise.promisify(cursor.next);
    ...
    async function one() {
      ...
      if (!remove) {
        ...
        await self.db.remove({ _id: version._id });
      }
      return one();
    }
  };
  ...
  self.revert = async function(req, version) {
    let unversionedFields = [];
    ...
    return self.apos.docs.update(req, newDoc);
  };
  ...
  self.find = async function(req, criteria, options) {
    const cursor = self.db.findWithProjection(criteria);
    ...
    if (versions.length) {
      ...
      const doc = await self.apos.docs.find(req, { _id: docId }).published(null).permission('edit').toObject();
      if (!doc) {
        throw 'notfound';
      }
  ...
  self.compare = async function(req, doc, version1, version2) {
    ...
    for (const docType of docTypes) {
      ...
      const manager = self.apos.docs.getManager(docType);
      if (!manager) {
        continue;
      }
      const docs = manager.find(req, { _id: { $in: ids } }).published(null).toArray();
    ...
    function compareObjects(schema, version1, version2) {
      ...
      function compareField(field, old, current) {
        ...
        if (field.type === 'array') {
          ...
          let currentLines = [];
          let currentText = '';
          let fieldType = self.apos.schemas.getFieldType(field.type);
    ...
    function compareAreas(version1, version2) {
      ...
      _.each(version1.items, function(widget1) {
        let manager = self.apos.areas.getWidgetManager(widget1.type);
        if (!manager) {
      ...
      _.each(version2.items, function(widget2) {
        let manager = self.apos.areas.getWidgetManager(widget2.type);
        if (!manager) {
          // No warning message here because it may have been removed deliberately
          // from a later version of the site
          return;
