  ...
  construct: function(self, options) {
    ...
    self.beforeShow = function(callback) {
      self.$el.on('click', '[data-open-changes]', function() {
        let $toggle = $(this);
        let $version = $toggle.closest('[data-version]');
        let currentId = $version.attr('data-version');
        let oldId = $version.attr('data-previous');
        let $versionContent = $version.find('[data-changes]');
        let noChanges = self.$el.find('[data-no-changes]').attr('data-no-changes');
