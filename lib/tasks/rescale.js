...
module.exports = function(self, argv, callback) {
  ...
  return self.files.count(function(err, total) {
    ...
    var n = 0;
    self.forEachFile({}, argv.parallel || 1,
          ...
          function(callback) {
            ...
            async.series([
              function(resumeCallback) {
                ...
                var url = self.uploadfs.getUrl() + '/files/' + file._id + '-' + file.name + '.' + resumeTestSize + '.' + file.extension;
                if (url.substr(0, 1) === '/') {
                  url = argv.resume + url;
                }
              ...
              function(callback) {
                return self.uploadfs.copyOut(originalFile, tempFile, function(err) {
                  if (err) {
                    console.error('WARNING: could not access ' + originalFile + ', perhaps it was deleted');
                    return fileCallback(null);
                  }
              ...
              function(callback) {
                if (!argv['crop-only']) {
                  return self.uploadfs.copyImageIn(tempFile, originalFile, function(err) {
                    if (err) {
                      console.error('WARNING: could not work with ' + tempFile + ' even though copyOut claims it is there');
                      return fileCallback(null);
                    }
                    return callback(null);
                  });
