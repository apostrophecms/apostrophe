...
module.exports = function(self, callback) {
  ...
  function migrateTypeSettings(callback) {
  ...
  function migrateAreas(callback) {
  ...
  function addHighSearchWords(callback) {
    var needed = false;
    return self.forEachPage({ highSearchText: { $exists: 1 }, highSearchWords: { $exists: 0 } }, function(page, callback) {
      ...
      page.highSearchWords = page.highSearchText.split(/ /);
      return self.pages.update({ _id: page._id }, { $set: { highSearchWords: page.highSearchWords } }, callback);
    }, callback);
  }

  async.series([ addTrash, moveTrash, moveSearch, trimTitle, trimSlug, fixSortTitle, fixObjectId, explodePublishedAt, missingImageMetadata, missingFileSearch, fixNullTags, fixNumberTags, fixTimelessEvents, normalizeFiles, fixButtons, fixCrops, migrateTypeSettings, migrateAreas, addHighSearchWords ], function(err) {
    return callback(err);
  });
};
