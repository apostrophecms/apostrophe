...
module.exports = function(self) {
  ...
  self.forEachDocumentInCollection = function(collection, criteria, limit, each, callback) {
    ...
    // add parallelism on top of that. Joy. Fix it.
      ...
      function fill() {
        ...
        function makeTask(id) {
          var fn = function() {
            ...
            return nextObject(function(err, page) {
              error = error || err;
              if (error) {
                return finished(id);
              }
              ...
              if (idsSeen[page._id]) {
                throw 'WTF ID seen twice!';
              }
              idsSeen[page._id] = true;
              return each(page, function(err) {
                error = error || err;
                return finished(id);
              });
            });
        ...
        function finished(id) {
          running--;
          _.remove(queue, function(fn) {
            return fn.id === id;
          });
          if ((error || eof) && (!queue.length)) {
            // End of a long road
