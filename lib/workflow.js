...
module.exports = function(self) {
  ...
  self.workflowUpdatePage = function(page, props, callback) {
  ...
  self.workflowApproveChanges = function(page, callback) {
    if (!page.draft) {
      return setImmediate(callback);
    }
  ...
  self.workflowRequestApproval = function(page, callback) {
    ...
    return self.pages.update({ _id: page._id },
      { $set: { submitDraft: page.submitDraft } },
      callback);
  };
    ...
    return async.eachSeries(slugs, function(slug, mainCallback) {
      return async.series({
        ...
        putPage: function(callback) {
          if (self.permissions.can(req, 'publish-page', page)) {
            published++;
            return self.workflowApproveChanges(page, callback);
          } else {
            submitted++;
            return self.workflowRequestApproval(page, callback);
          }
        }
      }, mainCallback);
    }, function(err) {
