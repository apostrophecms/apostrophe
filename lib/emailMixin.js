...
module.exports = function(self) {
  ...
  self.mixinModuleEmail = function(module) {
    module.email = function(req, from, to, subject, template, data, callback) {
      var options = module.options.email || {};
      if (!callback) {
      ...
      var baseUrl = req.baseUrl || (req.protocol + '://' + req.get('Host'));
      ...
      subject = module.options[template + 'Subject'] || subject;
      ...
      _.defaults(options, {
        // transport and transportOptions are ignored if module.options.mailer
      ...
      if (!module._mailer) {
        if (module.options.mailer) {
          // This will always work with apostrophe-site
          module._mailer = module.options.mailer;
        } else {
      ...
      return module._mailer.sendMail({
        from: fixAddress(from),
        to: fixAddress(to),
        subject: module.renderString(subject, finalData),
        text: module.render(template + '.txt', finalData),
        html: module.render(template + '.html', finalData)
      }, callback);
    };
