...
module.exports = function(self) {
  ...
  function setupPages(callback) {
    self.db.collection('aposPages', function(err, collection) {
      function indexType(callback) {
        self.pages.ensureIndex({ type: 1 }, { safe: true }, callback);
      }
      ...
      function indexPath(callback) {
        ...
        self.pages.ensureIndex({ path: 1 }, { safe: true, unique: true, sparse: true }, callback);
      }
      // For typical getDescendants calls
      function indexPathLevelRank(callback) {
        self.pages.ensureIndex({ path: 1, level: 1, rank: 1 }, { safe: true }, callback);
      }
      function indexSortTitle(callback) {
        self.pages.ensureIndex({ sortTitle: 1 }, { safe: true }, callback);
      }
      function indexStartDate(callback) {
        self.pages.ensureIndex({ startDate: 1 }, { safe: true }, callback);
      }
      function indexStart(callback) {
        self.pages.ensureIndex({ start: 1 }, { safe: true }, callback);
      }
      function indexStartAndSortTitle(callback) {
        self.pages.ensureIndex({ start: 1, sortTitle: 1 }, { safe: true }, callback);
      }
      function indexEndDate(callback) {
        self.pages.ensureIndex({ endDate: 1 }, { safe: true }, callback);
      }
      function indexEnd(callback) {
        self.pages.ensureIndex({ end: 1 }, { safe: true }, callback);
      }
      function indexTags(callback) {
        self.pages.ensureIndex({ tags: 1 }, { safe: true }, callback);
      }
      function indexPublished(callback) {
        self.pages.ensureIndex({ published: 1 }, { safe: true }, callback);
      }
      function indexPublishedAtBackwards(callback) {
        self.pages.ensureIndex({ publishedAt: -1 }, { safe: true }, callback);
      }
      ...
      function indexGeo(callback) {
        self.pages.ensureIndex({ geo: '2dsphere' }, { safe: true }, callback);
      }
      // Must be able to sort case insensitively on names
      function indexSortName(callback) {
        self.pages.ensureIndex({ sortLastName: 1, sortFirstName: 1 }, { safe: true }, callback);
      }
      self.pages = collection;
      async.series([ indexType, indexSlug, indexPath, indexSortTitle, indexStartDate, indexStart, indexEndDate, indexStartAndSortTitle, indexEnd, indexTags, indexPublished, indexPublishedAtBackwards, indexPublishedAtForwards, indexText, indexGeo, indexPathLevelRank, indexSortName ], callback);
    ...
    return self.db.collection('aposCache', function(err, collection) {
      ...
      return async.series({
        keyIndex: function(callback) {
          return self._cache.ensureIndex({ key: 1, cache: 1 }, { safe: true, unique: true }, callback);
        },
  ...
  self.ensureTextIndex = function(callback) {
    return self.pages.ensureIndex( { highSearchText: 'text', lowSearchText: 'text', title: 'text', searchBoost: 'text' }, { weights: { title: 100, searchBoost: 150, highSearchText: 10, lowSearchText: 2 }, safe: true }, callback);
  };
