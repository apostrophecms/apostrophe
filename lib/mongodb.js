...
module.exports = function(self) {
  ...
  function setupPages(callback) {
    self.db.collection('aposPages', function(err, collection) {
      ...
      function indexText(callback) {
        self.pages.ensureIndex( { highSearchText: 'text', lowSearchText: 'text', title: 'text' }, { weights: { title: 100, highSearchText: 10, lowSearchText: 2 }, safe: true }, callback);
      }
      ...
      async.series([indexType, indexSlug, indexPath, indexSortTitle, indexStartDate, indexStart, indexEndDate, indexStartAndSortTitle, indexEnd, indexTags, indexPublished, indexText, indexGeo], callback);
  ...
  self.initCollections = function(callback) {
    return async.series([checkVersion, setupPages, setupVersions, setupFiles, setupVideos, setupRedirects, setupCache ], callback);
  };
