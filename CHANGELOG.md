...
* Rich Text widget's now instantiate with a valid element from the `styles` option rather than always starting with an unclassed `<p>` tag.
...
* Rich Text widget's styles supports a `def` property for specifying the default style the editor should instantiate with.
